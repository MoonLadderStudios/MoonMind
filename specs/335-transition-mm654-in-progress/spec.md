# Feature Specification: Transition Jira Issue MM-654 to "In Progress"

**Feature Branch**: `change-jira-issue-mm-654-to-status-in-pr-a458ea00`
**Created**: 2026-05-09
**Status**: Draft
**Input**: User description: "Change Jira issue MM-654 to status 'In Progress'."

<!-- Moon Spec specs contain exactly one independently testable user story. Use /speckit.breakdown for technical designs that contain multiple stories. -->

## User Story - Move MM-654 into "In Progress"

**Summary**: As a delivery operator, I want the system to move Jira issue MM-654 into the "In Progress" workflow status so that the Jira tracker reflects that implementation work on MM-654 has started.

**Goal**: After the workflow runs, Jira issue MM-654's workflow status is "In Progress", with no other fields on MM-654 mutated and no other Jira issues modified.

**Independent Test**: Can be fully tested by running the workflow against issue key `MM-654` and then re-fetching the issue from the Jira tracker to confirm its current workflow status reads "In Progress" (or, if it was already "In Progress" before the run, that the workflow recorded a no-op rather than performing a redundant transition).

**Acceptance Scenarios**:

1. **Given** `MM-654` is in a workflow status from which a transition leading to "In Progress" is currently available, **When** the workflow runs against `MM-654`, **Then** `MM-654`'s workflow status becomes "In Progress" and the run report names the issue key, the prior status, the chosen transition, and the verified post-transition status.
2. **Given** MM-654 is already in workflow status "In Progress", **When** the workflow runs against `MM-654`, **Then** no transition is performed, no other fields on MM-654 are changed, and the run report describes the outcome as a no-op with the current status `In Progress`.
3. **Given** MM-654 is in a workflow status from which no available transition leads to "In Progress", **When** the workflow runs against `MM-654`, **Then** the workflow stops with an explicit "no matching transition" error, MM-654 is not modified, and the report names the available transition options for operator inspection.
4. **Given** MM-654 has more than one available transition whose target status name matches "In Progress" (case-insensitive), **When** the workflow runs against `MM-654`, **Then** the workflow stops with an explicit ambiguity error, MM-654 is not modified, and the report lists the candidate transitions for operator selection.
...
- MM-654 cannot be located by the configured Jira credentials (does not exist, or is not visible to the active credential): the workflow stops with an explicit "issue not found / not visible" error that names `MM-654`, performs no mutation, and does not silently fall back to any other issue key.
- Authentication or authorization to view or modify MM-654 fails: the workflow stops with a sanitized auth/permission error and performs no mutation; no credentials, tokens, cookies, or auth headers appear in any user-visible output, log, or artifact.
...
- "In Progress" is a discoverable Jira workflow status reachable from MM-654's current status, on the Jira project that owns MM-654.
...
- **DESIGN-REQ-001**: The system MUST update only the Jira issue identified by the literal issue key `MM-654`.
  - Source: User brief — `"Change Jira issue MM-654 to status 'In Progress'."`
  - Scope: in scope
  - Mapped FR: FR-001, FR-008
- **DESIGN-REQ-002**: The system MUST drive MM-654 to the workflow status named `In Progress`.
...
- **FR-001**: System MUST resolve the update target to the literal Jira issue key `MM-654` and reject any attempt to operate on a different issue key inferred from surrounding text.
- **FR-002**: System MUST discover MM-654's currently available workflow transitions before performing any mutation.
...
- **Jira Issue (MM-654)**: The single tracked work item being acted on; identified by its issue key, with attributes for current workflow status and a list of currently available workflow transitions.

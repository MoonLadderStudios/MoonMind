...
**Feature Branch**: `change-jira-issue-mm-653-to-status-in-pr-a018612a`
**Created**: 2026-05-09
**Status**: Draft
...
**Summary**: As a delivery operator, I want the system to move Jira issue MM-653 into the "In Progress" workflow status so that the Jira tracker reflects that implementation work on MM-653 has started.

**Goal**: After the workflow runs, Jira issue MM-653's workflow status is "In Progress", with no other fields on MM-653 mutated and no other Jira issues modified.

**Independent Test**: Can be fully tested by running the workflow against issue key `MM-653` and then re-fetching the issue from the Jira tracker to confirm its current workflow status reads "In Progress" (or, if it was already "In Progress" before the run, that the workflow recorded a no-op rather than performing a redundant transition).
...
- MM-653 cannot be located by the configured Jira credentials (does not exist, or is not visible to the active credential): the workflow stops with an explicit "issue not found / not visible" error that names `MM-653`, performs no mutation, and does not silently fall back to any other issue key.
- Authentication or authorization to view or modify MM-653 fails: the workflow stops with a sanitized auth/permission error and performs no mutation; no credentials, tokens, cookies, or auth headers appear in any user-visible output, log, or artifact.
...
- "In Progress" is a discoverable Jira workflow status reachable from MM-653's current status, on the Jira project that owns MM-653.
...
- **DESIGN-REQ-001**: The system MUST update only the Jira issue identified by the literal issue key `MM-653`.
...
- **FR-006**: System MUST treat the case where MM-653 is already in workflow status `In Progress` as a successful no-op, reporting the current status without invoking a redundant transition.
...
- **SC-005**: The run completes within a single operator-observed cycle, with no manual follow-up retry required when MM-653 is in an eligible source workflow status.

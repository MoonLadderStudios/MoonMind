...
- Intent: `runtime`. The brief asks the system to perform and verify a Jira workflow status change on MM-653; behavior is observable in the Jira tracker and through the run report.
...
- Single story: The brief describes one independently testable outcome (drive MM-653 to `In Progress`); no additional stories are deferred or out of scope.

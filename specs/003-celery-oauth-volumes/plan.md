...
└── tasks.md             # Generated by /agentkit.tasks (Phase 2)
```
...
```text
celery_worker/
└── agentkit_worker.py        # Celery entrypoint and queue routing hooks
...
**Structure Decision**: Extend existing Celery workflow modules (`moonmind/workflows/agentkit_celery/`) and deployment assets (Compose, worker/job images) because the feature modifies container lifecycle, queue routing, and operator runbooks without introducing new service boundaries.

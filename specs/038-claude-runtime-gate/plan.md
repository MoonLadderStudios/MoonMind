...
1. Remove `_run_claude_auth_status_check` and the `MOONMIND_CLAUDE_AUTH_STATUS_COMMAND` env hook so no OAuth subprocesses are invoked.  
...
5. Tests (`tests/unit/agents/codex_worker/test_cli.py`): 
   ...
   - `runtime=claude` with key -> verifies `claude --version` and skips OAuth status.  
   ...
   These tests already exist but will be updated to align with the new gating behavior and ensure no residual OAuth commands execute.
...
With these steps complete, Claude runtime will be fully gated on API-key availability without any residual OAuth tooling, satisfying PR 1 of the larger Claude OAuth removal plan.

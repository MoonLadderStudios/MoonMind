...
- [ ] T009 [P] Add failing unit tests for provider authenticity and actor authorization rejection helpers in `tests/unit/api/routers/test_task_proposals.py` for provider decision ingress (FR-003, FR-004, FR-005, DESIGN-REQ-022, DESIGN-REQ-031)
...
- [ ] T032 Update `api_service/api/routers/task_proposals.py` for GitHub/Jira provider decision ingestion, shared-secret/signature verification, provider event normalization, service invocation, and sanitized responses (FR-003, FR-004, FR-005, SCN-005, DESIGN-REQ-022, DESIGN-REQ-031)
...
- [ ] T034 Update recovery and admin surfaces in `api_service/api/routers/task_proposals.py` to inspect decision history, redeliver or sync external issue state, and controlled-promote through the same validation path (FR-016, SCN-007, DESIGN-REQ-026)

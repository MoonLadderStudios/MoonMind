...
**Branch**: `change-jira-issue-mm-667-to-status-in-pr-a8186bb9` (spec dir: `334-transition-mm667-in-pr`) | **Date**: 2026-05-09 | **Spec**: [/specs/334-transition-mm667-in-pr/spec.md](/work/agent_jobs/workspaces/6c0215bf-c818-4ea4-8749-cdc0e47ac221/repo/specs/334-transition-mm667-in-pr/spec.md)
...
| FR-007 (named errors: no match / ambiguous / not found / missing required fields) | partial | Existing tool errors for unknown issue and required-field surfacing exist (`_fetch_transitions` `expand_fields` enables `transitions.fields` metadata; `JiraToolError` raised on unavailable transition); no current ambiguity error specific to "In Progress" | Inline error mapping during the run produces explicit named outcomes: `no_matching_transition`, `ambiguous_transition`, `issue_not_found`, `missing_required_fields` — no mutation in any error case | Run-report assertion for each error path that applies |

# Quickstart: Verify MM-667 Transitions to "In Progress"

This story is a one-shot agent run that drives `MM-667` to status `In Progress` through MoonMind's trusted Jira tool surface. The independent test confirms either a successful transition or a deterministic named-error stop.
...
- The Jira binding configured with credentials that can read and transition `MM-667` in the project that owns it. **Credentials are NOT exposed to the agent runtime** — they live in the trusted service.
...
Read `MM-667` through the trusted Jira tool surface (or via Jira UI) and record the current `status.name`. Call this `priorStatus`.

### Step 2 — Run the workflow

Trigger the MoonMind agent run that consumes this spec. The agent will perform calls 1 → 4 from `contracts/transition-mm667.md` (skipping 2/3 on a no-op, skipping 3/4 on a `stopped:*` outcome).
...
Fetch `MM-667` directly from the Jira tracker (UI or trusted tool):

- If the run reported `transitioned` or `noop_already_in_progress`, the live Jira status of `MM-667` MUST be `In Progress`.
- If the run reported a `stopped:*` outcome, the live Jira status of `MM-667` MUST equal `priorStatus` from Step 1 (zero partial mutation, SC-002).
- `MM-667`'s description, labels, assignee, comments, and other fields MUST be unchanged versus Step 1 (FR-008, SC-004).

### Step 5 — Verify isolation

Spot-check at least one neighboring Jira issue (e.g., `MM-666` or `MM-668`) to confirm it is unmodified. The run MUST NOT have updated any other issue (SC-004).
...
| Observed outcome | Live Jira status of MM-667 after the run | Acceptable? |
|---|---|---|

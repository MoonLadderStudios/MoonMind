...
- T003 can run after T001 starts while T002 reviews existing test fixtures.
- User Story test tasks that modify `frontend/src/entrypoints/task-create.test.tsx` should run serially or under one owner to avoid same-file conflicts.
- T030 can run in parallel with quickstart review T034 after implementation is complete.

## Parallel Example: User Story 1

```bash
Task: "Review UI action expectations in specs/166-jira-preset-reapply/contracts/create-page-jira-preset-reapply.yaml"
Task: "Review existing Create page Jira and preset regression fixtures in frontend/src/entrypoints/task-create.test.tsx"
```

## Parallel Example: User Story 3

```bash
Task: "Run ./node_modules/.bin/tsc --noEmit -p frontend/tsconfig.json"
Task: "Confirm quickstart validation steps still match specs/166-jira-preset-reapply/quickstart.md"
```
...
- Validation tasks are present for each user story and cross-cutting edge cases: T014, T019, T028, T029, T030, T031, and T032.

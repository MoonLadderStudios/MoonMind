...
- [ ] T001 Define `OAuthSession` SQLAlchemy columns (`terminal_session_id`, `terminal_bridge_id`, `connected_at`, `disconnected_at`) and remove legacy URLs in `src/moonmind/models/oauth.py` (DOC-REQ-003)
- [ ] T002 Generate Alembic migration for the column changes in `src/moonmind/migrations/versions/` (DOC-REQ-003 validation)

## Phase 2: TerminalPTYBridge (Foundational)

- [ ] T003 Implement `TerminalPTYBridge` container startup logic in `src/moonmind/agent_runtime/terminal_bridge.py` replacing the dead `oauth_launch` (DOC-REQ-001)
...
- [ ] T005 [US1] Replace Temporal state transitions removing `oauth_runner_ready` in favor of `bridge_ready`, listening to connect/disconnect signals in `src/moonmind/workflows/oauth_session.py` (DOC-REQ-002)
- [ ] T006 [P] [US1] Update finalization to issue `CreateManagedAgentProviderProfile` with `oauth_volume` source in `src/moonmind/workflows/oauth_session.py` (DOC-REQ-004)

...
- [ ] T004 Prepare or update shared test fixtures for Codex OAuth Provider Profiles, OAuth verification evidence, slot policy metadata, and secret-like nested values in `tests/unit/` and `tests/integration/temporal/test_oauth_session.py` without implementing production behavior (FR-001 through FR-009)
...
- [ ] T010 [P] Add failing unit tests for workflow-facing profile snapshot redaction and compact ref preservation in `tests/unit/auth/test_oauth_session_activities.py` or `tests/unit/schemas/test_agent_runtime_models.py` (FR-002, FR-006, FR-009, SC-002, DESIGN-REQ-010, DESIGN-REQ-020)
...
- [ ] T013 [P] Add failing integration or boundary test for non-Codex profile independence and no Claude/Gemini task-scoped parity requirement in `tests/integration/temporal/test_oauth_session.py` or `tests/unit/api_service/api/routers/test_provider_profiles.py` (FR-008, SC-004, acceptance scenario 5, DESIGN-REQ-002)

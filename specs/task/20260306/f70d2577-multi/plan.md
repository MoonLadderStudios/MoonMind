...
- Required unit/contract command: `./tools/test_unit.sh`.
- Expand contract and unit coverage for configure-monitoring, callback ingestion, poll updates, dedupe, cancellation, and Continue-As-New behavior.
- Add Temporal integration/failure-path coverage under `tests/integration/temporal/` for callback races, missed callbacks, polling fallback, and Jules-specific normalization.

...
| FR-009 | partial | Authorization is checked before verify; cancelled/expired fail by status; superseded is defined in `spec.md` as a derived same-actor/profile safety condition, not a new lifecycle status | Add safe failures for cancelled, expired, superseded, and unauthorized sessions with no Provider Profile mutation | API unit |

...
- [X] T005 [P] Implement artifact-backed callback/result/failure payload helpers, preview redaction, short-lived download grants, and retention-class assignment in `moonmind/workflows/temporal/artifacts.py` (DOC-REQ-003, DOC-REQ-006, DOC-REQ-012, DOC-REQ-013)
...
- [X] T008 Implement callback verification, request-size limits, callback rate limiting, and router registration plumbing in `api_service/api/routers/execution_integrations.py`, `api_service/api/routers/__init__.py`, and `api_service/main.py` (DOC-REQ-003, DOC-REQ-008, DOC-REQ-013)
...
- [X] T009 [P] [US1] Add contract tests for `POST /api/executions/{workflowId}/integration` and the internal poll-reconciliation surface `POST /api/executions/{workflowId}/integration/poll` in `tests/contract/test_temporal_execution_api.py` (DOC-REQ-001, DOC-REQ-007, DOC-REQ-010)
...
- [X] T011 [P] [US1] Add unit tests for Jules start/status/fetch-result idempotency, ambiguous `start` timeout handling, and normalized status mapping in `tests/unit/workflows/adapters/test_jules_client.py` (DOC-REQ-002, DOC-REQ-005, DOC-REQ-006, DOC-REQ-014, DOC-REQ-015)
...
- [X] T017 [P] [US2] Add router tests for callback signature/auth validation, request-size rejection, callback rate limiting, and malformed payload handling in `tests/unit/api/routers/test_execution_integrations.py` (DOC-REQ-008, DOC-REQ-013)
...
- [X] T026 [P] [US3] Add unit tests for provider failure summaries, cancel outcomes, redacted artifact previews, short-lived download grants, retention-class handling, and result-fetch/artifact-write retry behavior in `tests/unit/workflows/temporal/test_temporal_service.py` and `tests/unit/workflows/temporal/test_artifacts.py` (DOC-REQ-006, DOC-REQ-012, DOC-REQ-013, DOC-REQ-014)
...
- [X] T028 [P] [US3] Add Temporal integration failure-path coverage for missed callbacks, provider `429/5xx`, ambiguous `start` timeouts, cancellation, provider failures, and Jules terminal states in `tests/integration/temporal/test_integrations_monitoring.py` (DOC-REQ-011, DOC-REQ-012, DOC-REQ-014, DOC-REQ-015)
...
- [X] T034 [P] Run `./tools/test_unit.sh`, then run `.venv/bin/python -m pytest tests/contract/test_temporal_execution_api.py` and `.venv/bin/python -m pytest tests/integration/temporal/test_integrations_monitoring.py` (fallback to `python` or `python3` when `.venv/bin/python` is unavailable) to cover unit, contract, and Temporal integration validation (DOC-REQ-001, DOC-REQ-002, DOC-REQ-003, DOC-REQ-004, DOC-REQ-005, DOC-REQ-006, DOC-REQ-007, DOC-REQ-008, DOC-REQ-009, DOC-REQ-010, DOC-REQ-011, DOC-REQ-012, DOC-REQ-013, DOC-REQ-014, DOC-REQ-015)
- [X] T035 Run runtime scope gates with `SPECIFY_FEATURE=047-integrations-monitoring` set: `./.specify/scripts/bash/validate-implementation-scope.sh --check tasks --mode runtime` and `./.specify/scripts/bash/validate-implementation-scope.sh --check diff --base-ref origin/main --mode runtime` after implementation changes are present (DOC-REQ-016)
...
| DOC-REQ-014 | T011, T015, T029, T030 | T012, T020, T025, T026, T028, T034 |
...
- The poll-recording contract in `contracts/integrations-monitoring.openapi.yaml` is an internal reconciliation/test surface; durable polling remains workflow-owned via timers inside `MoonMind.Run`.

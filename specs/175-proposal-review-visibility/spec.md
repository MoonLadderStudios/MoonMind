# Feature Specification: Proposal Review Visibility

**Feature Branch**: `175-proposal-review-visibility`  
**Created**: 2026-04-14  
**Status**: Draft  
**Input**:
````markdown
...
## User Story - Proposal Review Visibility

**Summary**: As an operator monitoring a run and reviewing proposals, I want API and Mission Control surfaces to show proposal progress, outcomes, and execution context so I can decide what to promote.

**Goal**: Expose proposals as a first-class run state with consistent status mapping, counts, errors, proposal links, and compact context about runtime, repository, publish behavior, and skill-selection intent.

**Independent Test**: Drive execution and proposal records through API projection and Mission Control views, then verify status mappings, proposal links, count/error summaries, and context display without exercising generation internals.

**Acceptance Scenarios**:

1. **Scenario**: Given a workflow is in the proposals stage, when execution API and Mission Control status are viewed, then mm_state=proposals is visible and dashboard compatibility maps proposals to running.
2. **Scenario**: Given a run has completed proposal generation with partial errors, when the execution summary is viewed, then requested status, generated count, submitted count, and redacted errors are visible.
3. **Scenario**: Given proposals were created by a workflow, when an operator opens execution detail, then the UI links to proposals filtered by originSource=workflow and originId=<workflow_id>.
4. **Scenario**: Given a stored proposal carries explicit skill selectors, runtime, repository, and publish settings, when proposal detail or promotion UI is viewed, then a compact context summary shows those values without exposing skill bodies or runtime-local materialization state.
5. **Scenario**: Given proposal generation or submission skips malformed candidates, when the run remains otherwise successful, then UI and API surfaces make partial success visible without presenting skipped candidates as promotable work.

### Edge Cases

- A workflow has no submitted proposals but has redacted generation or validation errors.
- A proposal inherits deployment defaults and therefore omits explicit skill selectors.
- A proposal has origin metadata but no promoted execution linkage yet.
- An API consumer still expects legacy dashboard status names while the workflow state is proposals.
- A redacted error message would otherwise include provider-sensitive details.

## Assumptions

- UI wording and layout can be refined during planning, but the displayed context and redaction requirements are fixed by this story.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: Execution API and UI surfaces MUST expose proposals as a first-class workflow state where applicable.
- **FR-002**: Dashboard compatibility mapping MUST map scheduled to queued, waiting_on_dependencies to waiting, awaiting_slot to queued, proposals to running, and completed to completed.
- **FR-003**: Execution detail MUST expose proposal counts and redacted errors from finish summary data.
- **FR-004**: Execution detail MUST link to proposals filtered by originSource=workflow and originId=<workflow_id> for workflow-originated proposals.
- **FR-005**: Proposal review surfaces SHOULD show whether the proposal carries explicit skill selectors or inherits deployment defaults.
- **FR-006**: Proposal review surfaces MUST show compact runtime, repository, and publish context when available.
- **FR-007**: Proposal review surfaces MUST NOT expose raw credentials, full skill bodies, runtime-local materialization outputs, or unsafe provider details.
- **FR-008**: Partial proposal success MUST be visible without changing the parent run's successful outcome solely because best-effort proposal work had recoverable errors.
...
- **DESIGN-REQ-017 - Review and status visibility**: This story owns API and UI surfaces expose proposals state mappings, counts, errors, filtered proposal links, and compact skill/runtime/repository/publish context.
...
- **Proposal Stage Projection**: The API/UI representation of a workflow while generation or submission is in progress.
- **Proposal Review Context**: Compact operator-facing metadata summarizing runtime, repository, publish mode, and skill-selection intent.
- **Filtered Proposal Link**: A navigation target from execution detail to proposals matching workflow origin identity.
- **Redacted Proposal Error**: A safe diagnostic string suitable for operator display without secrets or raw provider-sensitive content.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: API and UI tests prove proposals state appears consistently and maps to running for dashboard compatibility.
- **SC-002**: Execution detail tests prove filtered proposal links use workflow origin identity.
- **SC-003**: Proposal review tests prove explicit skill selectors versus inherited defaults are distinguishable without exposing skill bodies.
- **SC-004**: Failure display tests prove partial proposal errors are redacted and do not hide submitted proposal counts.

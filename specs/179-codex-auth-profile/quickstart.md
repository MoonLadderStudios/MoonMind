...
```bash
MOONMIND_FORCE_LOCAL_TESTS=1 ./tools/test_unit.sh tests/unit/api_service/api/routers/test_provider_profiles.py tests/unit/auth/test_oauth_session_activities.py tests/unit/schemas/test_agent_runtime_models.py
```

Expected result: focused tests for `STORY-001` fail before implementation changes and pass after the story is complete.
...
4. After implementation, run `/moonspec-verify` equivalent against `specs/179-codex-auth-profile/spec.md`.

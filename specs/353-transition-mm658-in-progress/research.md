# Research and Repo Gap Analysis: Transition MM-658 to "In Progress"
...
Decision: Use the already-registered tools `jira.get_issue`, `jira.get_transitions`, and `jira.transition_issue` exclusively. No new tool, no new adapter, and no raw HTTP from the agent runtime.
Evidence: `moonmind/mcp/jira_tool_registry.py` (registers `jira.get_issue`, `jira.get_transitions`, `jira.transition_issue` through `JiraToolService`); `moonmind/integrations/jira/tool.py` (`get_transitions` / `transition_issue` implementations); `moonmind/integrations/jira/models.py` (`GetTransitionsRequest`, `TransitionIssueRequest`). Existing unit tests at `tests/unit/integrations/test_jira_tool_service.py` confirm preflight transition lookup, expanded transition-field metadata, and credential exclusion from responses.
...
Decision: Perform case-insensitive trimmed name matching of `transition.to.name == "In Progress"` inline during the agent run. Treat zero matches as `no_matching_transition`, more than one match as `ambiguous_transition`, and exactly one match as the `selected` transition. Detect required fields with `expand_fields=true` on the discovery call and raise `missing_required_fields` (no value guessing). Do not reuse `select_done_transition` from `moonmind/workflows/temporal/post_merge_jira_completion.py` — that helper biases toward done-category semantics and starts with a done-category no-op short-circuit, which would falsely no-op when MM-658 happens to already be in a done-category status (irrelevant to "In Progress" target semantics).
Evidence: `post_merge_jira_completion.py` (done-category no-op short-circuit; `transitionName` matching is case-insensitive, but the surrounding `_is_done_category` short-circuit makes the helper unsuitable as-is; `_missing_required_fields` helper exists).
...
- Generalize `select_done_transition` to accept a target-status-name parameter and decouple the no-op check from done-category — rejected as **out of scope** for a one-shot run targeting MM-658. Defer to a follow-up story when this pattern needs to be repeated for arbitrary issues and target statuses.
...
Test implications: Per-run quickstart assertions cover the named outcomes (transitioned / no-op / no_matching_transition / ambiguous_transition / missing_required_fields). No new unit test required for this story.
...
5. `stopped:issue_not_found` — `jira.get_issue` for `MM-658` raises a not-found error.
...
Evidence: Spec FR-007, FR-009, FR-010, edge-cases section. `JiraToolError` raised in `tool.py` for unavailable transitions; existing redaction in `post_merge_jira_completion._scrub_exception`.
...
Evidence: `moonmind/workflows/temporal/post_merge_jira_completion.py` (`_scrub_exception`); `moonmind/integrations/jira/tool.py` raises `JiraToolError` with sanitized messages; `tests/unit/integrations/test_jira_tool_service.py` asserts no-credential responses; `tests/unit/workflows/temporal/test_post_merge_jira_completion.py` covers credential scrubbing on read failures.
...
|---|---|---|
| FR-001 | partial | Run-time discipline: pass exactly `MM-658` to every tool call. |

# Data Model: Transition MM-658 to "In Progress"
...
| Field | Type | Notes |
|---|---|---|
| `key` | string | Always equals `"MM-658"` for this story; never substituted. |
...
| `outcome` | enum | One of the eleven named outcomes in `research.md` ("Error Surface and Named Outcomes"). |
...
- **Issue ↔ Transition** is a one-to-many relation per call to `jira.get_transitions`; this story consumes only the transitions for `MM-658` returned in a single discovery response.
...
- **Mutation guarantee**: `transition.*` set ↔ exactly one `jira.transition_issue` call was issued, with `issue_key="MM-658"`, `transition_id=<the matched id>`, `fields={}`, `update={}`.
- **Single-issue invariant**: Across the entire run, `issueKey` in every Jira tool call equals `"MM-658"`. No other key is read or modified.
...
└── ok: priorStatus
    ...
    └── priorStatus does not match
        get_transitions(MM-658, expand_fields=true)
        ...
        └── ok: transitions[]
            ...
            └── len(matches) == 1
                ...
                └── missing == []
                    transition_issue(MM-658, matches[0].id, fields={}, update={})

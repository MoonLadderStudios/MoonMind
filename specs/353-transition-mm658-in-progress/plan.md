# Implementation Plan: Transition Jira Issue MM-658 to "In Progress"

**Branch**: `change-jira-issue-mm-658-to-status-in-pr-70727e51` (spec dir: `353-transition-mm658-in-progress`) | **Date**: 2026-05-15 | **Spec**: [spec.md](spec.md)
**Input**: Single-story feature specification from `/specs/353-transition-mm658-in-progress/spec.md`

## Summary

The story moves a single Jira issue (`MM-658`) into workflow status `In Progress`, exactly once, through MoonMind's trusted Jira tool surface. The technical approach is to perform the transition during this agent run by calling the already-registered trusted tools (`jira.get_issue`, `jira.get_transitions`, `jira.transition_issue`), with the agent applying case-insensitive trimmed name matching against the literal target `In Progress`, executing the transition only when exactly one match exists, and re-reading MM-658 afterward to verify the final status. Test strategy is verification-first: existing unit tests already cover the trusted tool surface and required-field handling; the per-run independent test is to re-fetch MM-658 from Jira after the run and confirm it reads `In Progress` (or that the workflow recorded a no-op). Repo gap analysis: every supporting tool, request model, and credential-redaction layer already exists in `moonmind/integrations/jira/` and `moonmind/mcp/jira_tool_registry.py`; the per-issue orchestration is performed inline by the agent during this run and produces a structured run report — no new persistent Python orchestration is required for this single-issue, one-shot story.

## Requirement Status

| ID | Status | Evidence | Planned Work | Required Tests |
|---|---|---|---|---|
| FR-001 (literal `MM-658` target) | partial | `moonmind/integrations/jira/models.py` (`TransitionIssueRequest.issueKey`) accepts any key; agent must pass exactly `MM-658` | Run-time discipline: pass `issueKey="MM-658"` to every Jira tool call in this story; no inferred substitutions | Run-report assertion (independent test) |
| FR-002 (discover available transitions before mutation) | implemented_unverified | `moonmind/integrations/jira/tool.py` (`get_transitions`); `moonmind/mcp/jira_tool_registry.py` registers `jira.get_transitions` | None — call existing tool with `issueKey="MM-658"` and `expand_fields=true` so required-field metadata is present | Verification via run report capturing transition list |
| FR-003 (case-insensitive trimmed match for `In Progress`, unambiguous) | partial | Existing `select_done_transition` in `moonmind/workflows/temporal/post_merge_jira_completion.py` supports `transitionName` matching but is post-merge-specific (done-category bias). | Inline matching during the run: lowercase-trim each candidate transition's `to.name`, match against `"in progress"`, fail with named ambiguity error when match count != 1 | Verification via run report asserting unique match (or named error) |
| FR-004 (only via trusted Jira tools, no raw HTTP) | implemented_verified | `moonmind/mcp/jira_tool_registry.py` registers transition tools through `JiraToolService`; `tool.py` ensures `_ensure_enabled`/`_ensure_action_allowed`/`_ensure_project_allowed` gates; agent runtime has no Jira credentials | None | Final verification only |
| FR-005 (re-read and report verified final status) | implemented_unverified | `jira.get_issue` registered in `moonmind/mcp/jira_tool_registry.py` | Inline post-transition `jira.get_issue(MM-658)` call; surface the observed status in the run report; flag if final status != `In Progress` | Run-report assertion |
| FR-006 (already `In Progress` → no-op, no transition invoked) | partial | `select_done_transition` no-op handling exists for done category only; not applicable to "In Progress" target | Inline check: if pre-transition status name (case-insensitive trimmed) equals `In Progress`, skip transition call and emit `noop_already_in_progress` outcome | Run-report assertion |
| FR-007 (named errors: no match / ambiguous / not found / missing required fields / tool unavailable / auth or permission / transient / validation / final status mismatch) | partial | Existing tool errors for unknown issue and required-field surfacing exist (`_fetch_transitions` `expand_fields` enables `transitions.fields` metadata; `JiraToolError` raised on unavailable transition); no current ambiguity error specific to "In Progress"; redaction pipeline already covers auth/permission, validation, and transient surfaces | Inline error mapping during the run produces explicit named outcomes: `no_matching_transition`, `ambiguous_transition`, `issue_not_found`, `missing_required_fields`, `tool_unavailable`, `auth_or_permission`, `transient_failure`, `validation_failure`, and `final_status_mismatch` — no mutation in any error case | Run-report assertion for each error path that applies |
| FR-008 (no other field updates, no other issues) | implemented_verified | `tool.py` `transition_issue` only POSTs to `/issue/<key>/transitions` with the chosen `transitionId`; passing `fields={}` and `update={}` keeps the request scoped | None — never call `jira.edit_issue` in this story; only invoke transition + verification reads against `MM-658` | Final verification via Jira issue change history |
...
| DESIGN-REQ-001 (only `MM-658`) | partial | See FR-001 + FR-008 | None beyond FR-001/FR-008 work | Run-report assertion |
...
**Language/Version**: Python 3.12 (existing MoonMind runtime); no new module files are added for this one-shot run.
...
**Constraints**: No raw Jira HTTP from the agent; no new credentials in the runtime; no mutation of any issue other than `MM-658`; no field updates beyond the workflow status; zero secret leakage.
**Scale/Scope**: Exactly one Jira issue (`MM-658`), one transition, one verification fetch, one structured run report.
...
| XII. Canonical Docs vs. Feature Artifacts | PASS | Plan/research/contracts/quickstart all live under `specs/353-transition-mm658-in-progress/`. |
...
```text
specs/353-transition-mm658-in-progress/
...
├── contracts/
│   └── transition-mm658.md  # Tool-call contract for the run
└── checklists/

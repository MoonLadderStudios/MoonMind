...
| `tasks.md` final verification wording drifted from the requested MoonSpec command name. | Low | Updated the final verification command and T020 from `/speckit.verify` to `/moonspec-verify`; no downstream artifact regeneration was required because this was command wording only. |
...
- `tasks.md` includes red-first unit/workflow tests, integration verification, implementation tasks, story validation, and final `/moonspec-verify` work for the single MM-351 story.

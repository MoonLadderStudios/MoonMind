...
**Independent Test**: While a run executes, poll `GET /api/workflows/runs/{id}` and `/runs/{id}/tasks` to confirm timestamps, payloads, and artifacts update; induce a failure to verify actionable messages.

### Tests
- [x] T017 [P] [US2] Extend `tests/unit/workflows/test_tasks.py` to confirm each Celery state change writes `WorkflowTaskState` with timestamps + artifact references.
...
**Independent Test**: Force a failure at publish, POST `/api/workflows/runs/{id}/retry`, and confirm the chain resumes using stored diff context; retry again without fixing credentials to ensure fast failure with guidance.
...
- [x] T025 [P] [US3] Add contract tests for `POST /api/workflows/runs/{id}/retry` in `tests/contract/test_workflow_api.py`, covering successful resumes and credential errors.
...
- **US1**: POST `/api/workflows/runs` with a staged Spec repo; verify Celery completes discovery→publish, emits Codex task IDs, and stores JSONL logs + PR URL.
- **US2**: Trigger a run, poll `GET /api/workflows/runs` and `/runs/{id}/tasks` to confirm timestamps/messages update; fetch `/runs/{id}/artifacts` to download logs.
- **US3**: Force a publish failure, call `/api/workflows/runs/{id}/retry`, ensure the chain resumes from publish using stored artifacts; retry again without fixing credentials to confirm fast failure + guidance.

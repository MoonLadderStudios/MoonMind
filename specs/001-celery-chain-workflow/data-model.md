...
- **Key Attributes**:
  ...
  - `selected_skill` (derived from payload field `selectedTool`) – selected skill identifier used for this stage attempt (default `speckit`).
...
- **Validation Rules**:
  ...
  - When `state` is terminal, payload should include `selectedTool` and `executionPath` to support skills-path observability.
...
- **Validation Rules**:
  ...
  - `executionPath=direct_only` requires `usedTools=false`.

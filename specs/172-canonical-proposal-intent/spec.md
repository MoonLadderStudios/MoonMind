# Feature Specification: Canonical Proposal Intent Capture

**Feature Branch**: `172-canonical-proposal-intent`  
**Created**: 2026-04-14  
**Status**: Draft  
**Input**:
````markdown
...
## User Story - Canonical Proposal Intent Capture

**Summary**: As an operator submitting proposal-capable work, I want every task creation path to persist proposal and skill intent in the canonical task payload so proposal behavior is durable and replayable.

**Goal**: Normalize and preserve proposal opt-in, proposal policy, runtime, repository, task skills, and step skills before MoonMind.Run starts, including managed-session-originated task creation.

**Independent Test**: Submit proposal-capable work through ordinary API creation, proposal promotion, schedules, and Codex managed-session internal task creation, then inspect the created run's initial parameters and lifecycle gating without requiring actual proposal promotion.

**Acceptance Scenarios**:

1. **Scenario**: Given global proposal generation is enabled and a task is submitted with task.proposeTasks=true and task.proposalPolicy, when MoonMind.Run starts, then initialParameters.task contains the nested proposal fields exactly as the canonical task payload supplied them.
2. **Scenario**: Given a Codex managed session creates an additional MoonMind task through the internal API path, when the run is created, then proposal opt-in and routing overrides are written to initialParameters.task.* and not only to session metadata, turn metadata, environment, or root-level fields.
3. **Scenario**: Given a task includes task.skills or step.skills, when the run is created, then those selectors remain in the durable task payload and are not replaced with runtime-local skill materialization state.
4. **Scenario**: Given global proposal generation is disabled or task.proposeTasks is false, when execution completes, then the workflow does not enter the proposals stage even if non-canonical session-local flags are present.
5. **Scenario**: Given a proposal-capable run enters proposals, when API or workflow state is inspected, then the state vocabulary uses the canonical proposals lifecycle value.

### Edge Cases

- Root-level or older proposal-policy fields exist on replayed payloads while new submissions must still write nested task.* fields.
- A task omits proposalPolicy but enables proposals, so global defaults remain resolvable later.
- A session attempts to carry proposal intent only in adapter-local metadata.
- The repository target is missing or malformed in a task that otherwise enables proposals.

## Assumptions

- Compatibility reads for already-running workflows may remain only where required for replay safety; this story controls the write contract for new work.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: All new task submission paths MUST normalize proposal intent into the canonical nested task payload before MoonMind.Run starts.
- **FR-002**: The system MUST preserve task.proposeTasks, task.proposalPolicy, task.skills, and step.skills in initialParameters.task when supplied.
- **FR-003**: Codex managed-session-created tasks MUST use the same canonical task payload contract as ordinary API task submissions.
- **FR-004**: Non-canonical locations such as root-level initialParameters.proposeTasks, turn metadata, session binding metadata, container environment, or adapter-local state MUST NOT be the durable write contract for new work.
- **FR-005**: The proposal stage gate MUST depend on global proposal enablement and initialParameters.task.proposeTasks, not on session-local flags alone.
- **FR-006**: The system MUST use the canonical lifecycle vocabulary, including proposals, across workflow state and execution-facing contracts touched by this story.
- **FR-007**: The repository target used for deduplication and future execution MUST remain taskCreateRequest.payload.repository or the equivalent canonical task payload repository before proposal storage exists.
...
- **DESIGN-REQ-005 - Durable proposal intent at submit time**: This story owns task.proposeTasks, task.proposalPolicy, task.skills, and step.skills are preserved in initialParameters.task.
- **DESIGN-REQ-006 - Canonical submission normalization**: This story owns all task creation paths, including Codex managed sessions, normalize proposal intent into nested task.* fields instead of non-canonical locations.
- **DESIGN-REQ-007 - Proposal lifecycle state**: This story owns the proposals state is first-class and entered only when global settings and task-level opt-in both allow generation.
...
- **Canonical Task Payload**: The nested task request persisted before run start, including repository, runtime, proposal flags, proposal policy, task skills, and step skills.
- **Proposal Intent**: The durable opt-in and routing data that determines whether a completed run may generate proposals.
- **Managed Session Task Creation**: A Codex session-originated request that creates another MoonMind task through the internal task creation surface.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: Every covered task creation path persists proposal intent in initialParameters.task in automated boundary tests.
- **SC-002**: A Codex managed-session-created task can be verified to enter or skip the proposals stage based only on canonical nested task fields and global settings.
- **SC-003**: No new submission path depends on root-level proposal fields or adapter-local flags as the durable contract.
- **SC-004**: Skill selectors supplied in task or step payloads survive run creation without embedding skill bodies or materialized directories.

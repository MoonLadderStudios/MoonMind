# Feature Specification: Safe Proposal Generation and Submission

**Feature Branch**: `173-proposal-submission-policy`  
**Created**: 2026-04-14  
**Status**: Draft  
**Input**:
````markdown
...
## User Story - Safe Proposal Generation and Submission

**Summary**: As an operator reviewing follow-up work, I want completed runs to generate and store policy-compliant proposal records without side effects so useful next steps are captured safely.

**Goal**: Generate candidates from run evidence, validate canonical payloads and skill selectors, resolve routing policy, store open proposals, and report partial outcomes without compromising the parent run.

**Independent Test**: Run proposal generation and submission activities against representative run artifacts, agent results, policies, and malformed candidates, then verify stored proposals, skipped candidates, routing, and finish summary counts without invoking promotion.

**Acceptance Scenarios**:

1. **Scenario**: Given a completed proposal-enabled run with artifacts and normalized agent results, when proposal generation runs, then candidates are derived from run evidence without commits, pushes, task creation, or other external side effects.
2. **Scenario**: Given generated candidates include canonical taskCreateRequest payloads, when proposal submission runs, then valid candidates are stored as open proposals and no execution is created.
3. **Scenario**: Given a candidate omits task.runtime.mode and task.proposalPolicy.defaultRuntime is configured, when submission resolves policy, then the default runtime is stamped only on that candidate and explicit runtime values are preserved.
4. **Scenario**: Given project and MoonMind targets have capacity, severity, and tag rules, when submission resolves routing, then project proposals keep the triggering repository and MoonMind run-quality proposals are rewritten only when they satisfy the configured gates.
5. **Scenario**: Given malformed candidates, unsafe log dumps, secrets, incompatible skill selectors, or mutable skill directory state appear in generated output, when submission validates candidates, then those candidates are skipped with visible redacted validation errors.
6. **Scenario**: Given some candidates fail validation after others succeed, when finalization records the outcome, then generated count, submitted count, and redacted errors appear in the typed result and reports/run_summary.json.

### Edge Cases

- A generator sees large skill context and must reference artifact-backed metadata instead of embedding large content.
- A candidate contains tool.type=agent_runtime instead of the canonical skill tool type.
- MoonMind target routing is requested for a generic project cleanup proposal that lacks run-quality category or approved tags.
- Submission is retried after a transient storage failure and must not duplicate proposals.
- Global proposal generation is enabled but all candidates are invalid or below the configured severity floor.

## Assumptions

- Candidate generation may be deterministic or model-assisted, but the storage contract and safety gates are deterministic and testable.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: Proposal generation MUST run in Temporal activities, not workflow code.
- **FR-002**: Generators MUST treat run inputs and logs as untrusted and MUST redact or exclude secrets and unsafe command output.
- **FR-003**: Generators MUST avoid side effects such as commits, pushes, task creation, or proposal storage.
- **FR-004**: Submission MUST validate taskCreateRequest payloads against the canonical task contract before storage.
- **FR-005**: Submission MUST validate task.skills and step.skills when present and MUST NOT store full skill bodies, mutable .agents/skills state, or ephemeral runtime materialization outputs.
- **FR-006**: Policy resolution MUST merge global defaults with task.proposalPolicy, preserve explicit candidate values, enforce target caps, enforce MoonMind severity and tag gates, and stamp defaultRuntime only when runtime is omitted.
- **FR-007**: Project-targeted proposals MUST keep the triggering repository as taskCreateRequest.payload.repository.
- **FR-008**: MoonMind-targeted proposals MUST be limited to run-quality improvements and rewritten only to the configured MoonMind repository when category, severity, and tag gates pass.
- **FR-009**: Submission MUST create proposal records only and MUST NOT promote proposals.
- **FR-010**: Proposal generation and submission failures MUST be best-effort, bounded, idempotent where retried, and visible through generated/submitted counts and redacted errors.
- **FR-011**: Generation MUST run on the LLM-capable activity fleet and submission/storage MUST run on a control-plane or integrations-capable boundary.

### Source Design Coverage

- **DESIGN-REQ-001 - Reviewable follow-up work**: This story owns proposals are durable review objects for useful follow-up work and must not automatically start new executions.
- **DESIGN-REQ-002 - Canonical task payload**: This story owns every proposal stores a canonical taskCreateRequest compatible with Temporal-backed execution submission.
- **DESIGN-REQ-003 - Human-gated execution**: This story owns proposal creation is distinct from execution; only explicit human promotion creates running work.
- **DESIGN-REQ-008 - Activity-based generation**: This story owns generators run in activities, analyze run artifacts/results/skill metadata, treat inputs as untrusted, avoid side effects, and use artifact refs for large context.
- **DESIGN-REQ-009 - Activity-based submission**: This story owns submission validates candidates, resolves policy, normalizes origin, enforces routing, preserves skill intent, and stores proposals only.
- **DESIGN-REQ-010 - Finish summary outcomes**: This story owns finish summaries and reports/run_summary.json record requested, generated, submitted, and redacted error counts.
- **DESIGN-REQ-011 - Proposal policy resolution**: This story owns global defaults and task proposalPolicy merge at submission, enforce target caps, severity gates, routing, and defaultRuntime only when needed.
- **DESIGN-REQ-012 - Target routing**: This story owns project proposals keep the triggering repository while MoonMind run-quality proposals require repository rewrite, category normalization, severity, and tag gates.
- **DESIGN-REQ-013 - Canonical payload validation**: This story owns stored candidates use supported runtime values, skill tool selectors, canonical task/step skills, and exclude secrets, raw logs, mutable skill directories, and ephemeral materialization state.
- **DESIGN-REQ-018 - Best-effort failure handling**: This story owns generation/submission errors must not corrupt successful parent runs; malformed candidates are skipped, retries are bounded/idempotent, and partial success is visible.
- **DESIGN-REQ-019 - Worker-boundary placement**: This story owns generation belongs on the LLM-capable fleet and submission/storage on control-plane or integrations-capable activity boundaries.
- **DESIGN-REQ-020 - Remaining implementation gaps**: This story owns the open work includes Codex session normalization verification, UI/state alignment, promotion linkage persistence, origin naming, and skill selector preservation end to end.

### Key Entities *(include if feature involves data)*

- **Proposal Candidate**: A generated follow-up item containing title, summary, category, tags, signal, and canonical taskCreateRequest.
- **Proposal Policy**: Global defaults plus per-task overrides for targets, caps, severity floors, MoonMind routing, and default runtime.
- **Stored Proposal**: A review object persisted through the proposal API that remains open until human promotion or dismissal.
- **Run Summary Proposal Outcome**: Typed and artifact-backed counts and redacted errors describing proposal generation and submission results.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: Automated activity-boundary tests cover valid storage, malformed candidate skipping, policy routing, defaultRuntime stamping, and redacted error reporting.
- **SC-002**: Proposal submission never creates a MoonMind.Run in this story's tests.
- **SC-003**: Retry tests prove duplicate proposal records are not created for the same submission attempt.
- **SC-004**: Generated proposal payloads contain no raw credentials, unsafe log dumps, mutable skill folders, or runtime-local materialization artifacts.

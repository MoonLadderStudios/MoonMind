...
- Agentkit capability verified on worker startup
...
- Agentkit CLI checks pass on both workers.
...
Legacy Agentkit phase records default to `selected_skill=agentkit` and `execution_path=skill` when explicit metadata is absent.

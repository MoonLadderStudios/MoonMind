...
- **Decision**: Keep existing persisted `agentkit_*` phase values and add normalized skills metadata extraction in runtime/API layers.
...
- **Decision**: Expose `selected_skill`, `execution_path`, `used_skills`, `used_fallback`, and `shadow_mode_requested` in `AutomationPhaseState`.
...
- **Decision**: When legacy `agentkit_*` phases have no explicit metadata, default to `selectedSkill=agentkit` and `executionPath=skill`.
...
- **Decision**: Extend contract phase enum to include `agentkit_analyze` and `agentkit_implement` while preserving existing values.

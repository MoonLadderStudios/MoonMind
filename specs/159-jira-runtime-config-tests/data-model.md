# Data Model: Jira Runtime Config Tests
...
- `defaultProjectKey`: string. Defaults to empty string. Normalized to the accepted Jira project-key format when provided.
...
- Non-empty `defaultProjectKey` must be normalized before it is surfaced to the browser.

## Jira Browser Source Contract
...
- `boards`: board list endpoint template scoped by project.

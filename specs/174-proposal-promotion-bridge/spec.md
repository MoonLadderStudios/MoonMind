# Feature Specification: Proposal Promotion Bridge

**Feature Branch**: `174-proposal-promotion-bridge`  
**Created**: 2026-04-14  
**Status**: Draft  
**Input**:
````markdown
...
## User Story - Proposal Promotion Bridge

**Summary**: As an operator approving a proposal, I want promotion to validate the stored task intent and create a new MoonMind.Run so reviewable follow-up work starts only after explicit approval.

**Goal**: Promote open proposals by merging overrides, preserving explicit skill intent, validating runtime and payload compatibility, creating a new Temporal-backed execution, and persisting promotion linkage.

**Independent Test**: Create stored proposal records with and without explicit skill selectors, promote them with runtime and task overrides, then verify new execution metadata, proposal linkage, validation failures, and origin identity without requiring proposal generation.

**Acceptance Scenarios**:

1. **Scenario**: Given an open stored proposal, when an operator promotes it without overrides, then the stored taskCreateRequest is validated and submitted through the same Temporal-backed create path used by /api/executions.
2. **Scenario**: Given an operator supplies taskCreateRequestOverride or runtimeMode, when promotion runs, then overrides are merged into the promoted execution request without mutating the stored proposal payload except for promotion linkage/status fields.
3. **Scenario**: Given the stored proposal carries explicit non-default task.skills or step.skills, when promotion changes runtime mode, then skill selectors remain present unless the operator intentionally overrides them.
4. **Scenario**: Given a selected runtime is disabled or incompatible with preserved skill selectors, when promotion validates the merged payload, then no workflow is created and the operator receives a validation error.
5. **Scenario**: Given promotion succeeds, when the API response is returned and the proposal is reloaded, then both the updated proposal and created execution metadata are available and the promoted workflow or execution identifier is persisted.
6. **Scenario**: Given a workflow-originated proposal, when its origin is stored and surfaced, then origin.source is workflow, origin.id is workflow_id, and metadata keys use workflow_id, temporal_run_id, trigger_repo, starting_branch, and working_branch.

### Edge Cases

- A proposal is no longer open when promotion is requested.
- An override attempts to remove explicit skill selectors accidentally.
- A proposal was produced before origin naming was standardized and still needs an explicit cutover or migration decision.
- Temporal execution creation succeeds but linkage persistence fails and must be handled deterministically.
- A task-facing runtime value is syntactically valid but disabled by backend runtime configuration.

## Assumptions

- The exact storage field name for promoted linkage may be chosen during planning, but it must be durable and visible through proposal retrieval.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: Promotion MUST load the stored proposal and verify it is open before creating execution work.
- **FR-002**: Promotion MUST merge taskCreateRequestOverride into the stored taskCreateRequest and construct runtimeMode shortcuts as explicit overrides.
- **FR-003**: The merged payload MUST validate against the canonical task contract, including task.skills and step.skills validation, before any workflow is created.
- **FR-004**: Promotion MUST preserve explicit skill-selection intent unless the operator intentionally overrides it.
- **FR-005**: Changing runtime mode MUST NOT automatically erase or rewrite agent skill selectors.
- **FR-006**: Disabled runtimes or runtime/skill incompatibilities MUST fail validation before workflow creation.
- **FR-007**: Promotion MUST create a new MoonMind.Run through TemporalExecutionService.create_execution or the same Temporal-backed create path used by /api/executions.
- **FR-008**: Successful promotion MUST store the promoted workflow or execution identifier on the proposal record.
- **FR-009**: The promote API response MUST include the updated proposal record and the created execution metadata.
- **FR-010**: Workflow-originated proposal metadata MUST use origin.source=workflow, origin.id=workflow_id, and snake_case metadata keys consistently.
...
- **DESIGN-REQ-004 - Repository-aware identity**: This story owns taskCreateRequest.payload.repository is the canonical repository target for deduplication and future execution.
...
- **DESIGN-REQ-014 - Workflow origin metadata**: This story owns workflow-originated proposals use origin.source=workflow, origin.id=workflow_id, snake_case metadata, and workflow_id as durable identity.
- **DESIGN-REQ-015 - Promotion bridge**: This story owns promotion loads an open proposal, merges overrides, validates the payload, creates a new MoonMind.Run through TemporalExecutionService, persists linkage, and returns proposal plus execution metadata.
- **DESIGN-REQ-016 - Skill and runtime preservation**: This story owns promotion preserves explicit agent skill intent, treats runtime override separately, fails incompatible or disabled runtime/skill selections, and uses the backend runtime list as source of truth.
...
- **Promotion Request**: Operator approval input containing optional taskCreateRequestOverride or runtimeMode shortcut.
- **Promoted Execution**: The new MoonMind.Run created from the validated proposal payload.
- **Promotion Linkage**: Stored proposal metadata identifying the execution or workflow created by successful promotion.
- **Proposal Origin**: Workflow source identity and diagnostic metadata for tracing the proposal back to its producing run.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: Promotion tests prove a new MoonMind.Run is created only after open-state and payload validation pass.
- **SC-002**: Runtime override tests prove explicit skill selectors are preserved by default and incompatible combinations fail before workflow creation.
- **SC-003**: Storage tests prove successful promotion persists a promoted execution/workflow identifier.
- **SC-004**: API contract tests prove promote responses include both updated proposal and created execution metadata.

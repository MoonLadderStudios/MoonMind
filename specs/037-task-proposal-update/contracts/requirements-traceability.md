...
| DOC-REQ-013 | FR-013 | `moonmind/workflows/task_proposals/service.py`, `moonmind/workflows/task_proposals/repositories.py`, `moonmind/workflows/task_proposals/task_contract.py` | Unit tests cover policy fallback for missing `proposalPolicy`, enforce `project=3`/`moonmind=2` defaults, validate severity vocabulary plus `high` floor, and verify no-op fallback when override values are missing or invalid. |

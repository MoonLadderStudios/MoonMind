...
| FR-008 | implemented_unverified | artifact IDs are immutable and later reports are queried via latest-link behavior, but MM-493-specific coexistence evidence is not yet isolated under this feature directory | add verification-first traceability and implementation contingency only if tests expose drift | unit + contract |

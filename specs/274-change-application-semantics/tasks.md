...
- [X] T004 Add reusable test registry entries for immediate, next-task, worker-reload, process-restart, and manual-operation apply modes in `tests/unit/services/test_settings_catalog.py` (FR-003, FR-007, DESIGN-REQ-019)
- [X] T005 Add reusable API test fixtures for settings actor/workspace identity and persisted audit reads in `tests/unit/api_service/api/routers/test_settings_api.py` (FR-004, SC-002)
...
- [X] T007 Review `frontend/src/generated/openapi.ts` update workflow and record whether generated OpenAPI refresh is needed after backend schema changes in `specs/274-change-application-semantics/quickstart.md` (contracts/settings-application-semantics.md)
...
- [X] T011 [P] Add failing unit tests for sanitized restored-reference diagnostics for missing SecretRef, provider profile, and supported OAuth/reference gaps in `tests/unit/services/test_settings_catalog.py` (FR-009, FR-010, FR-011, SC-005, DESIGN-REQ-025)
- [X] T012 [P] Add failing unit tests that late validation diagnostics remain visible after persistence for missing or disabled references in `tests/unit/services/test_settings_catalog.py` (FR-002, FR-012, SC-006, DESIGN-REQ-016)
...
- [X] T024 Extend restored-reference diagnostics for supported missing SecretRef, provider profile, OAuth/reference gaps without exposing plaintext in `api_service/services/settings_catalog.py` (FR-009, FR-010, FR-011, DESIGN-REQ-025)
...
- [X] T027 Update `frontend/src/components/settings/GeneratedSettingsSection.tsx` broken-reference diagnostic presentation to keep restored references clear and secret-safe (FR-010, FR-011, SC-005)
- [X] T028 Refresh `frontend/src/generated/openapi.ts` if backend schema changes require it, or document why generated OpenAPI is unaffected in `specs/274-change-application-semantics/quickstart.md` (contracts/settings-application-semantics.md)
...
- [X] T031 [P] Review `specs/274-change-application-semantics/spec.md`, `plan.md`, `tasks.md`, and `quickstart.md` for `MM-544` traceability and update only if implementation evidence changed (FR-013, SC-007)
- [X] T032 [P] Add edge-case test coverage for inconsistent descriptor apply metadata and missing restored references in `tests/unit/services/test_settings_catalog.py` if not fully covered by T008-T012 (Edge Cases, DESIGN-REQ-016, DESIGN-REQ-025)
- [X] T033 [P] Add UI edge-case coverage for long activation guidance and diagnostic text fitting without overlap in `frontend/src/components/settings/GeneratedSettingsSection.test.tsx` (SC-004, UI contract)
...
- [X] T036 Run `/moonspec-verify` for `specs/274-change-application-semantics/spec.md` and address any `ADDITIONAL_WORK_NEEDED` findings before final handoff
...
- Verification evidence on 2026-04-28:
  ...
  - Full unit verification: `./tools/test_unit.sh` passed with 4173 Python tests, 16 subtests, 1 xpass, and 460 frontend tests.

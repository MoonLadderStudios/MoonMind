...
| DOC-REQ-007 | `docs/LiveTaskHandoff.md` sections "Worker Lifecycle" and "Session Bootstrap" | Worker must report per-run live-session state (including ready/error and endpoints when a provider is configured). |
...
| DOC-REQ-012 | `docs/LiveTaskHandoff.md` section "Configuration" | Runtime/config must support documented live-session environment variables (provider, TTL, RW grant TTL, web toggle, concurrency limit). |
| DOC-REQ-013 | `docs/LiveTaskHandoff.md` sections "Failure Modes" and "Security Model" | If live-session provisioning fails, run must continue headless while live-session state is marked `ERROR` for diagnostics. |
...
| DOC-REQ-015 | `docs/LiveTaskHandoff.md` section "Worker Lifecycle > Image Requirements" | Runtime image must include baseline CLI networking packages (`openssh-client`, `ca-certificates`) and any packages required by the configured live-session provider. |
...
1. **Given** a running task run with live-session enablement requested, **When** the worker provisions session metadata successfully, **Then** the session status becomes `ready` and RO attach details are visible when applicable.
...
**Independent Test**: Run with missing or failing live-session transport and verify task continues headless while live session transitions to error/revoked/ended appropriately.

**Acceptance Scenarios**:

1. **Given** live-session startup cannot provision the configured provider, **When** prepare stage runs, **Then** live session is marked `error` and task execution continues.
...
- **FR-004**: Worker MUST provision or report per-run live-session state and report `starting`, `ready`, and `error` transitions with attach metadata when a provider is configured. (DOC-REQ-001, DOC-REQ-007)
...
- **FR-011**: Runtime/config surfaces MUST support documented live-session environment settings for enablement, provider, TTLs, web exposure, and concurrency controls. (DOC-REQ-012)
...
- **SC-004**: When live-session bootstrap fails in controlled test conditions, task execution still reaches terminal run status while live-session status is reported as `error`.

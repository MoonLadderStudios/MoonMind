...
### User Story 1 - Monitor Active Work Across Queue and Orchestrator (Priority: P1)

As a MoonMind operator, I want one dashboard view that shows running and queued work across Agent Queue and Orchestrator so I can monitor current platform activity without switching tools.

**Why this priority**: Observability of in-flight work is the core outcome and prerequisite for efficient operations.

**Independent Test**: Open the consolidated dashboard route, verify it renders queue and orchestrator data, and confirm entries update during polling without page refresh.

**Acceptance Scenarios**:

1. **Given** Agent Queue and Orchestrator each have active work, **When** the user opens the consolidated tasks page, **Then** the page shows rows for both sources with source-specific and normalized statuses.
...
### User Story 2 - Submit Queue and Orchestrator Runs (Priority: P1)

As an authenticated user, I want to submit Agent Queue jobs and Orchestrator runs from the dashboard so I can start work directly from the UI.
...
### User Story 3 - Inspect Queue and Orchestrator Execution Details (Priority: P2)

As a MoonMind operator, I want detail pages for queue jobs and Orchestrator runs so I can diagnose failures and review outputs.
...
2. **Given** an Orchestrator run with plan steps and artifacts, **When** the user opens run detail, **Then** the page renders step status history and available artifacts.
3. **Given** an invalid or inaccessible run/job identifier, **When** detail is requested, **Then** the UI shows a clear not-found or authorization error state.
...
- **FR-001**: System MUST provide a consolidated tasks page that lists active work from Agent Queue and Orchestrator in one view.
- **FR-002**: System MUST provide separate list pages for Agent Queue jobs and Orchestrator runs.
- **FR-003**: System MUST provide submit forms for Agent Queue jobs and Orchestrator runs.
- **FR-004**: System MUST provide detail pages for queue jobs and Orchestrator runs.
...
- **SC-001**: Users can load a consolidated active-work page that includes queue and orchestrator systems in one navigation flow.
- **SC-002**: Users can submit queue and orchestrator run/job types from dashboard forms with success and error outcomes clearly visible.

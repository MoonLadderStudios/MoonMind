...
2. **Given** both workers start, **When** logs are inspected, **Then** each worker confirms Agentkit CLI availability and queue configuration.
...
### User Story 2 - Skills-First Routing with Agentkit Always Available (Priority: P1)

As a platform engineer, I want workflow stages to resolve through skills-first policy while preserving Agentkit as default and always present on workers.

**Why this priority**: 015 umbrella semantics require skills-based orchestration without losing existing Agentkit behavior.
...
1. **Given** default settings, **When** workflow stages execute, **Then** they record `selectedSkill=agentkit` and `executionPath=skill`.
...
As a release owner, I want Codex workers to scale while preserving existing queue compatibility (`agentkit`, `codex`, `gemini`) during rollout.
...
1. **Given** `celery_codex_worker` is scaled, **When** tasks are queued, **Then** workers continue consuming the configured `agentkit` and `codex` queues.
...
- Agentkit CLI is missing in one worker image.
...
- **FR-001**: The system MUST run a dedicated `celery_codex_worker` service that consumes the configured Codex queue and remains compatible with the shared `agentkit` queue topology.
...
- **FR-004**: Worker startup MUST verify Agentkit CLI capability for Codex and Gemini worker entrypoints.
...
- **FR-006**: Workflow stage routing MUST remain skills-first with Agentkit default selection and allowlisted overrides.
...
- **CodexWorkerStartupProfile**: Captures queue bindings, Codex preflight result, Agentkit capability status, and embedding readiness diagnostics.
...
- **SC-002**: Worker startup logs always include Codex/Gemini CLI checks, Agentkit capability checks, and queue binding diagnostics.
...
- Agentkit remains installed in worker images and mirrored in skill directories.

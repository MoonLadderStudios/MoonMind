...
### `TaskPresetSeedDocument` (`api_service/data/task_step_templates/speckit-orchestrate.yaml`)
...
- Seed alignment migration updates the existing seeded `speckit-orchestrate` version row payload from YAML.

## Alignment Data Changes

- `speckit-orchestrate` template row:
  - Refresh `required_capabilities` and `is_active` from YAML.
- `speckit-orchestrate` version row (`1.0.0`):

...
1. Open `api_service/data/task_step_templates/agentkit-orchestrate.yaml`.
...
1. Run Alembic upgrade to include `202603010001_align_agentkit_orchestrate_publish_stage.py`.
2. Confirm migration completes without schema changes or errors.

## 3. Validate seeded data behavior

1. Query `task_step_templates` + `task_step_template_versions` for slug `agentkit-orchestrate`.

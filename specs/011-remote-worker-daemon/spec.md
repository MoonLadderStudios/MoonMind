...
### User Story 1 - Deterministic Worker Startup with Agentkit + Embedding Readiness (Priority: P1)
...
**Why this priority**: 015 umbrella requires Agentkit always available and actionable startup diagnostics for Codex/Gemini embedding profiles.
...
1. **Given** Codex and Agentkit CLIs are installed, **When** preflight runs, **Then** startup validates `codex login status` and `agentkit --version` before daemon loop starts.
...
2. **Given** a `codex_skill` job with `skillId=agentkit`, **When** worker executes it, **Then** it uses the skills path and records `executionPath=skill`.
3. **Given** a `codex_skill` job with an allowlisted non-Agentkit skill, **When** worker executes it, **Then** it runs through compatibility fallback and records `executionPath=direct_fallback`.
...
- `codex` CLI is present but `agentkit` CLI is missing.
...
- **FR-002**: Worker startup MUST verify Codex and Agentkit CLI availability and MUST fail fast when checks fail.
...
- **RemoteWorkerStartupProfile**: Preflight result for Codex/Agentkit availability and embedding profile readiness.
...
- **SC-001**: `moonmind-codex-worker` exits quickly with clear messaging when Codex/Agentkit CLI or required embedding credentials are missing.

# Feature Specification: Codex & workflow Tooling Availability

**Feature Branch**: `004-install-codex-spec`  
**Created**: 2025-11-07  
**Status**: Draft  
**Input**: User description: "Codex and workflow: The api_service/Dockerfile must install Codex CLI and GitHub workflow so they are usable by Celery tasks. Additionally, codex should be configured so that the ~/.codex/config.toml contains approval_policy = \"never\" so that it will not get stuck on permissions."
...
### User Story 2 - workflow CLI Availability (Priority: P2)

workflow maintainers need the GitHub workflow CLI preinstalled so discover, submit, and publish phases can execute within Celery jobs.

**Why this priority**: Without the workflow binary, automation cannot orchestrate repository checks or publish results; packaging it keeps workflows deterministic.

**Independent Test**: Start a worker from the updated image and execute the standard workflow smoke test; it should complete end-to-end without fetching additional tooling.

**Acceptance Scenarios**:

1. **Given** the updated image, **When** a Celery task runs the workflow CLI entrypoint, **Then** the binary is on the PATH for the worker user and completes the smoke test script.
...
- The build environment lacks network access to download the Codex or workflow binaries; the process must fail fast with instructions instead of producing a partial image.
...
- **FR-002**: The same image MUST bundle the GitHub workflow CLI, including any dependencies it requires to execute discover, submit, and publish phases within Celery.
...
- **Automation Runtime Image**: The container image used by the api_service and Celery workers; it now encapsulates Codex CLI, workflow CLI, and their metadata so every worker is consistent.
...
- Build infrastructure has enough network access or cached artifacts to install Codex CLI and GitHub workflow from trusted sources.
...
- **SC-002**: At least 95% of workflow automation runs finish on their first attempt without failures attributed to missing workflow binaries or dependencies.

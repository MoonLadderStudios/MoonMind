# Requirements Traceability: Step Approval Policy

| DOC-REQ | FR ID(s) | Implementation Surface | Validation Strategy |
|---|---|---|---|
| DOC-REQ-001 | FR-001 | `tool_plan_contracts.py` — new `ApprovalPolicyPolicy` dataclass | Unit test: construct with valid/invalid args |
| DOC-REQ-002 | FR-002 | `tool_plan_contracts.py` — extend `PlanPolicy` | Unit test: `PlanPolicy` with/without `approval_policy` |
| DOC-REQ-003 | FR-003 | `tool_plan_contracts.py` — update `parse_plan_definition()` | Unit test: parse JSON with/without `approval_policy` block |
| DOC-REQ-004 | FR-004 | `approval_policy.py` — `ReviewRequest` dataclass | Unit test: construction and `to_payload()` |
| DOC-REQ-005 | FR-004 | `approval_policy.py` — `ReviewVerdict` dataclass | Unit test: construction, validation of verdict values |
...
| DOC-REQ-009 | FR-007 | `approval_policy.py` — `build_feedback_input()` for skill steps | Unit test: verify `_review_feedback` key in inputs |
| DOC-REQ-010 | FR-007 | `approval_policy.py` — `build_feedback_instruction()` for agent_runtime | Unit test: verify feedback appended to instruction |
...
| DOC-REQ-016 | FR-011 | `run.py` — finish summary `approvalPolicy` metrics | Unit test: verify summary JSON shape |
| DOC-REQ-017 | FR-012 | `run.py` — compose with FAIL_FAST/CONTINUE | Unit test: test both failure modes with approval policy |

# Feature Specification: Step Approval Policy

**Feature Branch**: `086-step-review-gate`
**Created**: 2026-03-18
**Status**: Draft
**Input**: User description: "Fully implement docs/Tasks/StepApprovalPolicySystem.md"

## Source Document Requirements

Source: `docs/Tasks/StepApprovalPolicySystem.md`

| ID | Source Section | Requirement Summary |
|---|---|---|
| DOC-REQ-001 | §4.1 PlanPolicy Extension | System must define a `ApprovalPolicyPolicy` dataclass with `enabled`, `max_review_attempts`, `reviewer_model`, `review_timeout_seconds`, `skip_tool_types` fields |
| DOC-REQ-002 | §4.1 PlanPolicy Extension | `PlanPolicy` must accept an optional `approval_policy` field defaulting to disabled |
| DOC-REQ-003 | §4.2 Plan JSON Schema | Plan JSON must accept a `policy.approval_policy` object; missing or `enabled: false` must behave identically to current system |
...
| DOC-REQ-016 | §8.4 Finish Summary | `reports/run_summary.json` must include `approvalPolicy` metrics |
...
### User Story 1 - Enable Approval Policy for a Workflow (Priority: P1)

An operator creates a new task in Mission Control and enables the approval policy toggle. The system automatically validates each step's output against its inputs using an LLM reviewer. Steps that fail review are retried with feedback.

**Why this priority**: Core value proposition — the fundamental toggle-and-review capability.

**Independent Test**: Can be tested by creating a `MoonMind.Run` workflow with `approval_policy.enabled: true` in plan policy and verifying review activities execute after each step.

**Acceptance Scenarios**:

1. **Given** a plan with `approval_policy.enabled: true` and 3 skill nodes, **When** the workflow executes, **Then** a `step.review` activity runs after each of the 3 step executions
2. **Given** a plan with `approval_policy` absent, **When** the workflow executes, **Then** no review activities run and behavior is identical to current system
...
### User Story 3 - Configure Approval Policy Parameters (Priority: P2)

An operator configures approval policy parameters (max attempts, reviewer model, timeout, skip list) at plan level, workflow level, or via env var. The system respects the configured precedence.

**Why this priority**: Configuration flexibility enables cost control and customization.

**Independent Test**: Can be tested by setting `approval_policy` at different precedence levels and verifying correct resolution.

**Acceptance Scenarios**:

1. **Given** a plan with `approval_policy.max_review_attempts: 3`, **When** the workflow executes, **Then** steps are retried up to 3 times on review failure
2. **Given** a plan without `approval_policy` and workflow-level `approvalPolicy.enabled: true`, **When** the workflow executes, **Then** the approval policy is active
3. **Given** `MOONMIND_REVIEW_GATE_DEFAULT_ENABLED=true` and no plan/workflow override, **When** a workflow executes, **Then** the approval policy is active

---

### User Story 4 - Observe Review Results (Priority: P2)

An operator monitors a running workflow in Mission Control and can see review verdicts, retry progress, and final approval policy metrics in the finish summary.
...
1. **Given** a running workflow with approval policy enabled, **When** a step is under review, **Then** the workflow memo shows "Step N/M: tool_name (review attempt X/Y)"
2. **Given** a completed workflow with approval policy, **When** the finish summary is retrieved, **Then** it includes `approvalPolicy.stepsReviewed`, `approvalPolicy.totalReviewAttempts`, and per-outcome counts
...
- What happens with `max_review_attempts: 0`? → Treated as approval policy disabled for that step (execute once, no review).
...
- **FR-001**: System MUST add `ApprovalPolicyPolicy` with fields `enabled` (default false), `max_review_attempts` (default 2), `reviewer_model` (default "default"), `review_timeout_seconds` (default 120), `skip_tool_types` (default empty) → DOC-REQ-001
- **FR-002**: System MUST extend `PlanPolicy` with an optional `approval_policy` field defaulting to `ApprovalPolicyPolicy()` → DOC-REQ-002
- **FR-003**: Plan JSON parsing MUST accept and validate the `policy.approval_policy` block; absent/disabled must produce identical behavior to current system → DOC-REQ-003
- **FR-004**: System MUST define `ReviewRequest` and `ReviewVerdict` data contracts → DOC-REQ-004, DOC-REQ-005
- **FR-005**: `_run_execution_stage` MUST wrap each plan node in a review-retry loop when `approval_policy.enabled` is true and the node's tool type is not in `skip_tool_types` → DOC-REQ-006, DOC-REQ-018
...
- **FR-011**: Workflow memo MUST update to show review cycle state; finish summary MUST include `approvalPolicy` metrics → DOC-REQ-015, DOC-REQ-016
...
- **ApprovalPolicyPolicy**: Configuration object controlling approval policy behavior per plan
...
- **SC-001**: A workflow with approval policy enabled executes a review activity after every non-exempt step
...
- **SC-003**: A workflow with approval policy disabled behaves identically to the current system with zero overhead
...
- Temporal's default history limits (50,000 events) are adequate for typical plans with approval policy (worst case: ~40 extra events for a 20-step plan with 2 retries each)

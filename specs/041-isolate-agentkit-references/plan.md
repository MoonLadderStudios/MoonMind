...
Refactor workflow execution so skill adapters are the authoritative execution path, remove mandatory Agentkit checks from non-agentkit runtime flows, and introduce neutral workflow API/config naming with backwards-compatible SPEC aliases. The implementation will keep data persistence stable, add explicit unsupported-skill failures, and add telemetry/logging for legacy alias usage during migration.

## Technical Context

**Language/Version**: Python 3.11  
**Primary Dependencies**: FastAPI routing (`api_service`), Pydantic settings (`moonmind.config.settings`), Celery worker startup modules (`celery_worker/*`), workflow execution modules (`moonmind/workflows/skills/*`, `moonmind/workflows/agentkit_celery/tasks.py`).  
...
**Constraints**: Preserve existing workflow storage schema, preserve legacy SPEC-prefixed API compatibility during migration, avoid requiring Agentkit when selected skill/runtime does not need it.  
...
│   ├── runner.py
│   └── resolver.py
├── workflows/agentkit_celery/tasks.py
...
See `specs/036-isolate-agentkit-references/research.md`. Key decisions:
...
2. Gate Agentkit executable verification by effective selected skills, not by unconditional startup/task checks.
...
- **Quickstart** (`quickstart.md`): Validation commands for adapter execution, startup behavior without Agentkit, and API alias checks.
- No `DOC-REQ-*` IDs exist in `spec.md`; requirements-traceability matrix is not required.

## Implementation Strategy

### US1 – Skill-first execution without mandatory Agentkit dependency
...
- Update `moonmind/workflows/skills/resolver.py` to remove hardcoded builtin source fallback for agentkit and require resolvable sources.
...
- Refactor Agentkit CLI checks in:
  - `moonmind/agents/codex_worker/cli.py`
  - `celery_worker/agentkit_worker.py`
  - `celery_worker/gemini_worker.py`
  - `moonmind/workflows/agentkit_celery/tasks.py`
  so checks run only when selected/allowed skills require Agentkit.
- Ensure non-agentkit paths can start and run without Agentkit installed.
...
- **Risk**: Removing builtin source fallback could break environments without mirrored Agentkit skill content.  

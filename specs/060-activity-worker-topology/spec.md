...
| DOC-REQ-006 | `docs/Temporal/ActivityCatalogAndWorkerTopology.md` §6.3 (lines 257-297) | Skill execution must use the hybrid binding model with `mm.tool.execute` as the default path and registry-driven explicit activity bindings only when an operational reason is declared. |
...
- Q: What qualifies as a valid curated explicit skill-to-activity binding instead of the default `mm.tool.execute` path? → A: The registry may use an explicit activity binding only when it declares a concrete operational reason: stronger isolation, specialized credentials, or clearer routing; otherwise the default path remains `mm.tool.execute`.

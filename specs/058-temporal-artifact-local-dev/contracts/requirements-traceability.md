...
| DOC-REQ-014 | `docs/Temporal/WorkflowArtifactSystemDesign.md` §14 | FR-015 | `api_service/api/routers/temporal_artifacts.py`, `moonmind/schemas/temporal_artifact_models.py`, `specs/058-temporal-artifact-local-dev/contracts/temporal-artifacts.openapi.yaml` | Contract tests validate request/response schema and endpoint coverage for create/presign/complete/get/list/link/pin/delete |
...
| DOC-REQ-016 | `docs/Temporal/WorkflowArtifactSystemDesign.md` §5.3 | FR-017 | `moonmind/workflows/temporal/workflows/run.py`, `moonmind/workflows/temporal/activity_catalog.py`, activity queue topology configuration surfaces | Workflow unit tests verify `artifact.read` and related `artifact.*` execution-path calls resolve to the artifact queue and fail fast when routed to non-artifact queues |
...
- Source requirements tracked: **16** (`DOC-REQ-001` through `DOC-REQ-016`).

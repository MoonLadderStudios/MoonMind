...
The selected image supplies the canonical Compose definition, application code, migrations, portable Skills and release controller. Deployment-owned `.env`, interfaces, authentication and explicit configuration retain their existing authority. The shared controller operation (`depupd_` record under `deploy/state/update-operations/`) is the portable semantic identity for both this Skill and Settings Operations: neither path creates a `MoonMind.UserWorkflow`, and the browser, workflow engine, and application artifact service do not supervise its lifetime. The image-owned deployment-control invocation underneath remains the execution adapter for the recorded operation. Docker, durable state storage and Temporal supply the execution substrate. A managed Skill retains its authorized tool boundary: it never acquires a Docker socket or the deployment-owned controller secret, which stays server-to-controller only.
...
If the caller disappears, resume the printed submission with `--resume <submission-id>`. Keep its original image and inputs. Every submission and resume attaches to the shared deployment-controller operation in `deploy/state/update-operations/<depupd-id>.json` — the same operation Settings Operations observes. A duplicate submission or lost acknowledgment reattaches to the live operation instead of starting a second updater. After a terminal operation, `--retry-operation <depupd-id>` requests a fresh bounded attempt that preserves the first failure; a changed target is explicit new intent and starts a new operation. Inspect the durable result before retrying any side effect; preserve primary deployment success if only cleanup remains. Report the exact unfinished phase and its recorded recovery owner when bounded recovery exhausts.

## Options

Optional arguments are `--compose-project <name>`, `--image-repository <repository>`, and `--dry-run` (show the intended release operation without fetching or deploying). A `--dry-run` preview never establishes completion: it writes no submission, no controller operation, and proves nothing about the installed deployment. The deployment-owned `docker-compose.override.yaml` (or `.yml`) accompanies the image's base configuration.

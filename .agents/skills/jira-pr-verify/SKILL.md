---
name: jira-pr-verify
description: Verify a GitHub pull request against a Jira issue's goals, requirements, and acceptance criteria, then post a PR comment with the findings. Use when a user asks to compare a PR to a Jira story/task/bug, confirm whether a PR satisfies Jira requirements, audit implementation coverage from Jira, or publish a Jira-vs-PR verification summary.
metadata:
...
If Jira content is not already available to the managed agent, first use MoonMind's trusted Jira MCP tool path when it is exposed to the runtime; see
...
such as `test -n "$GITHUB_TOKEN"` or the preflight from
[the provider command catalog](references/provider-commands.md); do not run
`printenv`, `env`, `set`, or equivalent commands that can dump secret values
into workflow logs.
...
Before declaring GitHub access blocked, run the preflight checks from the managed
agent shell (see [the provider command catalog](references/provider-commands.md)
for the bundled-helper and equivalent `gh` invocation shapes; load it only
...
- `gh` is not installed,
- preflight reports no usable authentication,
- preflight reads fail for the target repository/PR.

When both `gh` and the connector are available, prefer `gh` for PR metadata,
diff, checks, and comments; see [the provider command catalog](references/provider-commands.md)
for the exact invocation shapes.
...
3. Inspect the PR.
- Use the PR metadata, diff, and checks operations from [the provider command catalog](references/provider-commands.md) (or local `git diff <base>...HEAD`) for implementation evidence.
- Read changed code, tests, docs, workflow files, and configuration relevant to the Jira ledger.
- Check CI status with the PR checks operation from [the provider command catalog](references/provider-commands.md) when available.

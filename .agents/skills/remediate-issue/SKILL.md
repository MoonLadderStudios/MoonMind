...
3. Inspect the production code and existing tests that own each gap. When a gap names a workflow, Activity, adapter, persistence layer, side-effect owner, or other authority handoff, exercise that real boundary. A test-only model, dictionary, mock, or hard-coded identity is not equivalent production evidence.
   Reuse the assessment/verifier requirement map, exact candidate identity and prerequisite attempt ledger. Prepare permitted test dependencies within the remaining two-attempt budget per owning service and candidate. When only evidence collection failed, preserve the saved candidate and return the exact evidence-retry action; do not spend an implementation attempt rewriting completed code or resetting a prerequisite budget.

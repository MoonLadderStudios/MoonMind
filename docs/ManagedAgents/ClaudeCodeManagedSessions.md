...
### 2.4 Model deterministic and non-deterministic security controls explicitly
...
| `bypassPermissions` | skip permission prompts and security checks except protected-path handling |
...
In `auto`, unauthorized, untrusted, or ambiguous actions are checked by a classifier path. The plane should record classifier decisions as distinct from user approvals.
...
This mirrors source-control security expectations while remaining session-native.

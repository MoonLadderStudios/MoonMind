...
Canonical for: semantic step re-executions, checkpointed side-effect policy, gated iteration, failed-step recovery primitive, autonomous story loops
...
MoonMind tasks often need more than one pass. An implementation step may fail tests, a verifier may report `ADDITIONAL_WORK_NEEDED`, a pull request resolver may need another repair pass, an autonomous story loop may need a fresh agent context, or an operator may resume a failed task from the last failed step.

All of those cases depend on the same primitive:

> Re-execute a logical step as a new Step Execution while explicitly controlling which previous side effects are preserved, restored, ignored, invalidated, promoted, or superseded.
...
2. per-execution identity, lineage, context, artifacts, checks, and disposition;
...
Logical Step: implement-story-S004
  Step Execution 1: failed quality gate
  Step Execution 2: failed integration evidence
  Step Execution 3: passed and accepted
```
...
| Workflow state | Current/latest compact step and Step Execution state, active loop state, budgets, child refs | Authoritative live state |
| Step ledger query | Operator-facing current step rows, latest Step Execution refs, checks, preserved provenance | Read projection of workflow state |
| Step Execution manifest artifact | Immutable full Step Execution evidence: context, outputs, diffs, gates, retrieval, memory proposals, side-effect classification | Authoritative Step Execution evidence |
...
| Derived app DB projection | Fast UI/API reads, degraded reads, optional Step Execution list | Repairable read model only |

Rules:

1. Step Execution evidence is append-only.
2. Workflow state carries compact refs and bounded summaries, not large payloads.
3. The step ledger may show only the latest/current Step Execution by default.
4. Step Execution history is available through expanded API/UI surfaces using Step Execution manifest refs.
5. App DB projections must be downstream of workflow state, artifact linkage, and git state. They must not invent step or Step Execution truth.
...
Repeating a step must never be implicit. Before launching a new Step Execution, MoonMind must know:

1. which logical step is being repeated;
2. which previous execution caused the repeat;
3. what input and context the new Step Execution sees;
4. what workspace state the new Step Execution starts from;
...
A passing Step Execution may advance the logical step only after workflow-owned gates accept the result. Agent self-assessment alone is not sufficient.
...
| **Step Execution** | One semantic execution of a logical step. Local Step Executions are scoped to `(workflowId, runId, logicalStepId, executionOrdinal)`. |
| **Local Execution Ordinal** | The execution ordinal within one `(workflowId, runId, logicalStepId)` scope. |
| **Lineage Execution** | Optional cross-run continuation ordinal used for UI/provenance, such as “resumed Step Execution 2” when a linked Resume execution starts local execution ordinal 1. |
...
| **Resume** | Product recovery action that creates a linked follow-up execution and starts new work at a step re-execution of the failed logical step. |
...
resume    = linked execution that begins by creating a step re-execution
```
...
2. Local Step Execution identity is explicit and monotonically increasing per `(workflowId, runId, logicalStepId)`.
...
16. Runtime adapters may execute Step Executions, but MoonMind owns Step Execution identity, checkpoint policy, durable evidence refs, and final advancement decisions.
...
### 6.1 Local Step Execution identity

The authoritative local key for a Step Execution is:

```text
(workflowId, runId, logicalStepId, executionOrdinal)
```

`execution_ordinal` is local to the current Temporal run of the current workflow execution.

Representative deterministic identifiers:

```text
stepExecutionId     = {workflowId}:{runId}:{logicalStepId}:execution:{execution_ordinal}
childWorkflowId   = {workflowId}:agent:{logicalStepId}:execution:{execution_ordinal}
checkpointId      = {workflowId}:{runId}:{logicalStepId}:execution:{execution_ordinal}:{boundary}
artifactLinkScope = step:{logicalStepId}:execution:{execution_ordinal}
```
...
Failed-step Resume creates a new linked follow-up execution. In the resumed execution, the first newly executed failed step usually starts at local execution ordinal `1`, because it is a new `(workflowId, runId, logicalStepId)` scope.

For operator clarity, the resumed Step Execution may also carry lineage:
...
```text
Step 3 — resumed Step Execution 2, local execution ordinal 1 in this run
```

Rules:

1. Local Step Execution identity remains the durable storage key.
...
All side-effecting Activities participating in a Step Execution must accept or derive stable idempotency keys.

Default key shape:

```text
{namespace}:{workflowId}:{runId}:{logicalStepId}:{execution_ordinal}:{operation}
```
...
| --- | --- |
| Create Step Execution manifest | `{workflowId}:{runId}:{logicalStepId}:{execution_ordinal}:manifest` |
| Capture workspace checkpoint | `{workflowId}:{runId}:{logicalStepId}:{execution_ordinal}:checkpoint:{boundary}` |
| Launch managed AgentRun | `{workflowId}:{runId}:{logicalStepId}:{execution_ordinal}:agent-run` |
| Run quality gate | `{workflowId}:{runId}:{logicalStepId}:{execution_ordinal}:gate:{gateName}` |
| Commit accepted changes | `{workflowId}:{runId}:{logicalStepId}:{execution_ordinal}:commit` |
| Create PR | `{workflowId}:{runId}:{logicalStepId}:{execution_ordinal}:publish-pr` |
| Jira transition | `{workflowId}:{runId}:{logicalStepId}:{execution_ordinal}:jira-transition:{targetStatus}` |
...
```json
{
  ...
  "context": {
    "contextBundleRef": "art_execution_context",
...
| `recover_from_failed_step` | A linked Resume execution is running the failed step again. |
...
| `succeeded` | Step Execution completed and required checks passed. |
...
| `superseded` | A later Step Execution replaced this Step Execution as the current candidate. |

### 7.3 Terminal dispositions

| Disposition | Meaning |
| --- | --- |
| `accepted` | Step Execution passed gates and may advance the logical step. |
| `retryable` | Another Step Execution is allowed under budget and policy. |
| `blocked` | Missing prerequisite, credential, infrastructure, or approval prevents progress. |
| `needs_human` | Automated Step Executions are exhausted or unsafe. |
| `discarded` | Step Execution evidence is retained but workspace effects are not reused. |
| `superseded` | A later Step Execution replaced this Step Execution. |
| `failed_unrecoverable` | The Step Execution or gate found a permanent blocker or unsafe condition. |
...
3. The context bundle must include the workspace policy and baseline visible to the Step Execution.
4. If two Step Executions see different retrieval or memory context, that difference must be recorded through the context bundle and manifest refs.
5. The bundle should include a digest and builder version so operators can diagnose context drift.

### 8.1 Retrieval context

Retrieval and RAG inputs must be Step Execution inputs, not hidden ambient state.
...
| State | Meaning |
| --- | --- |
| `proposed` | Step Execution generated a possible memory update. |
| `accepted_for_run_context` | Memory is visible to later Step Executions in the same run. |
...
Checkpoints are durable artifacts or durable read-model records that make step re-executions and Resume truthful.
...
```json
{
  ...
  "workspace": {
    ...
    "manifestRef": "art_execution_2_patch_manifest"
  },
  "stepOutputs": {
    "summaryRef": "art_execution_2_summary",
    "agentResultRef": "art_execution_2_result",
    "verificationRef": "art_execution_2_verification"
  },
...
Before a checkpoint can be used to start a new Step Execution or Resume execution, MoonMind must validate:

1. source `workflowId` and `runId`;
2. task input snapshot identity;
3. plan identity and digest;
4. logical step identity;
5. Step Execution provenance;
...
Repeated coding work is not purely functional. Each Step Execution may leave a changed workspace.

MoonMind must record a workspace policy before launching a repeated Step Execution.
...
| `apply_previous_execution_diff_to_clean_baseline` | Reset to a clean baseline, then apply the previous execution diff as an explicit patch artifact. | Preserve useful work while removing hidden drift. |
...
| Disposition | Meaning |
| --- | --- |
| `accepted` | Step Execution changes are committed or otherwise durable and approved to advance the logical step. |
| `candidate` | Step Execution produced useful changes but did not pass gates. Diff/checkpoint evidence is retained. |
| `discarded` | Step Execution changes are intentionally not carried forward. Evidence remains. |
| `superseded` | A later Step Execution replaces this Step Execution's candidate state. |
| `none` | Step Execution did not produce workspace changes. |
...
```json
{
  ...
  "headCommit": null,
  "workingTreeDiffRef": "art_diff_execution_2",
  "patchRef": "art_patch_execution_2",
  "commitCreated": false,
  "workspaceCheckpointRef": "art_workspace_after_execution_2",
...
```json
{
  "baselineCommit": "abc123",
  "headCommit": "def456",
  "workingTreeDiffRef": "art_diff_execution_3",
  "patchRef": "art_patch_execution_3",
  "commitCreated": true,
  "workspaceCheckpointRef": "art_workspace_after_execution_3",
...
```text
failed or partial Step Execution -> diff artifact only, no story advancement
passing Step Execution           -> commit, accepted git effect, logical step advancement
```
...
Every nontrivial Step Execution should classify side effects before the workflow advances.

Suggested side-effect classes:

| Class | Meaning | Step Re-execution posture |
| --- | --- | --- |
...
| `external_non_idempotent` | External mutation that cannot safely repeat. | Forbidden in autonomous step re-executions unless explicit policy permits. |
...
```json
{
  ...
  "idempotencyKey": "mm:task:run-1:verify:execution:1:jira-comment",
  "target": "MM-123",
  "disposition": "accepted"
}
```
...
1. Non-idempotent external work must not be hidden inside repeated implementation Step Executions.
2. Publication, Jira transitions, merge, deployment, and provider-account actions require gate-approved workflow state.
3. If a non-idempotent side effect already occurred, a later Step Execution must account for it explicitly rather than pretending the step can be reset.
...
`MoonMind.Run` owns orchestration. Activities own side effects. Runtime adapters execute Step Executions but do not own Step Execution semantics.
...
| `step_execution.write_manifest` | Write/update immutable or append-only Step Execution evidence. | `mm.activity.artifacts` |
| `step_execution.record_evidence` | Link runtime outputs, checks, and side-effect refs to Step Execution evidence. | `mm.activity.artifacts` |
...
| `workspace.apply_policy` | Apply selected workspace policy before a Step Execution. | `mm.activity.sandbox` |
...
| `retrieval.build_execution_manifest` | Build retrieval manifest for Step Execution context. | `mm.activity.artifacts` / retrieval worker if added |
...
4. Activities must be idempotent or guarded by idempotency keys derived from Step Execution identity.
...
while verdict == ADDITIONAL_WORK_NEEDED and budget remains:
  create new remediation Step Execution
  run remediation using latest verification report
  create new verification Step Execution
  verify again
...
Agent instructions may say what to do inside a Step Execution, but the parent workflow must own:

1. whether another Step Execution is allowed;
...
| `ADDITIONAL_WORK_NEEDED` | More bounded work is required and may be executed if budget remains. |
...
```json
{
  ...
  "confidence": "medium",
  "validatedRefs": {
    "diffRef": "art_execution_2_diff",
    "testReportRef": "art_execution_2_tests"
  },
  "invalidatedRefs": [],
  "remainingWorkRef": "art_remaining_work",
  "blockingEvidenceRefs": ["art_verify_report"],
  "recommendedNextAction": "reexecute_current_step",
...
```json
{
  ...
  "validatedRefs": {
    "commit": "def456",
    "diffRef": "art_execution_3_diff",
    "testReportRef": "art_execution_3_tests",
    "browserEvidenceRef": "art_execution_3_browser"
  },
  "invalidatedRefs": [],
...
1. maximum Step Executions;
2. maximum wall-clock time;
3. maximum provider spend or token estimate;
4. maximum consecutive no-progress Step Executions;
5. maximum failed command repeats;
6. maximum unsafe or policy-denied Step Executions.
...
A repeated Step Execution can change the accepted output of a logical step. Downstream steps that consumed the old output may no longer be valid.

Rules:

1. Activity/tool/agent outputs used by downstream steps must be associated with the producing Step Execution identity.
...
4. Preserved steps in Resume must carry `preservedFrom.workflowId`, `preservedFrom.runId`, `preservedFrom.logicalStepId`, and `preservedFrom.executionOrdinal`.
...
This rule is required for both failed-step Resume and autonomous story loops. Without it, MoonMind could accidentally present downstream work as valid even though it was built against a superseded upstream Step Execution.

---

## 15. Resume Relationship

Failed-step Resume is one consumer of Step Executions with Checkpointing.
...
Resume Step Execution properties:

1. `reason = recover_from_failed_step`;
2. local `Step Execution` starts in the resumed run scope, usually `1`;
3. `lineage.sourceExecutionOrdinal` points to the failed Step Execution in the source run;
...
5. conflate local Step Execution identity with cross-run lineage identity.
...
Step 3: Run tests — resumed Step Execution 2, local execution ordinal 1 running now
```
...
Managed runtimes execute Step Executions; MoonMind owns Step Execution semantics.

For a managed agent runtime Step Execution, `MoonMind.Run` should:
...
An agent may recommend another Step Execution, but it must not unilaterally create hidden Step Executions or mutate the Step Execution ledger.

### 16.1 Runtime context policy

Step Execution records should distinguish runtime/session policy from workspace policy.
...
```json
{
  "runtimeContextPolicy": "fresh_agent_run",
  "sessionPolicy": {
    "codex_cli": "new_epoch_per_execution",
    "claude_code": "new_process_per_execution"
  }
}
```
...
| Policy | Meaning |
| --- | --- |
| `fresh_agent_run` | Start a new `MoonMind.AgentRun` child workflow for the Step Execution. |
| `reuse_session_new_epoch` | Reuse a task-scoped managed session container but clear/reset to a new epoch before the Step Execution. |
| `reuse_session_same_epoch` | Keep session continuity across Step Executions; should be rare and explicit. |
...
For Codex managed sessions, Ralph-style clean context should usually mean `fresh_agent_run` plus `reuse_session_new_epoch` or equivalent `clear_session` semantics. For Claude Code, it should usually mean a new process/container per Step Execution. External or coordinated agents may have weaker runtime control; in that mode, MoonMind should still record Step Execution identity, context refs, known side effects, and checkpoint evidence where available. Features that require direct workspace restoration may be unavailable or require a different policy.
...
1. each logical step shows the latest/current Step Execution;
2. Step Execution count is visible;
...
Expanded surfaces should allow operators to inspect Step Execution history:
...
APIs should expose bounded Step Execution projections and artifact refs. They must not inline large transcripts, diffs, provider payloads, or verification reports.

Suggested API shape:

```http
GET /api/executions/{workflowId}/steps
GET /api/executions/{workflowId}/steps/{logicalStepId}/executions
GET /api/executions/{workflowId}/steps/{logicalStepId}/executions/{execution_ordinal}
```

Representative Step Execution list response:
...
When a Step Execution fails or a gate requests more work, MoonMind must classify the next state.
...
1. Step Execution context bundles must not include raw credentials.
...
5. Non-idempotent external work must not be re-executed without explicit policy.
6. Publication, Jira transition, merge, deployment, and provider-account actions require gate-approved state.
7. Failed Step Execution logs and diagnostics must be sanitized before display or publication.
8. Repo and local skill sources remain untrusted input; Step Execution context must respect skill source policy.
...
3. remediate Step Execution 1
4. verify remediation Step Execution 1 -> ADDITIONAL_WORK_NEEDED
5. remediate Step Execution 2
6. verify remediation Step Execution 2 -> FULLY_IMPLEMENTED
7. create pull request
8. move Jira to Code Review
```

If the post-remediation gate remains `ADDITIONAL_WORK_NEEDED` after budget exhaustion, `MoonMind.Run` stops before pull request creation and Jira movement. The final state includes the latest verification report, remaining work, remediation evidence, side-effect dispositions, and a recommended next action.
...
```text
source run:
  step 1 succeeded, local execution ordinal 1
  step 2 succeeded, local execution ordinal 1
  step 3 failed, local execution ordinal 1

resume run:
  step 1 preserved from source refs
  step 2 preserved from source refs
  step 3 local execution ordinal 1 executes from validated checkpoint
          lineage shows resumed Step Execution 2
...
The failed Step Execution remains visible as evidence even though its workspace state is not accepted.
...
```text
step 2 Step Execution 1 accepted API contract v1
step 3 Step Execution 1 implemented UI against API contract v1
step 4 verification failed and requested API change
step 2 Step Execution 2 accepted API contract v2
step 3 is invalidated because it consumed API contract v1
step 3 is re-executed or revalidated before step 4 can pass
```
...
2. hiding Step Executions inside a single agent transcript;
3. storing large diffs or logs in workflow state;
4. treating Temporal Activity retries as semantic step re-executions;
5. automatically replaying every possible external side effect;
6. committing memory proposals from failed Step Executions directly to the repo;
7. making failed-step Resume and autonomous loops separate recovery systems;
8. exposing full immutable Step Execution history in the default task detail view;
9. forcing every Step Type to use autonomous step re-execution semantics.
...
2. Add latest Step Execution refs to the step ledger.
3. Capture child workflow refs, summaries, diagnostics, and diffs per Step Execution.
4. Add bounded API support for Step Execution lists.
...
### Phase 3: Gated step re-execution loops

1. Add structured gate verdicts.
2. Add budgets and stop rules.
3. Add workspace policy selection for step re-executions.
...
2. Use Step Executions for each story implementation pass.
3. Use independent quality gates.
4. Commit only accepted Step Executions.
...
1. **Orchestrate, Don't Recreate**: agents execute Step Executions; MoonMind owns orchestration, state, and evidence.
2. **Own Your Data**: Step Execution context, outputs, diffs, and checkpoints are operator-controlled artifacts.
3. **Resilient by Default**: repeated work is explicit, bounded, idempotency-aware, and recoverable.
4. **Spec-Driven Development**: gates and Step Executions preserve traceability from requirement to implementation to evidence.
5. **Continuous Improvement**: failed Step Executions produce structured evidence and improvement signals rather than disappearing into logs.

The key desired-state rule is simple:

> A repeated step is not the same execution happening again. It is a new Step Execution with explicit context, checkpoint, side-effect policy, evidence, lineage, and stop rules.

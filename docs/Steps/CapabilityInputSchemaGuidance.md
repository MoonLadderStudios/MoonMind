# Capability Input Schema Guidance
...
This document defines how MoonMind should treat optional capability input schemas
for Skills, Presets, Tools, and future selectable capability types.

MoonMind should make structured input collection better when a capability provides
an input schema, but schema authoring must not become a barrier to third-party
Skill adoption. A plain Agent Skills-style `SKILL.md` with instructions and no
structured input schema remains valid. In that case, the runtime agent may infer,
request, or extract the information it needs from the natural-language task,
Skill body, Workflow instructions, and available context, as agents normally do.

---

## Core Policy

1. `inputSchema` / `input_schema` is optional for Skills and third-party
...
3. When absent, MoonMind must fall back to instruction-driven execution rather
   than rejecting the capability solely because it lacks a schema.
4. Capability authors should describe input data and semantics, not MoonMind UI
   components.
...
Create page experience for capabilities that choose to expose structured inputs.

---

## Capability Catalog Shape

A capability catalog item may expose a normalized input contract:
...
The fields are optional unless the specific capability type has a stronger
internal requirement. For example, many Tools have hard typed contracts because
they execute deterministic operations. Third-party Skills may omit `inputSchema`
and still be selectable.
...
capability. It should provide an instruction-oriented fallback such as a general
instructions field, context attachments, or direct launch with the selected Skill.
...
1. Load a selected capability's `inputSchema`, if present.
...
The capability file should not need to name `jira.issue-picker`,
...
Capability authors may include stable semantic hints in `inputSchema` when the
plain JSON Schema shape is not enough:
...
meaning in the current deployment. This is preferred over binding a capability to
a concrete widget name.
...
preferred source of truth for capability authors.
...
3. capability-authored semantic hints
4. narrowly-scoped capability-authored `uiSchema`, when allowed

Backend validation must always use `inputSchema` and capability-specific policy,
not `uiSchema`.
...
When a selected Skill or third-party capability has no `inputSchema`, MoonMind
...
- MoonMind derives default UI behavior from `inputSchema` without requiring
  capability-authored `uiSchema`.
- Capability-authored `uiSchema`, when present, is optional, safe, and
  non-authoritative.

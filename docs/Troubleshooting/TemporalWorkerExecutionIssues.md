...
## Validation
- The Docker worker containers (`docker compose restart api system temporal-worker-artifacts temporal-worker-integrations temporal-worker-llm temporal-worker-sandbox temporal-worker-workflow`) must occasionally be restarted to load Python code updates into memory.

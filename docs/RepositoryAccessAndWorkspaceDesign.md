...
A cloned repository carries the deployment's commit identity in its repository-local configuration. A Skill that owns its own publication commits inside the host, where the shared image provides a credential helper and no identity, so clone preparation writes one. The deployment already determines this value; an undeclared identity resolves to the documented default rather than blocking a commit-capable run.

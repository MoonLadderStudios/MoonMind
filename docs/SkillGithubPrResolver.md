...
2. **Merge conflicts:** If `mergeable` indicates conflict (`false`, `CONFLICTING`, or `DIRTY`) or `mergeStateStatus` is exactly `DIRTY` → Delegate to `fix-merge-conflicts` skill.
...
6. **Blocked:** If CI is running without failures while `mergeable` is clean (not in conflict) → Exit with `blocked_by_ci_running`.
...
3. Apply fixes in this strict priority order:
   - **Merge Conflicts:** If `mergeable` indicates conflict (`false`, `CONFLICTING`, or `DIRTY`) or `mergeStateStatus` is exactly `DIRTY`, you MUST read `.agents/skills/fix-merge-conflicts/SKILL.md`. Follow its procedure exactly to resolve the conflict.
   ...
   - **Blocked:** If CI is running but no failures while `mergeable` is clean, exit and state the PR is blocked waiting for CI.

...
`NO_DETERMINATION`, `BLOCKED`, and `FAILED_UNRECOVERABLE` block publication unless the existing contract explicitly models recoverable missing evidence. Malformed verdict envelopes are distinct from verifier judgments: inject the canonical verdict/action contract, retain a drifting raw action as diagnostics, derive the canonical action from the verdict, and use at most one report-only corrective re-verify within the existing verification recovery budget and selected runtime/provider authority before spending implementation-remediation cycles. Supply precise validation diagnostics and the original findings as untrusted data. The repair never reruns a successful implementation step, switches accounts/models, invents a missing verdict, or discards known failures; revalidate the corrected report against the unchanged candidate/scope. A `NO_DETERMINATION` without current-runtime recovery (missing tools, low confidence, or `recoverableInCurrentRuntime: false`) derives the automation-owned `blocked` continuation, never an implicit `needs_human`; an explicitly requested `needs_human` is preserved. Contract-invalid evidence downgrades fail-closed with a `downgradeReason` identifying the violating field.
...
A declared environment-class `BLOCKED` or malformed/degraded `NO_DETERMINATION` result publishes an annotated attention-required draft for the same accepted revision. Verifier-declared `NO_DETERMINATION` and `FAILED_UNRECOVERABLE` remain fail-closed. A persistently malformed report after the bounded repair retains its original contract cause and a concrete `blocked` recovery handoff, distinct from environment unavailability and implementation failure; repairing report transport is not verifying the candidate and never authorizes publication as verified work. An unrecoverable inconclusive native result without an explicit continuation stops `blocked` and must not implicitly become `needs_human`; genuine explicit human decisions stay intact. Continue the same PR from its validated current head through the existing #4443 evidence/continuation owner. Draft publication never turns explicit None or Branch into a PR grant and never enables merging after incomplete verification.

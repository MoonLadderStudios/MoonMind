...
| `document-update-orchestrate` | Parent discovers documents; children use PR with merge automation | Discovery source and child workspace context agree; preserve ordering and dependency semantics. `max_documents` defaults to 100 (explicit limit 1–1000); overflow fails before any child is created. Each child edits only its exact document and retains shared-owner/reference work as a coordinated handoff. Queue receipts are not document completion. |
...
A verified no-change implementation that explicitly establishes the authoritative issue is already implemented may complete that issue through the trusted transition boundary. An ambiguous no-change result remains non-mutating and explains the uncertainty.

...
`NO_DETERMINATION`, `BLOCKED`, and `FAILED_UNRECOVERABLE` block publication unless the existing contract explicitly models recoverable missing evidence. Malformed verdict envelopes are distinct from verifier judgments: inject the canonical verdict/action contract, retain a drifting raw action as diagnostics, derive the canonical action from the verdict, and use bounded corrective verification attempts before spending implementation-remediation cycles. A `NO_DETERMINATION` without current-runtime recovery (missing tools, low confidence, or `recoverableInCurrentRuntime: false`) derives the automation-owned `blocked` continuation, never an implicit `needs_human`; an explicitly requested `needs_human` is preserved. Contract-invalid evidence downgrades fail-closed with a `downgradeReason` identifying the violating field.

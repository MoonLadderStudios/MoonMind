...
A plan in which **every** authored step declares `repositoryOperation: read` is the one exception, because it declares no repository mutation authority at all. Managed publication is bound to step outputs, so `branch` and `pr` can never produce publish evidence for such a plan and the run would execute and then fail finalization with an unknown publish outcome. Per PUBLISH-004 that combination is rejected at admission with the authored step that would have to change, not admitted and failed late.

...
Preserve the resolved verifier's canonical `recommendedNextAction` through artifact publication and workflow admission. For `ADDITIONAL_WORK_NEEDED` or `NO_DETERMINATION`, an explicit `needs_human` or `blocked` action stops automatic remediation and evidence retries even when budget remains. Retain the candidate, verification and remaining-work references for the existing terminal handoff; describe this as a verifier-requested stop, not budget exhaustion. A missing action retains the verdict's default routing: for `NO_DETERMINATION` without current-runtime recovery the default is the automation-owned `blocked` continuation, not a new human-review requirement. New native final-verifier results carry an explicit supported continuation decision: an unrecoverable inconclusive result without an explicit action stops `blocked` rather than implying a human decision, while genuine explicit human decisions stay intact. `recoverableInCurrentRuntime: false` alone does not prohibit a separate remediation Skill from fixing an actionable implementation gap.

Gate ingestion rejects contradictory verdict/action pairs: `FULLY_IMPLEMENTED` permits only `advance`, and `BLOCKED` or `FAILED_UNRECOVERABLE` permits only `blocked`. Inconclusive explicit stops retain inline remaining work through a durable artifact reference. Generic review stops preserve the explicit stop disposition and blocked outcome status in their terminal manifest; the step ledger retains its existing failed lifecycle status and explicit action metadata.
...
- `NO_DETERMINATION`: stop with a `blocked` handoff carrying the validation diagnostics and recovery prerequisite, except a separately bounded safe evidence-recovery action permitted by policy. An explicit human stop is preserved; ordinary evidence or report failures never acquire a new human-review requirement. Do not blindly repeat paid implementation.

...
11. **Cumulative candidate-head tracking is opt-in and never gates the loop.** A remediation loop may carry a workflow-owned cumulative workspace head so each attempt continues from the prior candidate checkpoint. That head is supplied by the capture/persistence boundary; when it is absent, both halves of an admitted attempt run against the live cumulative workspace exactly like the authored remediation and verification steps. Absence of the head must not fail the run, and a headless attempt must still advance the loop out of remediation so its verifier's evidence is accepted — a loop left mid-remediation rejects the verification it just paid for. When a head is present it stays the single authority: an attempt or verifier bound to a different head version or attempt ordinal is rejected, and the head crosses Continue-As-New with the loop state so a continued run cannot mistake lost in-memory authority for an intentional opt-out.

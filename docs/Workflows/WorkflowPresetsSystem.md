...
`github-issue-search-and-implement` uses the workflow repository binding. Its typed `github.load_issue_preset_brief` operation accepts `issueSearch` plus `includeAllAuthors` (preset input `include_all_authors`, default `false`). By default only issues created by the GitHub account used for the search are eligible; the generated Create-page checkbox **Include issues created by other users** (unchecked, rendered immediately below the search field by the shared schema renderer) explicitly broadens author eligibility. A nonempty query discovers candidates through GitHub's best open-issue matches constrained with `author:<resolved-login>` over the whole query (parenthesized so `OR` cannot escape the scope) and an empty or whitespace-only query scans open issues in descending creation order with `creator=<resolved-login>`. The authenticated account is the account behind the exact credential used for the workflow's issue-search API requests (`GET /user` ID/login, IDs durable while logins can change), never the MoonMind session user, repository owner, assignee, commenter, Git commit author, or an unrelated interactive `gh` login. Bot-created issues count under the actual creating account. The checkbox broadens only author eligibility: repository binding, open-state, lifecycle, blocker, attempt-ownership, retry, verification, and publication policy still apply. No matching self-authored issue returns an explicit no-eligible-match result and never auto-broadens; unavailable or invalid identity in self-only mode stops with an actionable diagnostic and never falls back to all authors. Explicit Issue Implement / Issue Orchestrate selection is unchanged: deliberately choosing another author's issue remains allowed, and dependency reads are not author-filtered (a self-authored issue may depend on another author's issue, which still blocks). User-entered `author:` qualifiers are backend-owned: absent means the resolved account is applied; `author:@me` or the same account normalizes to one constraint; another author, exclusion of the required author, conflicting, or ambiguous/unsupported author syntax is a field-addressable search-field error requiring explicit opt-in. In all-author mode authored filters (including explicit author filters) are preserved and the checkbox only removes the automatic constraint. Candidate validation (`candidate.user.id == authenticated_user.id`, ID comparison only) runs before expensive checks and before any announcement or mutation, and the selected issue is revalidated against fresh issue detail before the trusted brief/admission handoff; a wrong-author or unverifiable candidate never produces a trusted brief, label, comment, or dispatch. `searchEvidence` carries `authorScope` (`authenticated_user`|`all`), `authenticatedUser` (ID/login in self-only mode), `selectedIssueAuthor`, and `authorMismatchesSkipped` alongside page/candidate counts through brief persistence, compaction, and continuation. Selection, discovery, and same-repository confirmation reuse one resolved credential per bounded operation with existing timeout/cancellation/rate-limit/retry handling; no raw token appears in inputs, history, artifacts, errors, URLs, or logs, and no deployment-global identity cache is added. Distinct no-eligible-self-authored-issue, conflicting-author-filter, invalid-author-evidence, identity/authentication/permission, and provider-unavailable/rate-limit outcomes never widen scope or launch implementation. New/current expansion with a missing input materializes `false`; saved true/false values persist through reload, edit, rerun, scheduling, and dispatch; already-admitted historical executions keep pinned behavior through the versioned compatibility mechanism while already-selected issues continue without a new search; the preset definition/input digest covers the new setting and persisted catalog rows receive the updated built-in definition through seed/catalog synchronization. New author-scoped executions require a worker whose code identity includes this enforcement (existing stale-code detection); the checkbox must not ship ahead of enforcement, and rollout/rollback ordering (stop new scoped executions before rollback to an unsupported worker) is an authorized deployment operation. Search and cached results discover candidates only. Every candidate is revalidated through the shared lifecycle eligibility decision: Available means no MoonMind status label (ordinary labels do not block); a Recovery-needed issue is a continuation candidate only with sufficient stopped-writer, prior-work, retry, and policy evidence. In-progress, code-review, needs-attention, closed, mixed/unknown states, unresolved active comments, and unsafe or incomplete handoffs are excluded, and existing explicit blockers and prerequisite rules continue to apply. Safe recoveries within equivalent scope/priority are preferred under a bounded fair policy so one difficult issue cannot monopolize recurring runs or starve new work. The scan is bounded to five pages of 100 candidates, excludes pull requests, and records pages/candidates examined. Missing, malformed, incomplete, or exhausted evidence stops before implementation or issue mutation with distinct no-eligible-candidate, incomplete-evidence, cooldown/hold, permission, and GitHub-unavailable outcomes. The preset never silently widens the query, never treats a rate-limited scan as empty, never launches a fresh attempt to escape exhausted retry history, and inherits cross-device retry/cooldown/hold history.

Declared dependencies carry an explicit scope. **Blocked from starting** means no useful implementation can begin, so the candidate is not selected. **Blocked from completing** means implementation can proceed but final acceptance or closure waits. `Depends on`, `Blocked by`, `Integration prerequisites:`, `Child issues` / `Sub-issues` lists, and GitHub tracking-list children are start-blocking. `Completion depends on` is completion-blocking, because that is what the declaration says: an issue whose body states that its *completion* depends on other issues can still receive the independently useful work it also describes, while `github.check_issue_blockers` still reports `blocked` and names the open dependency. Open completion dependencies travel with the brief as `completionDependencies`. Existing declarations are migrated deliberately, one convention at a time, rather than reinterpreted in place; only a declaration whose own words name completion changed meaning.

Explicit `Depends on`, `Blocked by`, `Completion depends on`, and `Integration prerequisites:` sentences contribute dependency evidence. Short references, qualified references, issue URLs, and inclusive numeric ranges are resolved through the selected repository's authorized access, with at most 100 declared prerequisites per candidate. The scan shares a 100-request prerequisite-lookup budget and reuses validated repeated identities. Exhaustion requests an explicit issue or narrower search. Confirmation uses fresh prerequisite reads with its own 100-request budget and re-fetches current issue detail to reject a lifecycle state added between search and brief loading; the later implementation preflight also checks current state. There are no todo-based default transitions: the legacy `status: todo` value is retained history only, never required for availability and never added or removed by lifecycle transitions. Legacy reconciliation assessment and repair run through the existing `github_issue.assess_legacy` / `github_issue.plan_legacy_repair` activity bindings onto the same surviving lifecycle policy, and retained preset expansions follow the controlled drainage path; see [GitHub Issue Legacy Cutover](GitHubIssueLegacyCutover.md).

Child issue lists declare completion prerequisites through that same lookup, recognized two ways. A `Child issues` or `Sub-issues` heading declares every entry its section lists. Anywhere else, GitHub's own tracking-list rendering is the declaration: a checkbox entry whose leading content is an issue reference, such as `- [ ] #4366 Split reliability tests`, is a tracked child whatever heading sits above it. Epic bodies name that section differently every time — `Required implementation issues`, `Implementation children`, `Work breakdown`, `Implementation checklist` — so the rendered list is the convention and the parser never enumerates heading wording. A heading that names its section as context (`related`, `superseded`, `see also`, `non-goals`, `out of scope`) keeps its references contextual, and a checkbox entry that does not lead with an issue reference is ordinary prose, so completion-criteria and acceptance checklists declare nothing. Markdown section nesting and legal heading indentation apply: categorized children remain in scope until a heading of the same or higher level ends the section. Fenced code, indented code, and HTML-comment examples do not declare children. Bulleted, numbered, and checkbox entries may start with short or qualified issue references, issue URLs, or Markdown links to issues. A spaced dash starts the child title; spaced ranges require an explicit `#` endpoint, such as `#10 — #12`, so `#10 — 2FA support` cannot invent a range. GitHub issue state is authoritative regardless of a checked or unchecked box: an open child blocks the parent, closed children allow its remaining requirements to be assessed, and unverifiable child state stops admission. Parent links, sibling related-work sections, and contextual references after a child entry do not become dependencies. This prevents automatic selection of an unfinished parent epic while its implementation belongs to open child issues; explicit selection is protected by the same implementation preflight.

Open start-blocking prerequisites block admission; closed prerequisites do not, and open completion dependencies block completion instead of admission. Only the leading reference list after a declaration contributes dependencies, not subsequent parent/related prose. Unknown or unavailable evidence blocks. These checks do not edit labels or issue content.

The selected issue and brief travel through trusted context and a durable attachment. Downstream blocker/status tools use the same identity and reject conflicts. The journey composes candidate discovery, shared advisory admission, trusted brief/prior-work resolution, phase-appropriate assessment, implementation only when needed, existing verification/remediation/publication, and durable terminal handoff. The brief announces before expensive assessment and settles this attempt's own announcement/writers before any recandidation on a detected competitor. The resolved issue and predecessor survive replay/retry; recovery of an already selected issue never repeats search, never resets a continuation to the base branch, and never creates a second PR. Instructions do not perform dynamic binding or read agent-local files. Explicit resolved tool repository/issue inputs remain execution arguments, not competing user-level overrides. A pinned historical plan keeps its old search/selection behavior until newly admitted authoring selects the current definition.

The immutable initial assessment identifies apparently present implementation and bounded gaps. It does not prove objective acceptance or landing. Assessment, verification, remediation, and finalization use the resolved `moonspec-verify` [acceptance policy](../../.agents/skills/moonspec-verify/references/acceptance-policy.md). Objective evidence binds actual candidate content, original scope and requirement identities, freshness, and the configured completion target. Only current target evidence allows the no-new-work completion path; candidate-only work requires the existing PR path. A later `FULLY_IMPLEMENTED` verifier result approves the candidate for publication; a clean restored work branch or an already-pushed commit does not make that candidate a no-change outcome, and a partial PR never constitutes a fully-implemented verdict. The search preset declares the same `code-review-handoff` role as explicit issue implementation. Failed, blocked, canceled, and exhausted outcomes that never reach the success-path finalization step are owned by the durable failed-attempt boundary (the `github_issue` finalize_failed_attempt activity plus the periodic handoff reconciler), never by a success-ordered preset node, so they still produce a safe-release handoff. Before updating issue status, the workflow creates or adopts a missing PR from its accepted remote branch and carries the confirmed URL into the trusted status tool; a resolved existing PR continues from its validated current head and is updated rather than duplicated. Completed work routes to missing verification, review/merge follow-up, or status finalization under the admitted policy. The resolved issue identity supplies the PR closing reference and post-merge completion target.

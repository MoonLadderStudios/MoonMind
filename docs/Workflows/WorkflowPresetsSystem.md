...
`github-issue-search-and-implement` uses the workflow repository binding. Its typed `github.load_issue_preset_brief` operation accepts `issueSearch`: a nonempty query discovers candidates through GitHub's best open-issue matches and an empty query scans open issues in descending creation order for eligible candidates. Search and cached results discover candidates only. Every candidate is revalidated through the shared lifecycle eligibility decision: Available means no MoonMind status label (ordinary labels do not block); a Recovery-needed issue is a continuation candidate only with sufficient stopped-writer, prior-work, retry, and policy evidence. In-progress, code-review, needs-attention, closed, mixed/unknown states, unresolved active comments, and unsafe or incomplete handoffs are excluded, and existing explicit blockers and prerequisite rules continue to apply. Safe recoveries within equivalent scope/priority are preferred under a bounded fair policy so one difficult issue cannot monopolize recurring runs or starve new work. The scan is bounded to five pages of 100 candidates, excludes pull requests, and records pages/candidates examined. Missing, malformed, incomplete, or exhausted evidence stops before implementation or issue mutation with distinct no-eligible-candidate, incomplete-evidence, cooldown/hold, permission, and GitHub-unavailable outcomes. The preset never silently widens the query, never treats a rate-limited scan as empty, never launches a fresh attempt to escape exhausted retry history, and inherits cross-device retry/cooldown/hold history.

Author scope is a work-selection boundary, not proof that issue content is trustworthy. By default only issues created by the authenticated GitHub account performing the search are eligible (`include_all_authors: false` preset input, `includeAllAuthors: false` trusted-tool input). The unchecked **Include issues created by other users** checkbox, rendered immediately below the search field by the shared schema renderer, explicitly broadens author eligibility; it never bypasses repository binding, open-state, lifecycle, blocker, attempt, recovery, retry, verification, or publication policy. Authenticated account means the account bound to the exact credential used for the workflow's issue-search API requests (resolved once per selection via authenticated `GET /user` and compared by durable account ID, never a hardcoded username, session user, owner, assignee, commenter, commit author, or unrelated CLI login). A nonempty query is constrained with `author:<login>` (parenthesized when top-level `OR` is present) and a blank query scans with `creator=<login>`; returned candidates are independently validated by account ID before expensive checks or announcement, and the selected issue is revalidated against fresh detail before the trusted brief handoff. Authored `author:` qualifiers are backend-owned: absent qualifiers gain the resolved account automatically, `author:@me` or the same account normalizes without duplication, and another author, exclusion of the required author, or conflicting/ambiguous author expressions fail with a field-addressable search-field error requiring explicit opt-in. In all-author mode authored filters pass through unchanged. Identity failures, conflicting filters, invalid author evidence, and empty self-authored results stop fail-closed with distinct diagnostics and never widen scope or launch implementation. Evidence records `authorScope` (`authenticated_user` or `all`), the validated `authenticatedUser` id/login, the `selectedIssueAuthor`, and `authorMismatchesSkipped`, persisted with the trusted brief. Missing current inputs materialize `false`; saved true/false values survive reload, edit, rerun, scheduling, and dispatch; pre-change frozen schedules without a recorded scope choice stop with an actionable refresh-required status and resume after reapply; already-admitted executions and already-selected issues continue unchanged without a new search. Rollout follows drain-and-replace: new author-scoped executions run only on workers that enforce the scope, and rollback stops new executions first.

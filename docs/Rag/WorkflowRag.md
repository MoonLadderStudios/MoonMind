...
In the current Codex runtime strategy, `prepare_workspace()` invokes `ContextInjectionService` before the command is built. `ContextInjectionService` resolves retrieval, persists a context artifact under `artifacts/context/`, prepends the retrieved context to `instruction_ref`, and adds security framing that treats retrieved text as untrusted reference data.
...
bounded, security-framed `context_text` into the existing first user message. It
...
Runtime injection must continue to preserve a security boundary that tells the model to treat retrieved content as untrusted reference material and to prefer current repository state when retrieved content conflicts with the checked-out workspace.
...
- **filters**
  ...
  `job_id`, `tenant`, `tenant_id`, `agent_run_id`, and `agentRunId`.

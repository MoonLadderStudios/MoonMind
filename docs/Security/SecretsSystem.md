...
Effective high-security mode uses deterministic precedence with no policy downgrade:

1. an explicit `True` always opts in,
2. an operator-required `True` from a supplied app or security settings object is sticky and cannot be disabled by an explicit `False` from a workflow/message payload,
3. environment/config-derived security settings,
4. default `false`.

When no source requires enforcement the scan contract returns an allow result (disabled-mode passthrough).
...
The managed workspace publish path scans git pushes before invoking `git push`. It resolves the outbound range from the recorded remote branch SHA when available, otherwise from the publish base ref, and builds the scan bundle from commit metadata plus per-file diffs with bounded item sizes. Truncated or otherwise uninspectable required content (commit metadata, changed-file list, or per-file diff exceeding its bound) fails closed as enforcement unavailable — a truncated prefix is never reported as complete coverage. A blocked scan returns file/commit locations such as `git.push.diff:<path>` without printing the detected value. A scanner failure is distinct from a content block and also prevents the push, with an enforcement-unavailable reason naming only the exception class.

...
The profile host drain after acquisition is bounded (120s). A janitor drain
that never answers returns the same 503 shape with
...
unchanged and the retry reattaches to the same owner.
...
10 minutes with a retryable failure that keeps the same key.

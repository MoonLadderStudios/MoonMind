...
The feature is designed as a **toggle** — when enabled, the system automatically wraps every step in a review-retry loop without requiring any changes to the plan itself.
...
class ReviewGatePolicy:
    ...
    skip_tool_types: tuple[str, ...] = () # tool types to exempt (e.g., "agent_runtime")
```
...
for index, node in enumerate(ordered_nodes, start=1):
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if not gate_active or attempt == max_attempts:
            break  # No review, or last attempt — accept whatever we got

        # Run the review activity
        ...
        # FAIL — prepare feedback for retry
...
**Precedence**: Plan-level policy takes priority. If the plan explicitly sets `review_gate.enabled: false`, the workflow-level parameter cannot override it. If the plan omits `review_gate`, the workflow-level parameter applies.
...
`skip_tool_types` allows exempting certain tool types from review. Common exemptions:

- Infrastructure tools (artifact read/write)

...
> Note: We intentionally do **not** model “worker/orchestrator/manifest” as a taxonomy. A workflow is an orchestrator; activities are execution steps. Manifest ingest exists as a separate Workflow Type only because it has materially different orchestration behavior (graph + aggregation).

...
## Purpose
This document defines how backend routing is handled across the `queue`, `orchestrator`, and `temporal` execution sources.
...
## Migration & Rollout
1. Existing tasks maintain their current source (`queue`, `orchestrator`).
2. New tasks that trigger Temporal capabilities will run under the `temporal` source.
3. Partial enablement allows fallback to `queue` or `orchestrator` if Temporal services are unavailable or disabled by feature flags.

## Support & Debugging
Engineers can force the resolution using the explicit `?source=temporal` (or `queue`/`orchestrator`) query string parameter if the canonical resolver fails or during testing.

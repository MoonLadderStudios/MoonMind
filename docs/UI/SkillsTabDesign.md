...
### 3.1 Navigation
- A new top-level navigation pill "Skills" will be added to the `.route-nav` list, pointing to `/tasks/skills`. This follows the existing pattern where all dashboard navigation links are prefixed with `/tasks` (e.g., `/tasks/list`, `/tasks/new`).

### 3.2 Main Layout
- The `/tasks/skills` route will display a split-pane layout or a list-detail view.
...
- The backend handler for `POST /api/tasks/skills` will write the provided markdown to `.agents/skills/local/{name}/SKILL.md`.
...
- Add a new route handler for `/tasks/skills` that renders `api_service/templates/task_dashboard.html` (the SPA shell handles routing client-side, or a specific template context flag can be set).
- *Alternatively*, handle the `/tasks/skills` route entirely client-side within `dashboard.js`.
...
- **Fetch Skills:** Use the existing `GET /api/tasks/skills` endpoint (already implemented in `task_dashboard.py`) to populate the list.
- **Render Preview:** Use a lightweight Markdown-to-HTML converter or rely on the backend to pre-render the content.
- **Submit Form:** Attach a submit handler to the creation form that sends the JSON payload to `POST /api/tasks/skills`.

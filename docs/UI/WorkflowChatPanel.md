...
The dashboard update stream (`GET /api/workflows/updates/stream`, owned by the dashboard updates surface in `docs/UI/DashboardSPAArchitecture.md`) is a separate transport from the binding-scoped chat transcript/stream facade. A 404 on that dashboard route is diagnosed and owned at the dashboard surface; it is never treated as chat transcript evidence and never changes the binding facade allowlist.

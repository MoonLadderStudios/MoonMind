...
writes `runtime.mode = "omnigent"`, and the trusted API compiler alone converts
that selector to the canonical external-agent identity:
...
| --- | --- | --- | --- |
| runtime | `mode=omnigent` | `external/omnigent` + nested `codex-native` | API compiler |
...
prohibits. No stage may be reordered; Provider Profile capacity is released last,
after credential-consuming host cleanup.
...
```json
{
  "perRow": {
    ...
    "matrixRows": ["static_launch"],
    "evidenceKind": "submission-matrix",
    "observedResult": "pass",
    "artifactRef": "cutover-artifact:static_launch:01",
...
```json
{
  ...
  "effectivePhase": "opt_in",
  "directLaunch": "available",
...
An explicit Omnigent selection never silently runs through direct Codex, another Provider Profile, another host mode, an arbitrary static host, or a broader network/mount policy. This is the single reconciled matrix spanning all twelve
failure stages. The `OMNIGENT_*` codes for stages 1–10 are owned by
...
The docs distinguish exactly these states, and no weaker state is written as a
stronger one:
...
matrix whenever it changes.

## 7. Reconciliation and cutover state

At the time of this reconciliation the deployed cutover phase is **1 — opt_in**
and fail-closed: the repository does not contain the complete protected-live
acceptance artifact, so no row is promoted to **supported** on the strength of
code presence or hermetic tests alone. The normal product path is
**implemented foundation** with **repository-verifiable** and hermetic coverage;
its promotion to **supported** and to any later **default** phase remains gated on
the protected browser-to-host matrix produced by the Milestone 1 evidence issues.
This document is the declarative reconciliation for roadmap item **1.0**; the
live-evidence items **1.1**, **1.2**, and **1.3** stay open until their
independently resolvable protected artifacts are linked.

## 8. Acceptance invariants

- Every listed canonical document names the identical `external/omnigent` +
  nested `codex-native` identity.

...
4. **Turn-start budget.** One watchdog per execution attempt starts when the marked message is dispatched and is evaluated wherever the snapshot is observed (reattach, heartbeat reconciliation, terminal poll), so it does not depend on a terminal SSE frame arriving. While the boundary is visible, every observation has projected an inactive turn (no `running` status and no active response id), and no item follows the boundary, the wait is bounded at 300s; only an observation initiated after the deadline can fire it. Exceeding it fails with `OMNIGENT_CURRENT_TURN_NOT_STARTED` (`integration_error`, remediation `retry_step_execution`): the provider accepted the turn and dropped it, and no work exists to preserve. The failed attempt's runtime binding is cleaned and its canonical turn command settled, both immutably, so the retry is a new step execution attempt with a fresh idempotency key, never a same-key redispatch. Any active projection or any ordered progress disarms this budget for the rest of the attempt.
...
| Accepted turn never started (inactive, no active response, no item after marker, 300s) | `integration_error` / `OMNIGENT_CURRENT_TURN_NOT_STARTED` | No provider work exists; `retry_step_execution` as a new attempt. Returned as a typed result from the shared Activity boundary on every execution shape. See §9.5. |

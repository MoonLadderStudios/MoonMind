...
Heartbeats expose `currentTurnProgress`, `terminalEventToolOnlyCandidate`, `turnEverActive`, `turnCurrentlyActive`, `turnStartWaitSeconds`, `turnQuietSeconds`, and their budgets on every observation path (including heartbeat and idle-stream reconciliation, reported as `markedTurnObservationSource`) so an operator can tell a transient injection response, a slow turn, a same-session recovery candidate, and a dropped turn apart while the Activity is alive.

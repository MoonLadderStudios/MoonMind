...
- **served** — the reviewed HTTP, SSE, and WebSocket surface the scoped facade actively serves, single-sourced from the facade route allowlist so there is no generic fallback proxy;
- **compatibility_review_required** — a recognized route whose payload contract is unsafe or has not been reviewed. The generic arbitrary-command shell route is in this state; it fails closed with an explicit compatibility diagnostic while declared terminal creation remains available through its narrower contract.

Every WebSocket connect and reconnect authenticates, resolves the durable binding, authorizes the caller, rejects caller-supplied identity, recomputes capabilities, negotiates only an allowlisted subprotocol, and validates binding state **before upgrade**. Terminal attach, raw binary input, bounded resize controls, and close are independently gated; terminal creation accepts only a declared shell from the bound session agent and never forwards an arbitrary command. A successful earlier HTTP bootstrap never authorizes a later WebSocket.

Workspace paths are lexically validated and resolved against the server-owned bound workspace before forwarding, including symlink containment. Provider file ids are exposed only as encrypted binding-scoped aliases, and download dispositions are rebuilt from path-free bounded filenames. Unknown or changed transports fail closed with a non-enumerating diagnostic rather than being generically proxied, and a terminal/revoked binding cannot open a new live transport.

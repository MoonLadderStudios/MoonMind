...
Heartbeats expose `currentTurnProgress`, `terminalEventToolOnlyCandidate`, `turnEverActive`, `turnStartWaitSeconds`, `turnQuietSeconds`, and their budgets on every observation path (including heartbeat and idle-stream reconciliation, reported as `markedTurnObservationSource`) so an operator can tell a slow turn, a same-session recovery candidate, and a dropped turn apart while the Activity is alive.

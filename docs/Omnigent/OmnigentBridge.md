...
3. **Quiescence.** A structurally complete turn (no unmatched tool call) must stay unchanged for 60s after a final assistant text, or 300s when the turn ended on a tool result. The snapshot normally also projects an inactive turn. When the bridge observed a live post-dispatch terminal event, a still-running snapshot that contains marked progress and ends on a matched tool result is also terminal after the 300s quiet period; the profile-bound coordinator retains the same session and requests the missing final assistant result instead of discarding the authoritative workspace.
...
5. **No-progress budget.** A turn that showed activity or progress but never became structurally complete, or whose running projection lacks a live post-dispatch terminal event, fails after 1800s with `OMNIGENT_CURRENT_TURN_TERMINAL_AMBIGUOUS`; cleanup is deferred because provider work may still be running.

Heartbeats expose `currentTurnProgress`, `terminalEventToolOnlyCandidate`, `turnEverActive`, `turnStartWaitSeconds`, `turnQuietSeconds`, and their budgets on every observation path (including heartbeat and idle-stream reconciliation, reported as `markedTurnObservationSource`) so an operator can tell a slow turn from a dropped one while the Activity is alive.

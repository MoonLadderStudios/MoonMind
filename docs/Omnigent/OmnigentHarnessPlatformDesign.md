...
Work above the effective limit waits as durable workflow state with an explicit reason. It does not occupy a long-running execution Activity slot, silently fall back to another runtime, profile, model, or Host Class, or launch beyond the configured host count.
...
2. **Admit host capacity.** With provider capacity held, the workflow polls a short control Activity that evaluates the aggregate host ceiling and the cold-launch rate against the durable host-lease ledger, and it waits on a workflow timer between polls. The poll names the run's stable host-lease identity (execution plan, request idempotency key, Host Class), so whether this run already holds a reservation is read from the ledger rather than from a caller-supplied flag. A payload that cannot name its binding has no reservation to reuse: its own `alreadyAllocated` flag is not evidence and does not bypass accounting.
   ...
   **This poll reserves nothing.** It is advisory: it reports the limiting layer so the run waits on the right one. The durable host-lease row is inserted in the allocating transaction under the admission lock, so a race the poll admitted may still be refused there and returned to durable waiting.
3. **Allocate the host, then launch.** The allocating transaction counts hosts and inserts the durable host-lease row under the transaction-scoped admission lock, so two worker replicas cannot both observe the same free slot. The launch then produces ordinary Docker arguments from the resolved launch policy — explicit CPU, memory, process, shared-memory, and timeout ceilings with no pool preparation or verification.
4. **Release on failure.** If host capacity cannot be admitted, provider capacity is released immediately. Provisional ownership is bounded: a run never occupies a provider lease indefinitely while waiting for a host slot. A request larger than the configured ceiling is **rejected**, not queued: waiting for it would wait forever, and clamping it would silently change a billing-relevant resource value.
...
#### Fixed limits and container-job slots

Resource limits are explicit per-container ceilings enforced directly by
Docker. There is no machine-derived budget, no CPU pool, and no resource
ledger: a positive `cpuMillis` becomes an ordinary Docker CPU quota, and new
policies require positive CPU limits while historical zero-valued documents
remain decodable for replay. Bootstrap policies carry fixed stock limits (2
CPUs, 4 GiB) with no capability probe, and bootstrap reconciliation migrates
only bootstrap-owned shared-CPU defaults to the fixed successor — custom
limits and historical versions keep their authority.

Container-job concurrency is one deployment-owned maximum active-job setting
(`MOONMIND_CONTAINER_BACKEND_MAX_ACTIVE_JOBS`, default 1), admitted atomically
under the backend-scoped capacity lock against the daemon's owned containers.
The retired aggregate settings (`MOONMIND_MACHINE_*`,

...
2. [x] **OpenAPI → types path:** add a documented, repeatable command (e.g. npm script calling `openapi-typescript`) that writes `frontend/src/generated/openapi.ts` (or agreed path); gate drift with CI or a scheduled check. — `api:types` script added and generated output checked in.

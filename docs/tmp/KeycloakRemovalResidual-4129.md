...
| `BearerTransport`/`JWTStrategy`/`FastAPIUsers` validation in `api_service/auth.py`, `current_active_user` in `auth_providers.py`, `get_jwt_strategy` in `websockets.py` | Retained until the #4124-era session contracts replace bearer validation (K4). No issuance route remains; `fastapi-users`/`PyJWT` stay as dependencies because the `User` model/migrations couple to the former and managed-session/fanout/proxy capabilities use JWT-secret-backed tokens. **Superseded in part:** #4124 (generic OIDC + trusted proxy through the shared boundary) and #4125 (every user-facing API/WebSocket path through the qualified #4121 session authority; legacy FastAPI Users JWT acceptance removed from request and WebSocket paths) have landed — see the point-in-time ledger in [KeycloakRemovalStatus-4130.md](./KeycloakRemovalStatus-4130.md). |

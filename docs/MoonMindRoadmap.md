...
- [ ] **1.0 Declarative reconciliation** — update the Create-to-host, workspace, adapter, host OAuth, combined-stack validation, and managed/external execution docs to match the completed implementation and the exact remaining authority boundaries. The single normative end-to-end sequence, complete failure/evidence matrix, evidence-qualified support language, and universal `external/omnigent`+`codex-native` identity are now reconciled in [`docs/Omnigent/NormalCodexProductPathReconciliation.md`](Omnigent/NormalCodexProductPathReconciliation.md) (MoonLadderStudios/MoonMind#3565). This item stays open until the completed-implementation and protected-live evidence in **1.1**–**1.3** is linked, so the reconciliation truthfully names the remaining live gates rather than promoting them to supported.
...
- [x] **6.3 Historical and Temporal compatibility** — persisted `codex_direct_compat` sessions preserve truthful direct provenance and render through the shared Workflow Detail journal read model with no active direct runtime, and pre-/post-cutover recorded histories replay on one current worker while runtime selection stays a submission-boundary side effect. The authoritative evidence contract for these surfaces lives in [`docs/Omnigent/CodexSupportAndCutover.md`](./Omnigent/CodexSupportAndCutover.md).
...
- [ ] **6.5 Public documentation and release-metadata reconciliation** — the public Omnigent story is now reconciled to the accurate opt-in state (`README.md`, `docs/MoonMindArchitecture.md`, and the combined-stack docs no longer promise a dated completion or describe Omnigent-backed runtimes as future work, and point at the published support matrix). Remaining, and still gated on the live/product stages above: retire obsolete Omnigent documents and align release metadata with the published support matrix once the protected-live evidence for phases 2–6 exists.

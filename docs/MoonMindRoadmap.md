...
- Static and on-demand hosts use the published Omnigent host image selection, run as UID/GID `1000:1000` from `/home/app`, keep Omnigent registration credentials separate from provider OAuth, and retain explicit image/tag overrides.
...
**Why it matters:** Static Compose is now a reliable bootstrap path, and deterministic on-demand launch exists. The remaining work is to make managed launch a first-class product contract: policy-selected, capacity-aware, observable, recoverable, and aligned with MoonMind's shared Docker/workspace/evidence substrate.

### Remaining work

- [ ] **2.0 Declarative design reconciliation** — update `docs/Omnigent/OmnigentAdapter.md`, `docs/Omnigent/OmnigentHostOAuth.md`, `docs/Omnigent/CombinedStackValidationAndRollback.md`, and `docs/Temporal/ManagedAndExternalAgentExecutionModel.md` to describe the actual canonical Compose plus deterministic on-demand Docker model, durable host lease, registration/readiness contract, capacity layers, mount/network policy, and cleanup semantics.
- [ ] **2.1 First-class launch selection** — replace the environment-only `OMNIGENT_CODEX_HOST_LAUNCH_PROFILE` decision with an explicit policy/profile-controlled host mode that can select static bootstrap or on-demand launch without workflow JSON editing or manual `hostId` handling.
- [ ] **2.2 Shared Docker/workspace substrate convergence** — reuse canonical `WorkspaceLocator`, daemon-visible mount resolution, bounded evidence, log cursors, output manifests, diagnostics, and cleanup primitives from the Container Jobs/workload plane wherever they fit; keep the long-lived Omnigent host/session lease semantics explicit rather than forcing hosts into a one-shot job model.
- [ ] **2.3 Capacity and policy hierarchy** — combine Provider Profile capacity, host/session capacity, machine/Docker capacity, resource limits, and network policy without conflicting counters. `ProviderProfileManager` remains authoritative for the provider account.
- [ ] **2.4 Workspace, artifact, auth, and cache mounts** — standardize workflow-scoped repositories, temporary work areas, the profile-bound Codex OAuth volume, artifact handoff paths, optional caches, UID/GID, safe targets, and local-versus-daemon path translation.
- [ ] **2.5 Readiness projection** — expose container creation, state initialization, exact host registration/heartbeat, `codex-native` capability, OAuth preflight, bridge reachability, and session readiness as durable Workflow Detail stages rather than opaque activity logs.
- [ ] **2.6 Lifecycle controls and janitor hardening** — expose typed interrupt, stop, terminate, harvest, remove, stale-lease reconciliation, credential-generation drain, and orphan cleanup operations; prove idempotency across worker crashes and partially completed Docker operations.
- [ ] **2.7 Static-to-managed compatibility** — keep the canonical `omnigent-host-codex` Compose profile as the supported local/bootstrap mode while routing static and on-demand hosts through the same binding, bridge, readiness, diagnostics, and artifact contracts.
- [ ] **2.8 Direct Codex cutover gate** — keep direct Codex managed-session execution behind a compatibility feature gate until on-demand Omnigent launch, cancellation, evidence harvest, and cleanup meet the same reliability bar.
- [ ] **2.9 End-to-end Docker tests** — cover static and on-demand launch, registration, session start, image/tag overrides, `/home/app` workdir, worker restart, duplicate prevention, cancellation, terminal harvest, host cleanup, reconnect drain, profile cooldown, orphan reconciliation, and retry after partial failure.

**Done means:** a workflow can request a Codex Omnigent run without a pre-provisioned host; MoonMind acquires the selected profile, launches or reuses exactly one policy-bound host, observes registration and readiness through the bridge, runs and harvests one session, cleans the host up, and releases all leases idempotently.

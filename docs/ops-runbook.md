...
- **Queue setup**: Confirm the Codex worker is bound to both Workflow queues: `${CELERY_DEFAULT_QUEUE:-agentkit}` for discovery and `${WORKFLOW_CODEX_QUEUE:-codex}` for Codex phases.
- **Codex auth volume**: Keep `CODEX_VOLUME_NAME`/`CODEX_VOLUME_PATH` aligned with the worker defaults (`codex_auth_volume` mounted at `/home/app/.codex`) and authenticate once using `./tools/auth-codex-volume.sh` before launching production runs.
...
- **Artifact locations**: Patches, JSONL logs, and GitHub API responses are stored under `var/artifacts/workflows/<run_id>/`. Mount this directory when running the worker locally to inspect failures. The retry endpoint reuses these artifacts when resuming failed publish tasks.

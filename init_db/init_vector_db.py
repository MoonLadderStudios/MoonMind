...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"CRITICAL: Failed to build embedding model: {e}", exc_info=True
            )
            sys.exit(1)

        logger.info(
            f"Initializing Qdrant client for host: {settings.qdrant.qdrant_host}, port: {settings.qdrant.qdrant_port}"
        )
        try:
            ...
            logger.info(
                f"Using dynamic embeddings dimensions for Qdrant: {embed_dimensions}"
            )
            ...
            try:
                ...
                if "404" in str(e):
                    ...
                    any_actual_indexing_performed_or_attempted = (
                        True  # Collection creation counts as an action
                    )
                    logger.info(
            ...
            logger.error(
                f"CRITICAL: Failed to initialize Qdrant client or VectorStore: {e}",
                exc_info=True,
            )
            sys.exit(1)
        ...
        try:
            ...
            logger.error(
                f"CRITICAL: Failed to initialize StorageContext: {e}", exc_info=True
            )
            sys.exit(1)
        ...
        if not settings.atlassian.confluence.confluence_enabled:
            logger.info(
                "Confluence indexing is disabled via settings (ATLASSIAN_CONFLUENCE_ENABLED=false). Skipping."
            )
        else:
            ...
            if missing_confluence_settings:
                logger.error(
                    f"Confluence indexing will be skipped due to missing critical settings: {', '.join(missing_confluence_settings)}."
                )
            else:
                logger.info("All required Confluence configurations are present.")
                process_confluence_data = True
                confluence_skipped = (
                    False  # Tentatively, as space keys still need parsing
                )

        if process_confluence_data:
            any_actual_indexing_performed_or_attempted = (
                True  # Mark that we are attempting Confluence
            )
            space_keys_str = settings.atlassian.confluence.confluence_space_keys
            space_keys = [
                key.strip() for key in space_keys_str.split(",") if key.strip()
            ]

            if not space_keys:
                logger.error(
                    "No valid Confluence space keys found after parsing ATLASSIAN_CONFLUENCE_SPACE_KEYS. Skipping Confluence indexing."
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Starting to process {len(space_keys)} Confluence space(s)."
                    )
                    for space_key in space_keys:
                        ...
                        try:
                            ...
                            if (
                                ...
                                and "total_nodes_indexed" in index_result
                            ):
                                total_nodes_indexed = index_result.get(
                                    "total_nodes_indexed", 0
                                )
                            logger.info(
                                f"Successfully processed space {space_key}. Nodes indexed: {total_nodes_indexed}."
                            )
                        except Exception as e:
                            logger.error(
                                f"Error indexing space {space_key}: {e}", exc_info=True
                            )
                    ...
                    logger.error(
                        f"Failed to initialize ConfluenceIndexer or during space processing setup: {e}",
                        exc_info=True,
                    )
                    confluence_skipped = (
                        True  # Consider it skipped if the indexer itself fails to init
                    )
        ...
        if not settings.github.github_enabled:
            logger.info(
                "GitHub integration is not enabled via settings. Skipping GitHub indexing."
            )
            ...
            logger.info(
                "No GitHub repositories configured in GITHUB_REPOS. Skipping GitHub indexing."
            )
        else:
            repos_str = settings.github.github_repos
            github_repo_list = [
                repo.strip() for repo in repos_str.split(",") if repo.strip()
            ]
            if not github_repo_list:
                logger.info(
                    "No valid GitHub repository names found after parsing GITHUB_REPOS. Skipping GitHub indexing."
                )
            else:
                any_actual_indexing_performed_or_attempted = (
                    True  # Mark that we are attempting GitHub
                )
                ...
                try:
                    github_indexer = GitHubIndexer(
                        github_token=settings.github.github_token, logger=logger
                    )
                    logger.info("GitHubIndexer initialized.")

                    for repo_full_name in github_repo_list:
                        logger.info(
                            f"Processing GitHub repository: {repo_full_name} (using default branch)"
                        )
                        try:
                            ...
                            if (
                                isinstance(index_result, dict)
                                and "total_nodes_indexed" in index_result
                            ):
                                nodes_indexed_count = index_result.get(
                                    "total_nodes_indexed", 0
                                )
                            logger.info(
                                f"Successfully indexed {nodes_indexed_count} nodes from GitHub repository {repo_full_name}."
                            )
                        except Exception as e:
                            logger.error(
                                f"Error indexing GitHub repository {repo_full_name}: {e}",
                                exc_info=True,
                            )
                            # github_skipped remains False as an attempt was made for at least one repo
                    logger.info(
                        "Finished processing all configured GitHub repositories."
                    )
                except Exception as e:
                    logger.error(
                        f"Error initializing GitHubIndexer or during repo processing setup: {e}",
                        exc_info=True,
                    )
        ...
        if not settings.google_drive.google_drive_enabled:
            logger.info(
                "Google Drive integration is not enabled via settings. Skipping Google Drive indexing."
            )
        elif not settings.google_drive.google_application_credentials:
            logger.error(
                "GOOGLE_APPLICATION_CREDENTIALS is not configured. Skipping Google Drive indexing."
            )
        elif not settings.google_drive.google_drive_folder_id:
            logger.error(
                "GOOGLE_DRIVE_FOLDER_ID is not configured. Skipping Google Drive indexing."
            )
        else:
            folder_ids_str = settings.google_drive.google_drive_folder_id
            folder_ids = [
                fid.strip() for fid in folder_ids_str.split(",") if fid.strip()
            ]
            if not folder_ids:
                logger.error(
                    "No valid Google Drive folder IDs found after parsing GOOGLE_DRIVE_FOLDER_ID. Skipping Google Drive indexing."
                )
            else:
                any_actual_indexing_performed_or_attempted = (
                    True  # Mark that we are attempting Google Drive
                )
                    ...
                    for folder_id in folder_ids:
                        ...
                        try:
                            index_result = google_drive_indexer.index(
                                folder_id=folder_id, storage_context=storage_context
                            )
                            nodes_indexed_count = 0
                            if (
                                isinstance(index_result, dict)
                            ...
                            logger.info(
                                f"Successfully indexed {nodes_indexed_count} nodes from Google Drive folder {folder_id}."
                            )
                        except Exception as e:
                            logger.error(
                                f"Error indexing Google Drive folder {folder_id}: {e}",
                                exc_info=True,
                            )
                            # google_drive_skipped remains False as an attempt was made
                    logger.info(
                        "Finished processing all configured Google Drive folders."
                    )
                except Exception as e:
                    logger.error(
                        f"Error initializing GoogleDriveIndexer or during folder processing setup: {e}",
                        exc_info=True,
                    )
        ...
        if not settings.atlassian.jira.jira_enabled:
            logger.info(
                "Jira integration is not enabled via settings. Skipping Jira indexing."
            )
        elif not settings.atlassian.atlassian_url:
            logger.warning(
                "ATLASSIAN_URL (for Jira) is not configured. Skipping Jira indexing."
            )
        elif not settings.atlassian.atlassian_username:
            logger.warning(
                "ATLASSIAN_USERNAME (for Jira) is not configured. Skipping Jira indexing."
            )
        elif not settings.atlassian.atlassian_api_key:
            logger.warning(
                "ATLASSIAN_API_KEY (for Jira) is not configured. Skipping Jira indexing."
            )
        elif not settings.atlassian.jira.jira_jql_query:
            logger.warning(
                "ATLASSIAN_JIRA_JQL_QUERY is not configured. Skipping Jira indexing."
            )
        else:
            any_actual_indexing_performed_or_attempted = (
                True  # Mark that we are attempting Jira
            )
            jira_skipped = False  # Tentatively false
            logger.info(
                f"Found Jira JQL query to process: {settings.atlassian.jira.jira_jql_query}"
            )
            ...
            try:
                ...
                logger.info(
                    f"Processing Jira query: {settings.atlassian.jira.jira_jql_query}"
                )
                ...
                logger.info(
                    f"Successfully indexed {nodes_indexed_count} nodes from Jira for query: {settings.atlassian.jira.jira_jql_query}."
                )
            except Exception as e:
                logger.error(
                    f"Error indexing Jira query {settings.atlassian.jira.jira_jql_query}: {e}",
                    exc_info=True,
                )
                jira_skipped = (
                    True  # Mark as skipped due to error during indexing or init
                )
        ...
        if not settings.local_data.local_data_path:
            logger.info(
                "LocalData path is not configured in settings (via LocalData env var). Skipping Local Data indexing."
            )
        elif not os.path.exists(local_data_mount_path) or not os.listdir(
            local_data_mount_path
        ):
            logger.warning(
            ...
            any_actual_indexing_performed_or_attempted = (
                True  # Mark that we are attempting Local Data
            )
            local_data_skipped = False  # Tentatively false
            logger.info(
                f"Local data path configured to: {settings.local_data.local_data_path}. Indexing from container path: {local_data_mount_path}"
            )
            ...
            try:
                ...
                try:
                    index_result = local_data_indexer.index(
                        storage_context=storage_context, service_context=Settings
                    )
                    nodes_indexed_count = 0
                    if (
                        isinstance(index_result, dict)
                        ...
                        nodes_indexed_count = index_result.get("total_nodes_indexed", 0)
                    logger.info(
                        f"Successfully indexed {nodes_indexed_count} nodes from local data directory {local_data_mount_path}."
                    )
                except Exception as e:
                    logger.error(
                        f"Error indexing local data directory {local_data_mount_path}: {e}",
                        exc_info=True,
                    )
        ...
        if not any_actual_indexing_performed_or_attempted:
            if (
                confluence_skipped
                and github_skipped
                and google_drive_skipped
                and jira_skipped
                and local_data_skipped
            ):
                logger.info(
                    "No data sources were enabled or configured correctly for indexing. Exiting."
                )
        ...
        logger.info(
            f"Data sources for which processing was attempted: {processed_sources_summary}."
        )
        ...
        if skipped_sources_summary:
            logger.info(
                f"Data sources that were skipped: {', '.join(skipped_sources_summary)} (due to being disabled, misconfigured, or errors during initialization/processing)."
            )
        else:
            if any_actual_indexing_performed_or_attempted:
                logger.info(
                    "All configured and enabled data sources were processed (or attempted)."
                )
            # If no indexing was attempted and no sources were skipped, it implies an issue or no sources configured.

    except Exception as e:
        logger.error(
            f"CRITICAL: An unexpected error occurred during vector database initialization: {e}",
            exc_info=True,
        )

...
def test_docker_compose_has_temporal_worker_auto_start_configured():
    ...
    assert compose_path.exists(), "docker-compose.yaml must exist at the repository root"
    ...
    for fleet in worker_fleets:
        assert fleet in services, f"Worker fleet '{fleet}' is missing from docker-compose.yaml"
        ...
        assert "profiles" not in service_config, f"Worker fleet '{fleet}' should not have profiles so it starts by default"
        
        # Verify the command is polling, not sleep
        env_vars = service_config.get("environment", [])
        command_var = next((env for env in env_vars if env.startswith("TEMPORAL_WORKER_COMMAND=")), "")
        assert "sleep" not in command_var, f"Worker '{fleet}' must not be configured to sleep. Found: {command_var}"

"""Integration tests for the MM-667 → "In Progress" run-time orchestration.

The "integration" tier here exercises the orchestration driver
(`transition_mm667_to_in_progress`) end-to-end against a stubbed trusted Jira
tool surface that conforms to `contracts/transition-mm667.md`. Every scenario
...
def test_scn_001_transition_path_emits_one_transition_call_and_verifies_final_status() -> None:
    ...
    # Calls 1→4 from contracts/transition-mm667.md.
    ...
    # Run-report contract.
    assert report["issueKey"] == MM667_ISSUE_KEY
...
def test_edge_tool_unavailable_emits_no_calls() -> None:
    outcome = asyncio.run(transition_mm667_to_in_progress(trusted_jira=None))
    report = outcome.to_dict()
...
def test_edge_tool_unavailable_via_explicit_signal() -> None:
    ...
    outcome = asyncio.run(transition_mm667_to_in_progress(trusted_jira=_DisabledStub()))
    report = outcome.to_dict()
    assert report["outcome"] == "stopped:tool_unavailable"
...
def test_single_issue_invariant_holds_across_all_scenarios() -> None:
    """Every Jira tool call across SCN-001..004 + edges must use issue_key="MM-667"
    ...
    for tag, stub in scenarios:
        asyncio.run(transition_mm667_to_in_progress(trusted_jira=stub))
        for tool_name, kwargs in stub.calls:
            assert kwargs.get("issue_key") == MM667_ISSUE_KEY, (
                f"{tag}/{tool_name} called with non-MM-667 issue_key: {kwargs!r}"
            )

    ...
    class _FakeRepo:
        ...
        async def list_runs(
            ...
            *,
            ...
            del status, feature_key, created_by, limit, with_relations
            return [self.run]
...
async def test_workflow_task_listing_contract(monkeypatch):
    ...
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.get(f"/api/workflows/speckit/runs/{run_id}/tasks")
...
async def test_workflow_artifact_listing_contract(monkeypatch):
    ...
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.get(f"/api/workflows/speckit/runs/{run_id}/artifacts")
        assert response.status_code == 200
        payload = response.json()
        assert payload["runId"] == str(run_id)

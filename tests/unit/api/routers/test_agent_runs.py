"""Unit tests for agent-run observability API router."""
...
from api_service.api.routers.agent_runs import router
...
def _encoded_agent_run_path_id(agent_run_id: str) -> str:
    return quote(agent_run_id, safe="")
...
def test_agent_run_observability_endpoints_reject_invalid_agent_run_id(
...
def test_get_observability_summary_returns_404_when_missing(
    ...
    test_client, _ = client
    with patch("api_service.api.routers.agent_runs.ManagedRunStore.load", return_value=None):
...
def test_get_observability_summary_emits_latency_metric_for_404(
    ...
    with patch("api_service.api.routers.agent_runs.ManagedRunStore.load", return_value=None):
        with patch("api_service.api.routers.agent_runs.get_metrics_emitter", return_value=metrics):
            response = test_client.get(f"/api/agent-runs/{uuid4()}/observability-summary")
...
def test_get_observability_summary_accepts_moonmind_agent_run_id(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
    ...
    mock_record = MagicMock()
    mock_record.model_dump.return_value = {"runId": agent_run_id, "status": "running"}
    mock_record.status = "running"
    mock_record.live_stream_capable = True
    ...
    assert response.status_code == 200
    load_record.assert_called_once_with(agent_run_id)
    assert response.json()["summary"]["runId"] == agent_run_id
...
def test_observability_summary_allows_parent_workflow_owner() -> None:
    ...
    agent_run_id = f"mm:{uuid4()}"
    child_workflow_id = (
        f"{agent_run_id}:agent:tpl:speckit-orchestrate:1.0.0:12:faa43a52"
    )
    app = FastAPI()
    app.include_router(router, prefix="/api")
    app.dependency_overrides[get_current_user()] = lambda: SimpleNamespace(
    ...
    with TestClient(app) as test_client:
        with patch(
            ...
            with patch(
                "api_service.api.routers.agent_runs._load_exact_execution_owner_binding",
                new=exact_lookup,
            ):
                response = test_client.get(
                    f"{_agent_run_api_path(agent_run_id)}/observability-summary"
...
def test_stream_agent_run_log_returns_400_for_invalid_stream(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
...
def test_stream_agent_run_log_returns_404_when_record_missing(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
    with patch("api_service.api.routers.agent_runs.ManagedRunStore.load", return_value=None):
...
def test_stream_agent_run_log_returns_404_when_artifact_ref_missing(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
    mock_record = MagicMock()
...
def test_stream_agent_run_log_returns_404_when_file_missing(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
    mock_record = MagicMock()
...
def test_stream_agent_run_log_returns_file_response(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
    mock_record = MagicMock()
    ...
    with patch("api_service.api.routers.agent_runs.ManagedRunStore.load", return_value=mock_record):
        with patch("pathlib.Path.is_file", return_value=True):
            with patch("pathlib.Path.is_relative_to", return_value=True):
                with patch("api_service.api.routers.agent_runs.FileResponse") as mock_file_response:
...
def test_file_backed_agent_run_endpoints_accept_moonmind_agent_run_id(
    client: tuple[TestClient, AsyncMock],
    suffix: str,
    ...
    with patch(
        ...
        with patch("pathlib.Path.is_file", return_value=True):
            with patch("pathlib.Path.is_relative_to", return_value=True):
                with patch(
                    "api_service.api.routers.agent_runs.FileResponse"
                    ...
                    response = test_client.get(
                        f"{_agent_run_api_path(agent_run_id)}{suffix}"
                    )
...
def test_stream_agent_run_log_uses_supported_artifact_root_layouts(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
...
def test_stream_agent_run_log_merged_synthesized_from_spool(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
...
def test_stream_agent_run_log_merged_prefers_event_journal_before_prebuilt_artifact(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_stream_agent_run_log_merged_falls_back_to_spool_when_event_journal_empty(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_stream_agent_run_log_merged_skips_malformed_active_rows(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_stream_agent_run_log_merged_reads_event_journal_once_and_sorts_by_sequence(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
    ...
    with patch("api_service.api.routers.agent_runs.ManagedRunStore.load", return_value=record):
        with patch(
            "api_service.api.routers.agent_runs._get_agent_runtime_artifacts_root",
            return_value=str(artifacts_root),
        ):
            with patch(
                "api_service.api.routers.agent_runs._iter_event_journal",
...
def test_stream_agent_run_log_merged_falls_back_when_spool_metadata_missing(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_stream_agent_run_log_merged_fallback_includes_system_annotations(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_stream_agent_run_log_merged_falls_back_to_legacy_log_artifact(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_stream_agent_run_log_merged_returns_404_when_both_artifacts_absent(
    client: tuple[TestClient, AsyncMock],
) -> None:
...
def test_stream_agent_run_log_merged_uses_spool_when_stdout_stderr_refs_are_absent(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_stream_agent_run_log_merged_filters_stale_spool_entries_from_previous_runs(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_stream_agent_run_log_merged_uses_prebuilt_artifact_when_available(
    client: tuple[TestClient, AsyncMock],
) -> None:
...
def test_get_agent_run_diagnostics_returns_404_when_record_missing(
    client: tuple[TestClient, AsyncMock],
) -> None:
...
def test_get_agent_run_diagnostics_returns_404_when_diagnostics_ref_missing(
    client: tuple[TestClient, AsyncMock],
) -> None:
...
def test_get_agent_run_diagnostics_returns_404_when_file_missing(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
    mock_record = MagicMock()
...
def test_get_agent_run_diagnostics_returns_file_response(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
    mock_record = MagicMock()
...
def test_get_agent_run_diagnostics_uses_supported_artifact_root_layouts(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
    monkeypatch: pytest.MonkeyPatch,
    env_root_name: str,
...
def test_get_agent_run_observability_events_reads_structured_spool_history(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_get_agent_run_observability_events_accepts_moonmind_agent_run_id(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
    agent_run_id = f"mm:{uuid4()}"
    ...
    with patch(
        ...
        with patch(
            ...
            with patch(
                ...
                response = test_client.get(
                    f"{_agent_run_api_path(agent_run_id)}/observability/events"
                )
...
def test_get_agent_run_observability_events_filters_spool_fallback_rows(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_get_agent_run_observability_events_prefers_persisted_event_artifact(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_get_agent_run_observability_events_applies_session_epoch_and_thread_filters(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_get_agent_run_observability_events_rejects_invalid_session_epoch_filter(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client

    with patch("api_service.api.routers.agent_runs.ManagedRunStore.load") as load_record:
        response = test_client.get(f"/api/agent-runs/{uuid4()}/observability/events?sessionEpoch=0")

    assert response.status_code == 422
    load_record.assert_not_called()

def test_get_agent_run_observability_events_rejects_blank_thread_id_filter(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
...
def test_observability_event_session_epoch_filter_coerces_string_rows() -> None:
    ...
    filtered = agent_runs_router._filter_observability_events(
        events,
        session_epochs={2},
        thread_ids={"thread-2"},
    )
...
def test_get_agent_run_observability_events_limits_to_oldest_matching_rows_after_since(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_get_agent_run_observability_events_keeps_artifact_fallback_rows_when_since_is_present(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_get_agent_run_observability_events_uses_record_snapshot_when_session_record_missing(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_get_agent_run_observability_events_emits_history_metrics_for_journal(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_get_agent_run_observability_events_emits_error_metric_on_history_failure(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
    mock_record = MagicMock()
...
def test_get_agent_run_observability_events_emits_error_metric_when_session_record_load_fails(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
    mock_record = MagicMock()
...
def test_get_agent_run_observability_events_ignores_metrics_emitter_failures(
    client: tuple[TestClient, AsyncMock],
    tmp_path,
) -> None:
    test_client, _ = client
...
def test_stream_agent_run_live_logs_serializes_canonical_event_aliases(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
...
def test_stream_agent_run_live_logs_accepts_moonmind_agent_run_id(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
    agent_run_id = f"mm:{uuid4()}"
    ...
    with patch(
        ...
        response = test_client.get(f"{_agent_run_api_path(agent_run_id)}/logs/stream")

    assert response.status_code == 400
    load_record.assert_called_once_with(agent_run_id)
    assert response.json()["detail"] == "Live streaming is not supported for this run."

def test_stream_agent_run_live_logs_returns_gone_for_terminal_run(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
...
def test_stream_agent_run_live_logs_ignores_metrics_emitter_failures(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _ = client
...
def test_get_agent_run_observability_events_allows_owner_access() -> None:
    owner_id = uuid4()
    app = FastAPI()
    app.include_router(router, prefix="/api")
    app.dependency_overrides[get_current_user()] = lambda: SimpleNamespace(
...
def test_load_agent_run_session_record_uses_targeted_standard_paths(
    ...
    with patch("pathlib.Path.rglob", side_effect=AssertionError("unexpected recursive scan")):
        record = agent_runs_router._load_agent_run_session_record("wf-task-1")
...
def test_iter_diagnostics_observability_events_dedupes_system_annotations(tmp_path) -> None:
    ...
    events = list(agent_runs_router._iter_diagnostics_observability_events(diagnostics_path))
...
def test_event_sort_key_uses_timestamp_before_sequence() -> None:
    ...
    ordered = sorted(
        ...
        key=agent_runs_router._event_sort_key,
    )
...
def test_iter_historical_artifact_events_chunks_large_logs_and_keeps_tail(
    ...
    events = list(
        agent_runs_router._iter_historical_artifact_events(
            record,
            None,
            limit_per_stream=2,
        )
...
def test_get_agent_run_artifact_session_projection_reads_durable_state_only(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, artifact_service = client
...
def test_get_agent_run_artifact_session_projection_returns_404_when_missing(
    ...
    with patch("api_service.api.routers.agent_runs.ManagedSessionStore.load", return_value=None):
        response = test_client.get(
            "/api/agent-runs/wf-task-1/artifact-sessions/sess:wf-task-1:codex_cli"
        )
...
def test_get_agent_run_artifact_session_projection_returns_404_for_invalid_session_id(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _artifact_service = client
...
def test_get_agent_run_artifact_session_projection_returns_404_for_task_mismatch(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, _artifact_service = client
...
def test_get_agent_run_artifact_session_projection_allows_owner_access() -> None:
    owner_id = uuid4()
    app = FastAPI()
    app.include_router(router, prefix="/api")
    artifact_service = AsyncMock()
...
def test_post_agent_run_artifact_session_control_routes_clear_session_and_returns_projection(
    client: tuple[TestClient, AsyncMock],
) -> None:
    test_client, artifact_service = client

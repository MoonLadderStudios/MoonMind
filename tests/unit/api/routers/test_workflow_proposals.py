...
def test_create_proposal_accepts_workflow_origin(
    ...
    test_client, service, _execution_service = client
    proposal = _build_proposal()
    proposal.origin_source = WorkflowProposalOriginSource.WORKFLOW
    service.create_proposal.return_value = proposal
    ...
    assert response.status_code == 201
    kwargs = service.create_proposal.await_args.kwargs
    assert kwargs["origin_source"] == WorkflowProposalOriginSource.WORKFLOW
    payload = response.json()
...
def test_promote_proposal_rejects_invalid_state(
    ...
    test_client, service, _execution_service = client
    proposal = _build_proposal()
    service.promote_proposal.side_effect = WorkflowProposalStatusError("invalid state")
...
def test_sync_proposal_delivery_endpoint_returns_validation_errors(
    ...
    test_client, service, _execution_service = client
    proposal = _build_proposal()
    service.sync_proposal_delivery.side_effect = WorkflowProposalValidationError(

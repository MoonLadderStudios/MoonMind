...
@pytest.mark.asyncio
async def test_describe_execution_syncs_from_temporal(service, mock_session, mock_client_adapter):
...
@pytest.mark.asyncio
async def test_list_executions_sourced_from_temporal(service, mock_session, mock_client_adapter):
...
@pytest.mark.asyncio
async def test_cancel_action_routes_to_temporal(service, mock_session, mock_client_adapter):
    ...
    await service.cancel_execution(workflow_id="mm:123", reason="testing", graceful=True)
    
    mock_client_adapter.cancel_workflow.assert_called_once_with("mm:123")

@pytest.mark.asyncio
async def test_action_validation_relies_on_temporal(service, mock_session, mock_client_adapter):
    ...
    mock_client_adapter.signal_workflow.side_effect = MockTemporalError("Temporal workflow is already closed")
    ...
    assert "Temporal signal failed: Temporal workflow is already closed" in str(exc.value)

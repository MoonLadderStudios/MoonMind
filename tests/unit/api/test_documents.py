...
def mock_dependencies():
    with (
        patch(
            "api_service.api.routers.documents.get_storage_context"
        ) as mock_storage_context,
        patch(
            "api_service.api.routers.documents.get_service_context"
...
def test_load_confluence_documents_by_page_id(
    mock_confluence_indexer, mock_dependencies
):
    ...
    assert (
        "Successfully loaded 5 nodes from page ID '12345'." in json_response["message"]
    )
...
def test_load_confluence_documents_by_space_and_title(
    ...
    assert json_response["status"] == "success"
    assert (
        "Successfully loaded 5 nodes from page title 'My Page' in space 'SPACE'."
...
def test_load_confluence_documents_by_cql_query(
    ...
    response = client.post(
        "/v1/documents/confluence/load", json={"cql_query": "label = 'test-label'"}
    )
    assert response.status_code == 200
    json_response = response.json()
    assert json_response["status"] == "success"
    assert (
        "Successfully loaded 5 nodes from Confluence using CQL query: 'label = 'test-label''."
        in json_response["message"]
    )
    assert json_response["total_nodes_indexed"] == 5
...
def test_load_confluence_documents_by_space_key(
    ...
    assert json_response["status"] == "success"
    assert (
        "Successfully loaded 5 nodes from Confluence space 'BIGSPACE'."
        in json_response["message"]
    )
    assert json_response["total_nodes_indexed"] == 5

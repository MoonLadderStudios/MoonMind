...
def client():
    ...
    with patch("api_service.main.build_embed_model") as MockBuildEmbedModel:
        ...
        mock_embed_instance._get_query_embedding = MagicMock(
        ...
        mock_embed_instance.get_text_embedding = MagicMock(
            return_value=[0.1] * 100
        )  # Alias for query embedding often
...
def mock_qdrant_client_autouse():
    with patch("moonmind.factories.vector_store_factory.QdrantClient") as mock_qdrant:
        ...
        mock_collection_info.config.params.vectors.size = (
            100  # Match dimension from client fixture
        )
        mock_collection_info.config.params.vectors.distance = (
            Distance.COSINE
        )  # Match expected distance

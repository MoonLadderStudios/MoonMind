...
@pytest.mark.asyncio
async def test_default_drift_recovery_retries_once_after_catalog_sync(
    monkeypatch: pytest.MonkeyPatch,
):
    ...
    async def _flaky_resolve(session, *, selection, consumer_type, consumer_id, user):
        calls["resolve"] += 1
        if calls["resolve"] == 1:
            raise HTTPException(
                409,
                "stable upstream identity is unavailable (profile team-codex@2)",
            )
    ...
    snapshot = await resolve_default_agent_profile_snapshot(
        ...
        consumer_type="workflow",
        consumer_id="drift-recovery-1",
        user=SimpleNamespace(id=uuid4()),
    )

    assert snapshot["version"] == 2
    assert calls == {"resolve": 2, "sync": 1}
...
async def test_default_drift_recovery_stops_after_second_409(
    ...
    async def _always_409(session, *, selection, consumer_type, consumer_id, user):
        raise HTTPException(409, "stable upstream identity is unavailable")

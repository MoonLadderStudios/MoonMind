...
async def test_stream_writes_file(streamer, tmp_path):
    ...
    ref, content = await log_streamer.stream_to_artifact(        reader, run_id="run-1", stream_name="stdout"
    )
...
async def test_stream_empty(streamer, tmp_path):
    ...
    ref, content = await log_streamer.stream_to_artifact(        reader, run_id="run-2", stream_name="stderr"
    )

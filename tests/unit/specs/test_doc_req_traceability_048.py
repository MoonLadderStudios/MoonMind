"""DOC-REQ traceability gate for the Jules external events feature."""
...
def test_jules_external_events_doc_req_traceability_contract() -> None:
    spec_text = _FEATURE_SPEC.read_text(encoding="utf-8")
    doc_req_ids = {
        f"DOC-REQ-{match.group(1)}" for match in _DOC_REQ_PATTERN.finditer(spec_text)
    }
    assert doc_req_ids, "Expected DOC-REQ entries in 048 spec.md"

...
class TestGitHubIndexer(unittest.TestCase):
    def setUp(self):
        ...
        self.indexer = GitHubIndexer(
            github_token="dummy_test_token_for_indexer", logger=MagicMock()
        )
        # This one is for tests that specifically want to test the token override mechanism or a different token.
        self.indexer_with_token = GitHubIndexer(
            github_token="test_token_override", logger=MagicMock()
        )
    ...
    @patch("llama_index.core.VectorStoreIndex.from_documents")
    def test_index_success_private_repo_with_token(
        self, mock_vs_from_docs, MockGithubReader
    ):
        mock_vs_from_docs.return_value = MagicMock()
        mock_reader_instance = MockGithubReader.return_value
    ...
    def test_index_with_filter_extensions(self, mock_vs_from_docs, MockGithubReader):
        ...
        mock_reader_instance.load_data.return_value = [
            Document(text="Filtered doc")
        ]  # Assume one doc matches
    ...
    def test_index_reader_load_data_raises_exception(self, MockGithubReader):
        ...
        mock_reader_instance.load_data.side_effect = Exception(
            "Git clone error or repo not found"
        )
        ...
        self.assertIn(
            "Failed to load repository content", str(context.exception.detail)
        )
        self.assertIn(
            "Git clone error or repo not found", str(context.exception.detail)
        )
    ...
    def test_index_no_documents_found(self, MockGithubReader):
        ...
        with patch(
            "moonmind.indexers.github_indexer.VectorStoreIndex.from_documents"

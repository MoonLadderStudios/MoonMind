...
from moonmind.workflows.orchestrator.storage import ArtifactPathError, resolve_artifact_root


def test_resolve_artifact_root_uses_base_when_override_is_base_path(monkeypatch, tmp_path):
    monkeypatch.chdir(tmp_path)
    assert resolve_artifact_root("artifacts", "artifacts") == (
        tmp_path / "artifacts"
    ).resolve()

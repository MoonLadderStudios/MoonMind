...
def test_attempt_two_cannot_exceed_parent_remaining_schedule_to_close() -> None:
    first_stc = 21600
    for elapsed in (0.0, 600.0, 18000.0, 21599.0):
        retry_stc = agent_run_module.profile_bound_retry_start_to_close_seconds(
            first_stc_seconds=first_stc,
            elapsed_seconds=elapsed,
        )
        # Attempt 2 is bounded by what the parent has left (clamped to the
        # minimum probe while budget remains), never a fresh window
        # on top of elapsed time.
        assert retry_stc is not None
        assert retry_stc <= first_stc
        assert elapsed + retry_stc <= first_stc + 60 or retry_stc == 60
        assert retry_stc >= 60


def test_exhausted_budget_returns_no_retry() -> None:
    # Once the first attempt consumed the whole budget there is no probe:
    # the lane must propagate the original failure instead of mutating the
    # provider or repository after execution authority expired.
    assert (
...
@pytest.mark.asyncio
async def test_exhausted_budget_reraises_without_second_attempt(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Activity-boundary test: no probe runs after the budget expires.

    When the first lane attempt fails after consuming its entire
    StartToClose budget, the lane must re-raise the original failure
    instead of issuing a minimum-probe second attempt.
    """

    lane_start = datetime(2026, 9, 10, 5, 12, 50, tzinfo=timezone.utc)
    attempt_two_now = datetime(2026, 9, 10, 11, 12, 50, tzinfo=timezone.utc)
    now_calls = {"count": 0}
    ...
    async def _fake_execute(
        *,
        ...
        stc_calls.append(stc_seconds)
        raise RuntimeError("attempt 1 consumed the budget")
    ...
    with pytest.raises(RuntimeError, match="attempt 1 consumed the budget"):

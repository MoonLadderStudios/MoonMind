"""Unit tests for the inline MM-658 → "In Progress" run-time orchestration helpers.
...
- T012: single-issue invariant (every emitted Jira tool call carries `issue_key="MM-658"`)
...
- T027: verification of `jira.get_transitions(expand_fields=true)` exposing
        `transitions[*].fields`
- T028: verification of `jira.get_issue` exposing `status.name` for `MM-658`
- T029: verification of the run-report builder canonical shape

Per spec.md, plan.md, data-model.md, and contracts/transition-mm658.md
in `specs/353-transition-mm658-in-progress/`.
"""
...
def test_build_outcome_transitioned_path() -> None:
    ...
    assert payload["issueKey"] == MM658_ISSUE_KEY
...
def test_single_issue_invariant_every_tool_call_uses_mm658() -> None:
    ...
    assert outcome.to_dict()["issueKey"] == MM658_ISSUE_KEY

    # Every recorded tool call must use issue_key="MM-658".
...
def test_transition_issue_called_with_empty_fields_and_update() -> None:
    ...
    assert transitions_calls[0]["issue_key"] == MM658_ISSUE_KEY
...
def test_concrete_jira_tool_service_is_wrapped_with_request_models() -> None:
    ...
    outcome = asyncio.run(transition_mm658_to_in_progress(trusted_jira=service))
    ...
    assert transition_request.issue_key == MM658_ISSUE_KEY
...
def test_verification_get_transitions_expand_fields_returns_fields_metadata() -> None:
    ...
    async def _run() -> None:
        await jira.get_transitions(issue_key=MM658_ISSUE_KEY, expand_fields=True)
...
# ---------------------------------------------------------------------------
# T028 — verification: jira.get_issue surfaces status.name for MM-658
# ---------------------------------------------------------------------------


def test_verification_get_issue_surfaces_status_name() -> None:
    """T028: the trusted `jira.get_issue` tool, called with `issue_key="MM-658"`,
    ...
    async def _run() -> dict[str, Any]:
        return await jira.get_issue(issue_key=MM658_ISSUE_KEY)

    payload = asyncio.run(_run())

    assert payload["fields"]["status"]["name"] == "In Review"
    assert jira.calls[0][1]["issue_key"] == MM658_ISSUE_KEY

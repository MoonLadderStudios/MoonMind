                    ...
                    for generation, action, desired in ((1, "Pause", "safe_point"), (2, "Resume", "resumed")):
                        ...
                        if action == "Pause":
                            ...
                            for _ in range(2):
                                ...
                                # The terminated run carries positive closure evidence:
                                # already_terminal satisfies the bounded scope without
                                # claiming physical host cleanup.
                                ...
                                assert mixed.targets[1].reason == "control_target_terminal"
...
async def _assert_durable_mixed_fanout(tmp_path, observed, adapter):
    ...
    try:
        ...
        async with sessions() as session:
            ...
            assert persisted[1]["reason"] == "control_target_terminal"
    finally:
        await engine.dispose()

...
@pytest.mark.asyncio
async def test_create_execution_defaults_autonomous_remediation_release_gate_closed(
    tmp_path, mock_client_adapter
):
    async with temporal_db(tmp_path) as session:
        owner_id = uuid4()
        ...
        with pytest.raises(TemporalExecutionValidationError, match="release matrix passes"):
            await service.create_execution(
                workflow_type="MoonMind.UserWorkflow", owner_id=owner_id,
                ...
                initial_parameters={"workflow": {"instructions": "Repair target", "remediation": {
                    "target": {"workflowId": target.workflow_id},
                    "authorityMode": "admin_auto",
                }}},
            )
...
@pytest.mark.asyncio
async def test_create_execution_accepts_autonomous_remediation_only_from_server_gate(
    tmp_path, mock_client_adapter
):
    ...
    async with temporal_db(tmp_path) as session:
        ...
        service = TemporalExecutionService(
            session, client_adapter=mock_client_adapter,
            autonomous_remediation_authority=lambda: {
                "schemaVersion": COMBINED_SCHEMA_VERSION,
                "matrixVersion": MATRIX_VERSION,
                "issue": "MoonLadderStudios/MoonMind#3626",
                "status": "supported", "autonomousMutationAllowed": True,

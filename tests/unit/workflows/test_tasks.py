...
def test_run_codex_preflight_check_runs_bootstrap_login_shell(monkeypatch):
    """Shell-based Codex preflight should run ``rg`` + ``codex login status`` with the auth volume."""
...
async def test_celery_chain_happy_path_persists_task_states(tmp_path, monkeypatch):
    ...
    assert submit_state.payload["selectedTool"] == "speckit"
    ...
    assert publish_state.payload["selectedTool"] == "speckit"

...
def test_retired_write_and_admin_paths_removed():
    """MoonLadderStudios/MoonMind#4108: no writer/collection/overlay lifecycle."""
    for retired in (
        "ensure_collection_ready",
        "ensure_overlay_collection",
        "upsert_overlay_vectors",
        "upsert_memory_vectors",
        "upsert_canonical_vectors",
        "delete_vectors",
        "delete_overlay_collection",
        "sync_collection_dimensions",
        "_create_collection",
        "collection_freshness_at",
    ):
        assert not hasattr(RagQdrantClient, retired), f"retired {retired} must be removed"

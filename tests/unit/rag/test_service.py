...
def test_retrieve_direct_flow_has_no_long_term_memory_branch() -> None:
    """#4109: hosted Mem0 memory is retired; retrieval merges planning+vector only."""
    ...
    assert pack.usage["latency_ms"] == 5.0
    assert not hasattr(service, "long_term_memory_service")
    assert not hasattr(service, "add_or_update_long_term_memory")
...
def test_hosted_mem0_adapter_is_retired_from_package() -> None:
    """#4109: the Mem0 module, import path, and package requirement are gone."""
    ...
    with pytest_module.raises(ModuleNotFoundError):
        __import__("moonmind.rag.long_term_memory")

    pyproject = (
        Path(__file__).resolve().parents[3] / "pyproject.toml"
    ).read_text(encoding="utf-8")
    assert not re.search(r"(?m)^\s*mem0ai\s*=", pyproject)

"""Unit tests for preset CLI helper client."""
...
from moonmind.agents.cli.presets import PresetClient, merge_expanded_steps
...
def test_list_templates_and_expand(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("moonmind.agents.cli.presets.requests.get", fake_get)
    monkeypatch.setattr("moonmind.agents.cli.presets.requests.post", fake_post)

    client = PresetClient(base_url="http://localhost:8000", token="token")

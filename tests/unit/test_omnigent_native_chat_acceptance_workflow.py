...
def test_native_chat_acceptance_workflow_uses_matrix_and_fail_closed_builder() -> None:
    workflow = yaml.safe_load(WORKFLOW.read_text(encoding="utf-8"))
    steps = workflow["jobs"]["publish"]["steps"]
    download = next(
        step for step in steps if step["name"] == "Download complete matrix evidence"
    )
    assert download["with"]["run-id"] == "${{ inputs.evidence_run_id }}"
    build = next(
        step for step in steps if step["name"] == "Build native chat acceptance report"
    )
    assert "build_native_chat_acceptance.py" in build["run"]

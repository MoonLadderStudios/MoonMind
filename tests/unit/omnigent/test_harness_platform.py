...
def make_impl(package="omnigent", version="1.0.0", digest="sha256:" + "a" * 64, kind="core", entry=None):
        ...
        harnesses=[
            {
                ...
                "runtimeRequirements": {"binaries": [{"name": "opencode", "versionConstraint": ">=1.17.7"}]},
                "capabilities": {"integrationMode": "native-server", "authModel": "own-auth", "resume": "warm-reattach", "forkHistory": "preamble", "modelFamily": "multi", "effortFamily": "none", "elicitation": "sse-permission", "interrupt": True, "streaming": True, "subagents": True},
                "setupSteps": [],
            },
            {
                "id": "codex-native",
                "aliases": [],
                "label": "Codex",
                "implementation": {"sourceKind": "core", "package": "omnigent", "version": "1.0.0", "digest": "sha256:" + "e" * 64, "pluginEntryPoint": None},
                "runtimeRequirements": {},
                "capabilities": {"integrationMode": "native-server", "authModel": "oauth_volume", "resume": "warm-reattach", "interrupt": True, "streaming": True},
                "setupSteps": [],
            },
        ],
    ...
    return OmnigentAgentProfileV2.model_validate(
        {
            ...
            "harness": {"id": "opencode-native", "catalogRef": "omnigent-harness-catalog:sha256:" + "d" * 64, "implementationRef": "omnigent-harness-implementation:sha256:" + "a" * 64},
            "requirements": {"harness": {"required": ["interrupt"], "preferred": ["streaming"]}, "moonmind": {"required": ["repository.read"]}, "host": {"required": ["workspace.bind"]}},
    ...
    return OmnigentAgentProfileV2.model_validate(
        {
            "schemaVersion": "moonmind.omnigent-agent-profile.v2",
            "endpointRef": "default",
            "source": {"kind": "bundle", "bundleArtifactRef": "artifact:bundle123", "bundleDigest": "sha256:" + "f" * 64, "importReceiptRef": "omnigent-agent-import:receipt123", "importedAgentId": "moonmind-opencode-default", "importedAgentVersion": "1.0.0", "importedContentDigest": "sha256:" + "b" * 64},
            "harness": {"id": "opencode-native", "catalogRef": "omnigent-harness-catalog:sha256:" + "d" * 64, "implementationRef": "omnigent-harness-implementation:sha256:" + "a" * 64},
...
def test_catalog_row_has_exact_implementation_and_trust():
    ...
    quarantined = classify_harness_trust(harnessId="opencode-native", implementation=impl, trustState=TrustState.quarantined)
...
def test_agent_profile_pins_harness_and_catalog():
    ...
    assert profile.harness.implementationRef.startswith("omnigent-harness-implementation:sha256:")
...
def test_skill_intent_resolves_to_immutable_refs():
    skill_set = ResolvedSkillSet.model_validate(
        {"resolvedSkillSetRef": "artifact:abcd", "resolvedSkillSetDigest": "sha256:" + "a" * 64, "skillDeliveryRef": "skill-delivery:sha256:" + "b" * 64}
    )
    ...
    second = ResolvedSkillSet.model_validate(skill_set.model_dump(by_alias=True, mode="json"))
...
def test_credential_binding_set_ref_includes_id_version_digest():
    ...
    assert bs.ref.startswith("omnigent-credential-bindings:opencode-go-primary@3#sha256:")
...
def test_plan_selects_provider_without_generation():
    ...
    profile = OmnigentAgentProfileV2.model_validate(
        {
            ...
            "endpointRef": "default",
            "source": {"kind": "upstream", "upstreamId": "opencode-native-ui", "upstreamVersion": "1.0.0", "upstreamSnapshotDigest": "sha256:" + "d" * 64},
            "harness": {"id": "opencode-native", "catalogRef": catalog.catalogRef, "implementationRef": make_impl(digest="sha256:" + "a" * 64).implementation_ref()},
    ...
    envelope = compile_execution_plan(
        ...
        trust_record=classify_harness_trust(harnessId="opencode-native", implementation=make_impl(digest="sha256:" + "a" * 64), trustState=TrustState.core_trusted),
...
def test_runtime_binding_records_generation():
    ...
    binding = create_runtime_binding(
        ...
        providerLeases={"primary-model": {"providerProfileRef": "opencode-go-default", "providerLeaseRef": "provider-lease:123", "credentialGeneration": 4, "credentialRuntimeRef": "credential-runtime:opencode-go-default:4"}},
    ...
    with pytest.raises(HarnessPlatformError) as exc:
        assert_runtime_binding_generation_sticky(binding=binding, provider_profile_ref="opencode-go-default", slot="primary-model", new_generation=5)
    assert exc.value.code == HarnessPlatformFailure.OMNIGENT_CREDENTIAL_GENERATION_FENCED
    # matching passes
    assert_runtime_binding_generation_sticky(binding=binding, provider_profile_ref="opencode-go-default", slot="primary-model", new_generation=4)
...
def test_class_capability_negotiation_blocks_missing_required():
    with pytest.raises(HarnessPlatformError) as exc:
        compute_class_admission(
            ...
            profile_requirements={"required": ["nonexistent-required"], "preferred": []},
    ...
    assert exc.value.code == HarnessPlatformFailure.OMNIGENT_CAPABILITY_REQUIRED_UNSUPPORTED
...
def test_host_class_admission_not_exact_host():
    ...
    validate_policy_for_host_class(policy=policy, host_class=hc, harness_integration_mode="native-server", materializer_refs=["codex-oauth-home@1"])
    # But exact host not yet exists - admission is class-level, not exact-host proof
    assert hc.declares_harness("opencode-native", make_impl(digest="sha256:" + "a" * 64).implementation_ref())
...
def test_exact_host_attestation_validates_all():
    ...
    validate_exact_host_attestation(
        ...
        expectedImplementation={"package": "omnigent", "version": "1.0.0", "digest": "sha256:" + "a" * 64, "pluginEntryPoint": None, "runtimeDependencies": [{"name": "opencode", "version": "1.18.11", "digest": "sha256:" + "d" * 64}]},
    ...
    with pytest.raises(HarnessPlatformError) as exc:
        validate_exact_host_attestation(
            ...
            expectedOmnigentBuildDigest="sha256:" + "b" * 64,
            expectedHarnessId="opencode-native",
            expectedImplementation={"package": "omnigent", "version": "9.9.9", "digest": "sha256:" + "a" * 64, "pluginEntryPoint": None},
            requiredCapabilities=["interrupt"],
        )
...
def test_provider_capacity_shared_across_harnesses():
    cap = effective_capacity(provider_capacity=5, materializer_capacity=10, host_capacity=3, policy_capacity=8, backend_capacity=10)
    assert cap == 3  # min
...
def test_materializer_versioned_allowlisted():
    ...
    # secret-safe handle
    handle = materialize_credential(materializer_ref="codex-oauth-home@1", provider_profile_ref="codex", provider_lease_ref="lease:1", credential_generation=2)
    assert "secret" not in json.dumps(handle).lower() or "secret" in handle.get("cleanupRef", "") is False
    ...
    assert exc.value.code == HarnessPlatformFailure.OMNIGENT_CREDENTIAL_MATERIALIZER_UNAVAILABLE
...
def test_launch_policy_generic():
    ...
    hc = get_host_class("omnigent-native-standard@3")
    validate_policy_for_host_class(policy=policy, host_class=hc, harness_integration_mode="native-server", materializer_refs=["codex-oauth-home@1"])
    validate_policy_for_host_class(policy=policy, host_class=hc, harness_integration_mode="native-server", materializer_refs=["opencode-auth-json@1"] if "opencode-auth-json@1" in hc.materializerRefs else ["codex-oauth-home@1"])
...
def test_execution_plan_is_secret_free_and_non_self_referential():
    payload_dict = {
        ...
        "harnessId": "opencode-native",
        "harnessImplementationRef": "omnigent-harness-implementation:sha256:" + "c" * 64,
        "agentSource": {"kind": "upstream", "upstreamId": "opencode-native-ui", "upstreamVersion": "1.0.0", "upstreamSnapshotDigest": "sha256:" + "d" * 64},
        "credentialBindingSetRef": "omnigent-credential-bindings:test@1#sha256:" + "e" * 64,
        "credentialBindings": {"primary-model": {"providerProfileRef": "opencode-go-default", "materializerRef": "codex-oauth-home@1"}},
        "hostClassRef": "omnigent-native-standard@3",
        "launchPolicyRef": "omnigent-on-demand@1",
        "executionRealizerRef": "generic-omnigent-host@1",
        "model": {"qualifiedId": "opencode/model", "effort": None, "routeRef": "opencode-go", "normalizedOptions": {}, "modelConfigDigest": "sha256:" + "f" * 64},
        "resolvedSkills": {"resolvedSkillSetRef": "artifact:test", "resolvedSkillSetDigest": "sha256:" + "a" * 64, "skillDeliveryRef": "skill-delivery:sha256:" + "b" * 64},
        "classAdmissionDecision": {"requiredSatisfied": ["interrupt"], "preferredSatisfied": ["streaming"], "degraded": [], "unknown": []},
        "runtimeValidationRequirements": ["exact-harness-implementation", "exact-vendor-runtime", "exact-network-egress", "exact-skill-delivery", "live-model-option"],
    ...
    assert "planRef" not in json.dumps(envelope.payload.model_dump(by_alias=True, mode="json"))
    # Verify envelope validation passes
    verified = verify_execution_plan_envelope(envelope.model_dump(by_alias=True, mode="json"))
...
def test_runtime_binding_fenced():
    ...
    binding = create_runtime_binding(
        executionPlanRef=plan_ref,
        providerLeases={"primary-model": {"providerProfileRef": "p1", "providerLeaseRef": "lease:1", "credentialGeneration": 2, "credentialRuntimeRef": "cred:1"}},
        hostBindingRef="host-binding:1",
        hostLeaseRef="host-lease:1",
    ...
    session = OmnigentSessionAggregate.model_validate(
        {
            ...
            "agentProfileSnapshotRef": "omnigent-agent-profile:sha256:" + "a" * 64,
            "agentSourceRef": "upstream:opencode-native-ui",
            "resolvedSkillRefs": {"skillDeliveryRef": "skill-delivery:sha256:" + "b" * 64},
...
def test_adding_harness_no_new_branch():
    ...
    new_harness = {
        ...
        "aliases": [],
        "label": "Qwen",
        "implementation": {"sourceKind": "core", "package": "omnigent", "version": "1.0.0", "digest": "sha256:" + "f" * 64, "pluginEntryPoint": None},
        "runtimeRequirements": {},
        "capabilities": {"integrationMode": "native-server", "authModel": "own-auth", "interrupt": True},
        "setupSteps": [],
    }
    extended = create_catalog_snapshot(
        ...
        omnigentBuildDigest="sha256:" + "b" * 64,
        sourceDigest="sha256:" + "c" * 64,
        harnesses=[h.model_dump(by_alias=True, mode="json") for h in catalog.harnesses] + [new_harness],
        observedAt=datetime.now(UTC),
    )
        ...
        harnesses=[
            {
                ...
                "aliases": [],
                "label": "Community",
                "implementation": {"sourceKind": "plugin", "package": "community", "version": "1.0.0", "digest": "sha256:" + "a" * 64, "pluginEntryPoint": "plugin.main"},
                "runtimeRequirements": {},
    ...
    with pytest.raises(HarnessPlatformError) as exc:
        compile_execution_plan(
            agent_profile={
                "schemaVersion": "moonmind.omnigent-agent-profile.v2",
                "endpointRef": "default",
                "source": {"kind": "upstream", "upstreamId": "community-ui", "upstreamVersion": "1.0.0", "upstreamSnapshotDigest": "sha256:" + "d" * 64},
                "harness": {"id": "community-plugin", "catalogRef": catalog.catalogRef, "implementationRef": impl.implementation_ref()},
                "requirements": {"harness": {"required": [], "preferred": []}, "moonmind": {"required": []}, "host": {"required": []}},
                "credentialSlots": [{"id": "primary-model", "optional": False, "acceptedAuthModels": ["own-auth"], "acceptedProviderIds": ["openai"]}],
                "model": {},
                "workspace": {},
                "skills": [],
                "tools": [],
            ...
            harness_catalog=catalog,
            trust_record=trust,
            resolved_skills={"resolvedSkillSetRef": "artifact:test", "resolvedSkillSetDigest": "sha256:" + "a" * 64, "skillDeliveryRef": "skill-delivery:sha256:" + "b" * 64},
            credential_binding_set=create_binding_set(bindingSetId="test", version=1, bindings={"primary-model": {"providerProfileRef": "p1", "materializerRef": "codex-oauth-home@1"}}),
            host_class_ref="omnigent-native-standard@3",
            launch_policy_ref="omnigent-on-demand@1",
...
def test_codex_continuity_preserved():
    ...
    profile_v1 = {
        ...
        "harness": {"id": "codex-native", "catalogRef": catalog.catalogRef, "implementationRef": make_impl(package="omnigent", digest="sha256:" + "e" * 64).implementation_ref()},
        "providerRequirements": [{"id": "primary-model", "acceptedAuthModels": ["oauth_volume"], "acceptedProviderIds": ["openai"]}],
        "model": {"model": "gpt-5"},
        "workspace": {},
        "skills": [],
    }
    ...
    bs = create_binding_set(bindingSetId="codex-openai-oauth", version=1, bindings={"primary-model": {"providerProfileRef": "codex_openai_oauth", "materializerRef": "codex-oauth-home@1"}})
    envelope = compile_execution_plan(
        agent_profile=validated,
        harness_catalog=catalog,
        trust_record=classify_harness_trust(harnessId="codex-native", implementation=make_impl(package="omnigent", digest="sha256:" + "e" * 64), trustState=TrustState.core_trusted),
        resolved_skills={"resolvedSkillSetRef": "artifact:test", "resolvedSkillSetDigest": "sha256:" + "a" * 64, "skillDeliveryRef": "skill-delivery:sha256:" + "b" * 64},
...
def test_opencode_go_composition():
    ...
    agent_profile = {
        ...
        "source": {"kind": "upstream", "upstreamId": "opencode-native-ui", "upstreamVersion": "1.0.0", "upstreamSnapshotDigest": "sha256:" + "d" * 64},
        "harness": {"id": "opencode-native", "catalogRef": catalog.catalogRef, "implementationRef": make_impl(digest="sha256:" + "a" * 64).implementation_ref()},
        "requirements": {"harness": {"required": ["interrupt"], "preferred": ["streaming"]}, "moonmind": {"required": []}, "host": {"required": []}},
        "credentialSlots": [{"id": "primary-model", "optional": False, "acceptedAuthModels": ["own-auth"], "acceptedProviderIds": ["opencode"]}],
    ...
    runtime = create_runtime_binding(
        ...
        providerLeases={"primary-model": {"providerProfileRef": "opencode-go-default", "providerLeaseRef": "lease:1", "credentialGeneration": 7, "credentialRuntimeRef": "cred:opencode-go-default:7"}},
...
def test_lifecycle_order_proves_cleanup_last():
    ...
    with pytest.raises(ValueError):
        validate_lifecycle_order(["release_provider_leases_last", "compile_execution_plan_envelope"])
    # Host attestation before session
    with pytest.raises(ValueError):
        validate_lifecycle_order(["create_or_reattach_session", "exact_host_harness_attestation"])
...
def test_support_matrix_distinguishes_realizer():
    ...
    key_generic = compute_support_combination_key(SupportKeyPayload.model_validate({**base, "executionRealizerRef": "generic-omnigent-host@1"}))
...
def test_catalog_freshness():
    ...
    assert_catalog_fresh(stale, now=datetime.now(UTC), max_age_seconds=3600, allow_stale_offline=True)

# Model config digest uniqueness per normalized options
def test_model_config_digest_includes_options():
    d1 = compute_model_config_digest(qualifiedId="opencode/model", effort=None, routeRef="opencode-go", normalizedOptions={})
    d2 = compute_model_config_digest(qualifiedId="opencode/model", effort=None, routeRef="opencode-go", normalizedOptions={"temperature": 0.7})
    assert d1 != d2
    d3 = compute_model_config_digest(qualifiedId="opencode/model", effort="high", routeRef="opencode-go", normalizedOptions={})
    assert d1 != d3

# Discovery != trusted != installed etc
def test_trust_states_distinct():
    impl = make_impl()
    for state in [TrustState.core_trusted, TrustState.plugin_approved, TrustState.quarantined, TrustState.blocked]:
        trust = classify_harness_trust(harnessId="test", implementation=impl, trustState=state)
...
def test_companion_descriptor_rejects_secret():
    with pytest.raises(Exception):
        CompanionDescriptor.model_validate(
            {
                "harnessImplementationRef": "omnigent-harness-implementation:sha256:" + "a" * 64,
...
def test_skill_delivery_mismatch_fenced():
    skills = ResolvedSkillSet.model_validate({"resolvedSkillSetRef": "artifact:test", "resolvedSkillSetDigest": "sha256:" + "a" * 64, "skillDeliveryRef": "skill-delivery:sha256:" + "b" * 64})
    with pytest.raises(HarnessPlatformError) as exc:
        assert_skill_delivery_attestation(planned=skills, attested_delivery_ref="skill-delivery:sha256:" + "c" * 64, attested_digest="sha256:" + "a" * 64)
    assert exc.value.code == HarnessPlatformFailure.OMNIGENT_SKILL_DELIVERY_MISMATCH

# Support classification
def test_support_classification():
    assert classify_support(trust_state="quarantined", launchable=False, has_conformance_evidence=False, has_experimental_evidence=False, is_static_connected=False, host_owned_auth=False) == SupportClassification.quarantined
    assert classify_support(trust_state="core_trusted", launchable=False, has_conformance_evidence=False, has_experimental_evidence=False, is_static_connected=False, host_owned_auth=False) == SupportClassification.discovered_only
    assert classify_support(trust_state="core_trusted", launchable=True, has_conformance_evidence=False, has_experimental_evidence=True, is_static_connected=False, host_owned_auth=False) == SupportClassification.experimental
    assert classify_support(trust_state="core_trusted", launchable=True, has_conformance_evidence=True, has_experimental_evidence=True, is_static_connected=True, host_owned_auth=True) == SupportClassification.connected_host
    assert classify_support(trust_state="core_trusted", launchable=True, has_conformance_evidence=True, has_experimental_evidence=True, is_static_connected=False, host_owned_auth=False) == SupportClassification.fully_managed
...
def test_failure_taxonomy_has_remediation():
    ...
    assert remediation_for(HarnessPlatformFailure.OMNIGENT_HARNESS_CATALOG_STALE) == "refresh_catalog_snapshot"
    assert remediation_for(HarnessPlatformFailure.OMNIGENT_CREDENTIAL_GENERATION_FENCED) == "reconcile_generation_fence"
...
def test_plan_digest_self_referential_rejected():
    payload = {
        ...
        "agentSource": {"kind": "upstream", "upstreamId": "opencode-native-ui", "upstreamVersion": "1.0.0", "upstreamSnapshotDigest": "sha256:" + "d" * 64},
        "credentialBindingSetRef": "omnigent-credential-bindings:test@1#sha256:" + "e" * 64,
        "credentialBindings": {"primary-model": {"providerProfileRef": "p1", "materializerRef": "codex-oauth-home@1"}},
        "hostClassRef": "omnigent-native-standard@3",
        "launchPolicyRef": "omnigent-on-demand@1",
        "executionRealizerRef": "generic-omnigent-host@1",
        "model": {"qualifiedId": None, "effort": None, "routeRef": None, "normalizedOptions": {}, "modelConfigDigest": "sha256:" + "f" * 64},
        "resolvedSkills": {"resolvedSkillSetRef": "artifact:test", "resolvedSkillSetDigest": "sha256:" + "a" * 64, "skillDeliveryRef": "skill-delivery:sha256:" + "b" * 64},
        "classAdmissionDecision": {"requiredSatisfied": [], "preferredSatisfied": [], "degraded": [], "unknown": []},

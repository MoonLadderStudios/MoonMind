  ...
  return (
    <section className="provider-profiles rounded-3xl border border-mm-border/80 bg-transparent p-6 shadow-sm">
      ...
      <div className="mt-8 border-t border-slate-200 dark:border-slate-700 pt-8">
        ...
        <form
          className="provider-profile-form space-y-6"
          onSubmit={(event) => {
        ...
        >
          ...
          <fieldset className="provider-profile-fieldset rounded-2xl border border-amber-200/60 dark:border-amber-800/40 bg-amber-50/30 dark:bg-amber-900/10 p-5 space-y-4">
          ...
          <fieldset ref={tierSectionRef as unknown as React.RefObject<HTMLFieldSetElement>} className="rounded-2xl border border-slate-200 dark:border-slate-700 p-5 space-y-4" aria-labelledby="tier-section-title">
            ...
            <ol className="provider-tier-list space-y-4" aria-label="Model and effort tiers">
              {tierDrafts.map((tier, index) => {
                ...
                return (
                  <li key={tier.clientId} data-tier-client-id={tier.clientId} className={`provider-tier-card rounded-2xl border p-4 shadow-sm ${isDefault ? 'border-violet-300 dark:border-violet-700 bg-violet-50/40 dark:bg-violet-950/20' : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800'}`}>
                    <fieldset className="provider-tier-fieldset space-y-3">
                      <legend className="provider-tier-legend">
                      ...
                      {canWriteProviderProfiles ? (
                        <div className="tier-card__actions">
                          <button type="button" className="text-xs font-medium text-slate-600 dark:text-slate-400 hover:underline" onClick={() => handleDuplicateTier(tier)} aria-label={`Duplicate Tier ${tierNumber} as new last tier`}>Duplicate tier</button>

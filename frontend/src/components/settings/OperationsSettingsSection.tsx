...
export function OperationsSettingsSection({
  ...
  if (!workerPauseConfig) {
    return (
      <section className="rounded-3xl border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-900/20 p-6 text-sm text-amber-900 dark:text-amber-400 dark:border-amber-900/50 dark:bg-amber-900/20 dark:text-amber-400 shadow-sm">
    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 dark:bg-slate-100 dark:border-slate-800 dark:bg-slate-900 dark:bg-slate-100 p-6 shadow-sm">
        <div className="space-y-2">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white dark:text-slate-900">Operations</h3>
      ...
      <section className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 dark:bg-slate-100 dark:border-slate-800 dark:bg-slate-900 dark:bg-slate-100 p-6 shadow-sm">
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <h4 className="text-lg font-semibold text-slate-900 dark:text-white dark:text-slate-900">{stateLabel}</h4>
            ...
            <div className="grid gap-4 md:grid-cols-4">
              <div className="rounded-2xl bg-slate-50 dark:bg-slate-800 dark:bg-slate-200/50 p-4 text-center">
                <div className="text-sm font-medium text-slate-500 dark:text-slate-400">Queued</div>
                <div className="text-2xl font-bold text-slate-900 dark:text-white dark:text-slate-900">{metrics.queued || 0}</div>
              </div>
              <div className="rounded-2xl bg-slate-50 dark:bg-slate-800 dark:bg-slate-200/50 p-4 text-center">
                <div className="text-sm font-medium text-slate-500 dark:text-slate-400">Running</div>
                <div className="text-2xl font-bold text-slate-900 dark:text-white dark:text-slate-900">{metrics.running || 0}</div>
              </div>
              <div className="rounded-2xl bg-slate-50 dark:bg-slate-800 dark:bg-slate-200/50 p-4 text-center">
                <div className="text-sm font-medium text-slate-500 dark:text-slate-400">Stale</div>
                <div className="text-2xl font-bold text-slate-900 dark:text-white dark:text-slate-900">
            ...
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)]">
              <section className="rounded-3xl border border-slate-200 dark:border-slate-800 p-5">
                <h4 className="text-base font-semibold text-slate-900 dark:text-white dark:text-slate-900">Worker controls</h4>
                ...
                <form className="space-y-4" onSubmit={handleResume}>
                  <fieldset className="space-y-3" disabled={actionMutation.isPending}>
                    ...
                    <button
                      type="submit"
                      className="inline-flex items-center justify-center rounded-full bg-emerald-600 dark:bg-emerald-500 px-5 dark:bg-emerald-500 py-2.5 text-sm font-semibold text-white dark:text-slate-900 transition hover:bg-emerald-500 dark:hover:bg-emerald-400"
                    >
                      Resume workers
                    </button>
                  </fieldset>
              ...
              <section className="rounded-3xl border border-slate-200 dark:border-slate-800 p-5">
                <h4 className="text-base font-semibold text-slate-900 dark:text-white dark:text-slate-900">Recent actions</h4>
                  ...
                  {snapshot?.audit?.latest && snapshot.audit.latest.length > 0 ? (
                    snapshot.audit.latest.map((event, index) => (
                      <div
                        ...
                        className="rounded-2xl bg-slate-50 dark:bg-slate-800 dark:bg-slate-200/50 p-4"
                      >
                        <div className="text-sm font-semibold text-slate-900 dark:text-white dark:text-slate-900">

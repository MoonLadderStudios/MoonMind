...
describe.skip("Task Create Entrypoint", () => {
  ...
  it("keeps manual authoring available without optional presets or image upload", async () => {
    renderWithClient(
...
describe("Task Create MM-641 authoring validation", () => {
  ...
  it("does not offer Jira issue browsing even when a stale runtime config advertises it", async () => {
    const initialData = mockPayload.initialData as {
      dashboardConfig: {
        sources?: Record<string, unknown>;
        system?: Record<string, unknown>;
      };
    };
    const staleJiraBrowserPayload: BootPayload = {
      ...mockPayload,
      initialData: {
        ...initialData,
        dashboardConfig: {
    ...
    renderWithClient(<WorkflowStartPage payload={staleJiraBrowserPayload} />);

    expect(await screen.findByLabelText("Instructions")).toBeTruthy();
    expect(screen.queryByRole("button", { name: /Browse Jira/ })).toBeNull();
    expect(
      fetchSpy.mock.calls.some(([url]) => String(url).startsWith("/api/jira/")),
    ).toBe(false);
  });

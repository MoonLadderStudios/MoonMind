...
- **Basic operation survives change.** A release must preserve the ability to create, dispatch, observe, cancel, and recover work, including recurring work and executions admitted before the release. Container health, version agreement, and a passing helper test do not establish that contract. Qualify the actual installed route and retain the previous execution capability until the replacement is proven. A new guard must include an executable recovery disposition for ordinary deployment drift and transient unavailability.
...
- **Gates steer before they stop.** Validation, approval, publish, and readiness gates should preserve validated progress by returning actionable adaptation paths before halting. Prefer bounded retry, reroute, degraded mode, draft publication, or additional verification/remediation steps when operator intent and security boundaries are preserved. Reserve hard blockers for actions that are unauthorized, untrusted, ambiguous, authority-sensitive, credential-sensitive, billing-relevant, uncertain in source authority, or less constrained.
...
- **One release owner.** Normal host, dashboard, and agent updates use the immutable image-owned release controller in `docs/Steps/DockerComposeUpdateSystem.md`. Individual fleet replacement is not a completed release. Startup and recovery detect installed workers whose effective Temporal route has no matching live pollers; `awaiting_promotion` without a durable recovery owner is not an acceptable steady state.
- **Verify ordinary traffic.** Before and after a release/routing change, inspect current/ramping versions, live pollers for every affected workflow and Activity queue, and pinned executions that still need older workers. Qualify the candidate, then verify an ordinary unpinned execution through the installed route. Candidate readiness, process liveness, and product availability are different facts.
- **Recovery must remain reachable.** The owner that repairs task routing runs outside the routing dependency it repairs, on the existing trusted deployment-control substrate. It resumes durable release records with fencing, compare-and-set promotion, and bounded retries. A scheduled repair workflow on the stranded queue cannot be the sole recovery mechanism. Add no permanently running container for this responsibility.
- **Identity is not compatibility.** A release, schema, Skill, profile, or provider identity difference requires a typed compatibility/availability decision at its owner; it is not automatically a fatal user error. Retain replay-safe payload readers or exact old workers for admitted work. Never repair a mismatch by silently changing credentials, model, effort, provider, source, publication intent, or security policy. Unknown semantic compatibility remains blocked with precise evidence and ownership.
- **Separate saved-work proof from publication and completion.** An unchanged checkout may remain recoverable when its authored remote base advances: prove its exact saved revision is still reachable without moving the checkout or claiming the newer revision. That no-change evidence grants no new candidate, PR discovery, merge, or issue-completion authority. Keep exact-tip proof distinct through serialization so retained remediation consumers cannot authorize a stale branch write from reachability alone. Owned candidates and actual writes still require exact remote-head verification; diverged, deleted, or unreadable source evidence must remain blocked. Cover ordinary concurrent base advancement through real Git and the production result consumer.
- **Preserve accepted work and truthful outcomes.** Transient capacity/routing/service failures use durable waiting or bounded continuation under the original identity and cumulative budget. Keep workspace, issue claim, and remote effects authoritative across recovery. Distinguish requested, started, waiting, skipped, and terminal outcomes. Request acceptance, a process exit, and a database projection cannot prove enqueueing, cancellation, or successful work.
- **Deliver usable recovery inputs.** A verifier artifact reference is not a readable agent input. The owning host must authorize and materialize declared evidence, then supply the actual local paths required by the resolved Skill before launch. Cover historical requests, fresh hosts, and reused workspaces; re-admitting inputs must preserve candidate edits. A ready repository or a new workflow patch marker cannot stand in for evidence delivery. Preserve transient provider failures as retryable across adapter/result/Activity boundaries, and reconcile uncertain remote writes before repeating them.
- **Carry verification capabilities across child launch.** A child that can modify repository work must also be admitted with the capabilities required to verify it. Declare that complete minimum before planning and host launch; an installed CLI, inherited profile, or successful Git push does not prove usable test authority. Supported default and explicit inputs must reach the same authorized test service with the child's actual identity and workspace. Preserve recorded authority during replay, and test the production child-creation path through capability consumption and rejection of foreign workspace access.
- **Preserve control evidence through projections.** A full report passing validation is insufficient when its compact Activity result loses the fields consumed by the workflow. Preserve typed identity, scope, freshness, and requirement bindings; keep detailed evidence in authorized artifacts. Never truncate identities or silently discard nested contract fields through a generic metadata filter. Required tests must pass the published, serialized result into its production consumer and prove both valid acceptance and rejection of missing, malformed, or expired evidence.
- **Refresh evidence after delegated mutation.** A repair child can legitimately advance the published candidate. Before post-merge finalization, re-read the same authorized repository and PR, confirm its merge, and carry its verified current head across the parent handoff. Preserve exact-head, issue, branch, and target checks; neither the original publication SHA nor a child's success claim proves the final revision. Test a child that repairs and merges a newer head while returning only its historical terminal contract.
- **Prove cleanup before re-admission.** An interrupted allocation is not objective task failure. The runtime owner must distinguish work that never reached a provider session from work requiring reattachment. A fresh admission after interruption requires durable, fenced cleanup evidence bound to the exact plan, request and admission attempt; preserve the workspace, selected provider/model and cumulative retry and execution budgets. Debit cleanup time before scheduling replacement execution. A cleanup hint, worker exit, missing heartbeat or failed canonical settlement cannot authorize another host. Test worker replacement through the production workflow entrypoint at allocation and active-session boundaries, including lost cleanup acknowledgements, concurrent fencing, budget exhaustion and replacement completion.
- **Exercise the automatic owner.** Every escaped regression requires a minimized required-CI journey entering through the same public entrypoint and recovery owner as production. Calling a repair helper from the test proves manual recovery only. Cover populated prior releases, replacement/restart, lost acknowledgements, concurrent delivery, and post-cutover traffic where relevant. Prove the new test fails for the original defect; mocked success fields and fixtures alone are insufficient.
- **Qualify what ships.** Bind required-check and release receipts to the exact source and immutable image digest promoted. Test impact selection and aggregate gates so relevant journeys cannot silently skip. An independent successful image build cannot qualify the supported default release.
- **Update architectural authority together.** Release ownership, versioning, readiness, and recovery changes update their owning architecture and operator entrypoints in the same change. Remove contradictory active guidance. State unimplemented guarantees as implementation gaps; stronger prose is not implementation evidence.

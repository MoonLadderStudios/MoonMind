...
- **Converge on Omnigent.** Move toward Omnigent as the single agent runtime provider. Codex, Claude Code, OpenCode, and other supported harnesses belong behind one generic execution lifecycle, not separate MoonMind launch, chat, checkpoint, and recovery architectures. Extend that path for new agent-runtime work. Retire direct, profile-bound, and other overlapping alternatives as their replacements prove the supported journeys. Temporary migration paths need identified consumers and a removal condition, not permanent feature flags or silent fallback. This does not replace Temporal orchestration or the authorized Docker Backend for container jobs.
...
- **One decision, one owner.** Prefer small functions, data structures, and explicit module boundaries over speculative frameworks. Derive other views from the owning decision instead of persisting competing selectors or adding policy engines to keep them in agreement. A shared abstraction must remove duplication and be easier to understand and test than the paths it replaces.
...
- **Preserve intent, not deployment accidents.** Workflows, schedules, presets, and profiles retain the requested task, harness, model, account, and permissions. They must not independently pin managed runtime images or worker releases for future launches. The installed release owns managed runtime selection and coordinated updates. Record the exact artifacts used by each attempt without making historical deployment metadata a permanent launch requirement. Reconcile an existing attempt before creating a new one.
...
- **Temporal owns durable orchestration.** Workflow code stays deterministic and side-effect-free. Side effects belong in Activities or external services, with compact non-sensitive payloads. Preserve replay and in-flight safety at that boundary rather than spreading worker-version contracts into saved task configuration.
...
- **Keep deployment small.** Docker Compose is the canonical local-first path. Reuse an existing service unless an independent lifecycle or isolation boundary justifies another always-on container. Bounded on-demand containers need ownership and cleanup, not idle infrastructure. Prefer a safe maintenance transition to unnecessary rolling-deployment machinery.

...
- **Test-Driven Development.** Before changing behavior, write or update a test that expresses the intended outcome and observe it fail for the expected reason. Make the smallest correct implementation, then refactor with tests green. Reproduce bugs before fixing them. Test observable behavior at the relevant production boundary, not incidental implementation details or merely mocked success. Passing isolated tests does not prove a complete user journey.
...
For behavior-preserving refactors, first establish passing tests for the behavior being preserved. Documentation-only changes do not need artificial failing tests. Missing dependencies are not evidence of the intended failure. Report any unexecuted test-first step honestly.

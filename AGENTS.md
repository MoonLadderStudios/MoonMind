...
**Issues describe outcomes, not permanent implementation obligations.** Before acting on a checklist, compare it with current code, the operator's current architectural direction, and existing PRs. Do not rebuild completed work or preserve a mechanism the operator has explicitly superseded. Reuse saved work and continue the existing implementation owner. When authorized to revise scope, replace stale requirements and consolidate duplicate issues instead of appending another conflicting checklist. Keep user-visible outcomes, access controls, data integrity, and active-work preservation explicit. An evidence-backed no-code resolution is valid when the work is already done, redundant, or no longer needed.

Acceptance should prove the supported behavior, not freeze every legacy class, fixture variant, status, or timeout value. Retire obsolete tests with the authorized removal of their production path, while retaining coverage for surviving behavior and necessary migration. Do not delete coverage for a still-supported requirement merely because it is expensive. Measure before adding an optimization, and prefer removing unnecessary work over adding machinery to manage it. This is review guidance, not a request for a new validator, approval step, or issue state machine.
...
- **Docker Compose is the local deployment path.** Use one portable deployment controller for host and UI requests, capable of repairing the stack without a healthy MoonMind workflow or API. Prefer pull and changed-service recreation over parallel candidate/retained fleets and promotion machinery. The [update documentation](docs/Steps/DockerComposeUpdateSystem.md) owns the transition and recovery requirements. Future launches follow the installed managed runtime, not independent image or worker-release pins in schedules or profiles. Record actual attempt artifacts and reconcile uncertain launches without rewriting history.
...
Reuse relevant CI evidence for the actual candidate instead of rerunning it in another sandbox. Pending checks remain pending. An unavailable environment is not an implementation defect and must not consume repeated implementation attempts. Hardware or operator-owned checks remain explicitly unverified when they have not run, not silently waived. Source revisions identify the code tested, not a runtime compatibility requirement.

...
[ "$CLAUDE_CONFIG_DIR" = "/home/app/.claude" ] || exit 73
[ -d "$claude_root" ] && [ -w "$claude_root" ] || exit 74
# CLAUDE_CONFIG_DIR keeps user-level .claude.json state inside the exclusive,
# generation-bound OAuth volume instead of the read-only container home.
[ ! -e "$claude_root/.claude.json" ] || [ -w "$claude_root/.claude.json" ] || exit 80

...
def is_error(line: str) -> bool:
    error_signatures = [
        "error:", "error ",  # General error terms, case handled by .lower()
    ...
    if "0 failures" in line_lower or \
       "0 errors" in line_lower or \
       "exit code 0" in line_lower or \
       "completed successfully" in line_lower or \
       "error summary" in line_lower or \
       "no error" in line_lower or \
       "error report" in line_lower and "generated" in line_lower: # e.g. "Error report generated"
        return False

    return any(sig in line_lower for sig in error_signatures)

def find_and_cluster_errors(log_lines: list[str], log_context_lines_count: int = 5) -> list[dict]:
    ...
    for cluster_indices in clusters:
        ...
        error_events.append({
            "log_context": log_context_snippet,
...
# --- API Request Helper ---
def make_api_request(url: str, headers: dict, timeout: int = 15, is_json: bool = True) -> dict | str | None:
    """
...
def get_owner_and_repo_from_git():
    try:
        remote_url_bytes = subprocess.check_output(["git", "config", "--get", "remote.origin.url"], stderr=subprocess.STDOUT)
        remote_url = remote_url_bytes.decode("utf-8").strip()
        match = re.search(r"(?:[:/])([^/]+)/([^/.]+)(?:\.git)?$", remote_url)
        if match:
            owner, repo = match.groups()
            print(f"Auto-detected OWNER: {owner}, REPO: {repo} from git remote 'origin'.", file=sys.stderr)
            return owner, repo
        else:
            print(f"Could not parse OWNER and REPO from remote URL: {remote_url}", file=sys.stderr)
            return None, None
    except subprocess.CalledProcessError:
        print("Could not find git remote 'origin'. OWNER and REPO must be set manually or git remote added.", file=sys.stderr)
        return None, None
...
def get_current_git_commit_sha():
    try:
        sha_bytes = subprocess.check_output(["git", "rev-parse", "HEAD"], stderr=subprocess.STDOUT)
        ...
        print(f"Error getting Git commit SHA: {e.output.decode('utf-8').strip()}", file=sys.stderr)
        return None
    except FileNotFoundError:
        print("Error: Git command not found. Is Git installed and in your PATH?", file=sys.stderr)
        return None
...
def get_current_git_branch():
    try:
        branch_bytes = subprocess.check_output(["git", "rev-parse", "--abbrev-ref", "HEAD"], stderr=subprocess.STDOUT)
        branch_name = branch_bytes.decode("utf-8").strip()
        if not branch_name or branch_name == "HEAD":
            print("Error: Could not determine current branch. Are you in a detached HEAD state?", file=sys.stderr)
            return None
        return branch_name
    except subprocess.CalledProcessError as e:
        print(f"Error getting Git branch: {e.output.decode('utf-8').strip()}", file=sys.stderr)
        return None
    except FileNotFoundError:
        print("Error: Git command not found. Is Git installed and in your PATH?", file=sys.stderr)
        return None
...
def get_latest_action_run(branch_name):
    ...
    if not OWNER or not REPO:
        ...
        if detected_owner and detected_repo:
            ...
            print("Error: OWNER and REPO are not set and could not be auto-detected.", file=sys.stderr)
            return None
    if not OWNER or not REPO:
    ...
    headers = {"Accept": "application/vnd.github.v3+json", "User-Agent": "Python-GitHubActionStatusScript"}
    if GITHUB_TOKEN:
    ...
    if runs_data.get("total_count", 0) == 0 or not runs_data.get("workflow_runs"):
        print(f"No workflow runs found for '{WORKFLOW_FILE_NAME}' on branch '{branch_name}'.", file=sys.stderr)
        return None
...
def get_job_log_content(job_id: int, job_name: str, owner: str, repo: str, token: str) -> list[str] | None:
    print(f"Attempting to fetch log content for job '{job_name}' (ID: {job_id})", file=sys.stderr)
    log_api_url = f"https://api.github.com/repos/{owner}/{repo}/actions/jobs/{job_id}/logs"
    base_headers = {"Accept": "application/vnd.github.v3+json", "User-Agent": "Python-GitHubActionStatusScript"}
    if token:
    ...
    req_log_location = urllib.request.Request(log_api_url, headers=base_headers, method='GET')

    try:
        print(f"Fetching log location from: {log_api_url}", file=sys.stderr)
        opener.open(req_log_location, timeout=30)
        print(f"Warning: Request to {log_api_url} did not redirect as expected.", file=sys.stderr)
        return None
        ...
        if 300 <= e_redirect.code < 400 and 'Location' in e_redirect.headers:
            ...
            print(f"Redirected to log location: {actual_log_download_url}", file=sys.stderr)
            headers_for_storage = {"User-Agent": base_headers["User-Agent"]}
            req_actual_log = urllib.request.Request(actual_log_download_url, headers=headers_for_storage, method='GET')
            try:
                with urllib.request.urlopen(req_actual_log, timeout=60) as response_actual_log:
                    if response_actual_log.status != 200:
                        print(f"Error: Expected 200 for log download from {actual_log_download_url}, got {response_actual_log.status}.", file=sys.stderr)
                        try: print(f"Storage Response body: {response_actual_log.read().decode('utf-8', 'replace')}", file=sys.stderr)
                        except: pass
                        return None
                    print("Successfully initiated download from redirected URL.", file=sys.stderr)
                    content_type = response_actual_log.info().get('Content-Type', '').lower()
                    print(f"Downloaded content type: {content_type}", file=sys.stderr)
                    zip_data_bytes = response_actual_log.read()
            except Exception as e_storage:
                print(f"Error downloading log from storage {actual_log_download_url}: {e_storage}", file=sys.stderr)
                return None
        else:
            print(f"HTTP Error (not redirect/no Location) fetching log location: {e_redirect.code} {e_redirect.reason} from {e_redirect.url}", file=sys.stderr)
            try: print(f"API Response content: {e_redirect.read().decode('utf-8', 'replace')}", file=sys.stderr)
            except: pass
            return None
    except Exception as e_initial:
        print(f"Unexpected error fetching log location {log_api_url}: {e_initial}", file=sys.stderr)
        return None
    ...
    try:
        if 'text/plain' in content_type:
            ...
            log_content_lines = zip_data_bytes.decode('utf-8', errors='replace').splitlines()
        elif 'application/zip' in content_type or 'application/octet-stream' in content_type or not content_type:
            if not content_type:
                ...
                with zipfile.ZipFile(zip_data_io, 'r') as zip_ref:
                    ...
                    for member_name in zip_ref.namelist():
                        if member_name.endswith(".txt") or '/' not in member_name:
                            log_text_parts.append(zip_ref.read(member_name).decode('utf-8', errors='replace'))
                    if not log_text_parts: return []
                    log_content_lines = "".join(log_text_parts).splitlines()
            except zipfile.BadZipFile:
                print("Error: Not a valid zip. Fallback to plain text.", file=sys.stderr)
                log_content_lines = zip_data_bytes.decode('utf-8', errors='replace').splitlines()
        else:
            print(f"Warning: Unhandled Content-Type '{content_type}'. Attempting plain text.", file=sys.stderr)
            log_content_lines = zip_data_bytes.decode('utf-8', errors='replace').splitlines()
    except Exception as e_proc:
        print(f"Error processing log data for '{job_name}': {e_proc}", file=sys.stderr)
        return None

    if not log_content_lines and zip_data_bytes: return [] # Empty but successfully processed
    print(f"Successfully processed log for job '{job_name}'. Lines: {len(log_content_lines)}", file=sys.stderr)
...
def main():
    ...
    try:
        ...
        if OWNER and REPO: # OWNER/REPO are global, set by get_latest_action_run
            ...
            print("Error: OWNER/REPO not determined. Repo info missing.", file=sys.stderr)
        ...
        if latest_run:
            ...
            wf_run_data["workflow_name"] = latest_run.get("display_title", latest_run.get("name", WORKFLOW_FILE_NAME))
            ...
            if wf_run_data.get("conclusion") == 'failure' and latest_run.get('jobs_url'):
                if OWNER and REPO:
                    jobs_list = fetch_jobs_for_run(latest_run['jobs_url'], GITHUB_TOKEN)
                    if jobs_list:
                        for job in jobs_list:
                            if job.get("conclusion") == "failure":
                                print(f"Processing failed job: {job.get('name')} (ID: {job.get('id')})", file=sys.stderr)
                                job_data = {
                                    "id": job.get("id"),
                                    "name": job.get("name"),
                                ...
                                log_lines = get_job_log_content(job.get("id"), job.get("name"), OWNER, REPO, GITHUB_TOKEN)

                                error_clusters = []
                                synthetic_step_name = f"Analysis of {job.get('name', 'job')}"
                                if log_lines is not None: # Can be [] for empty but successfully processed log
                                    if log_lines: # Only cluster if there are lines
                                        error_clusters = find_and_cluster_errors(log_lines, LOG_CONTEXT_LINES)
                                    job_steps = [{
                                        "name": synthetic_step_name,
                                        "status": "completed", # Assuming job completed to give a conclusion
                                        "conclusion": job_data["conclusion"], # Should be 'failure'
                                        "number": 1, # Synthetic step number
                                        "error_summary": {"error_clusters": error_clusters}
                                    }]
                                    ...
                                    job_steps = [{
                                        ...
                                        "error_summary": {
                                            "error_clusters": [{
                                                "log_context": [f"Failed to retrieve logs for job: {job.get('name')}"],
                                                "errors": [{"log_line_number": 1, "raw_message": "Log content unavailable."}]
                                            }]
                                        }
                                    }]
                        ...
                        print(f"Failed to fetch job list from {latest_run['jobs_url']}", file=sys.stderr)
                        output_data["failed_jobs"].append({"error": f"Failed to fetch job list from {latest_run['jobs_url']}"})
                else:
                    print("Cannot fetch jobs: OWNER or REPO not determined.", file=sys.stderr)
                    output_data["failed_jobs"].append({"error": "OWNER/REPO undetermined, cannot fetch job details."})

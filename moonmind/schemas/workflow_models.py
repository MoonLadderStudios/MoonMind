...
class OrchestratorPlanStepStateModel(BaseModel):
    ...
    started_at: datetime | None = Field(None, alias="startedAt")
    completed_at: datetime | None = Field(None, alias="completedAt")
    worker_task_id: Optional[str] = Field(None, alias="workerTaskId")
    worker_state: Optional[OrchestratorTaskState] = Field(None, alias="workerState")
    message: Optional[str] = Field(None, alias="message")
...
class OrchestratorCreateRunRequest(BaseModel):
    ...
    def _require_instruction_when_skill_not_explicit(
        ...
        normalized_instruction = (
            None if self.instruction is None else str(self.instruction).strip()
        )

...
class ConfluenceLoadRequest(BaseModel):
    ...
    max_pages_to_fetch: Optional[int] = None  # Renamed for clarity, for space/CQL loading
    ...
    @root_validator(pre=False, skip_on_failure=True) # post-validation, ensure other field validations passed
    def check_exclusive_loading_method(cls, values):
        ...
        identifiers_count = sum([
            bool(page_id),
            bool(cql_query),
            bool(page_title and space_key), # page_title is only an identifier if space_key is also present
            bool(space_key and not page_title and not page_id and not cql_query) # space_key alone
        ])

        if identifiers_count == 0:
            raise ValueError('A loading method must be specified: page_id, page_title (with space_key), cql_query, or space_key.')
        if identifiers_count > 1:
            # Refine which specific combination caused the error if possible, or keep generic
            active_methods = []
            if page_id: active_methods.append("page_id")
            if cql_query: active_methods.append("cql_query")
            if page_title and space_key: active_methods.append("page_title_with_space_key")
            ...
            raise ValueError(f'Only one loading method can be specified. Provided: {identifiers_count} methods. Check inputs for page_id, cql_query, page_title/space_key, or space_key alone.')

...
class GoogleDriveIndexer:
    ...
    def index(
        ...
        self.logger.info(
            f"Starting Google Drive indexing. Folder ID: {folder_id}, File IDs: {file_ids}"
        )

        if not folder_id and not file_ids:
            self.logger.error(
                "Either folder_id or file_ids must be provided for Google Drive loading."
            )
            raise ValueError(
                "Either folder_id or file_ids must be provided for Google Drive loading."
            )
        ...
        self.logger.info(
            f"Initializing GoogleDriveReader. Service account path: {self.service_account_key_path}"
        )
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to initialize GoogleDriveReader: {str(e)}",
            )
        ...
        if not docs:
            ...
            self.logger.info(
                "Google Drive indexing complete (no documents found) and storage context persisted."
            )
            return {"index": index, "total_nodes_indexed": total_nodes_indexed}

        self.logger.info(
            f"Loaded {len(docs)} documents from Google Drive. Converting to nodes."
        )
        try:
        ...
        if nodes:
            ...
            self.logger.info(
                f"Successfully indexed {total_nodes_indexed} nodes from Google Drive."
            )
        else:
            self.logger.info(
                "No nodes were generated from the loaded documents from Google Drive."
            )

        storage_context.persist()
        self.logger.info(
            "Google Drive indexing complete and storage context persisted."
        )

...
def summarize_text_gemini(base_prompt: str, input_text: str, model: any):
    ...
    try:
        ...
        if response.prompt_feedback and response.prompt_feedback.block_reason:
            logger.warning(
                f"Gemini content generation was blocked. Reason: {response.prompt_feedback.block_reason}. Input (start): '{input_text[:100]}...'"
            )
            return None
        ...
        if not response_text_content:
            logger.warning(
                f"Gemini API returned no parsable text content. Input (start): '{input_text[:100]}...'. Full response: {response}"
            )
            return None
...
def update_summaries(
    ...
    try:
        ...
        logger.exception(
            f"Failed to initialize model via model_factory: {e}. Aborting summary generation."
        )
        return
    ...
    try:
        ...
        logger.error(
            f"Base prompt file not found at {prompt_file_path}. Aborting summary generation."
        )
        return
    except IOError as e:
        logger.error(
            f"IOError reading base prompt from {prompt_file_path}: {e}. Aborting summary generation."
        )
        return
    except Exception as e:
        logger.error(
            f"An unexpected error occurred while loading base prompt from {prompt_file_path}: {e}. Aborting summary generation."
        )
        return
    ...
    if not os.path.exists(output_dir):
        try:
            ...
            logger.error(
                f"Failed to create output directory {output_dir}: {e}. Aborting."
            )
            return

    logger.info(f"Searching for '{input_ext}' files in '{input_dir}'")
    try:
        for input_file_path in find_files(
            search_directory=input_dir, target_extension=input_ext
        ):
            ...
            output_file_name = relative_path_from_input_dir.replace(
            ...
            if not os.path.exists(output_file_dir):
                try:
                    ...
                    logger.error(
                        f"Failed to create subdirectory {output_file_dir} for output file {output_file_path}: {e}. Skipping this file."
                    )
            ...
            if not replace_existing and os.path.exists(output_file_path):
                logger.info(
                    f"Output file {output_file_path} already exists and replace_existing is False. Skipping."
                )
            ...
            try:
                ...
                logger.info(
                    f"Generating summary for {input_file_path} to be saved at {output_file_path}."
                )
                ...
                if summary_text is None:
                    logger.error(
                        f"Failed to generate summary for content from {input_file_path}. Skipping writing to {output_file_path}."
                    )
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to write summary to {output_file_path}: {e_write}"
                    )
                ...
                logger.error(
                    f"Input file not found during processing {input_file_path}: {e_fnf}. Skipping."
                )
                files_failed_other += 1
            except IOError as e_io:
                logger.error(
                    f"IOError during processing of input file {input_file_path}: {e_io}. Skipping."
                )
                files_failed_input_read += 1
            except Exception as e_inner:
                logger.exception(
                    f"An unexpected error occurred processing input file {input_file_path}: {e_inner}. Skipping."
                )
                files_failed_other += 1
    except (
        FileNotFoundError
    ):  # This will catch if input_dir itself is not found by find_files
        logger.error(
            f"The input directory {input_dir} was not found. Cannot process files."
        )
    except Exception as e_outer:
        logger.exception(
            f"An unexpected error occurred during the main file finding/processing loop: {e_outer}"
        )
    ...
    logger.info(
        f"  Skipped (output file exists, replace_existing=False): {files_skipped_existing_output}"
    )

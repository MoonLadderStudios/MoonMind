...
PATCH_CATALOG: tuple[PatchCatalogEntry, ...] = (
    ...
    PatchCatalogEntry(
        patch_id="provider-profile-manager-db-lease-persistence-v1",
        workflow_types=(
            "MoonMindProviderProfileManagerWorkflow.release_slot",
            "MoonMindProviderProfileManagerWorkflow._acquire_slot",
            "MoonMindProviderProfileManagerWorkflow._acquire_single_flight_validation_lease",
            "MoonMindProviderProfileManagerWorkflow._acquire_exclusive_maintenance_lease",
            "MoonMindProviderProfileManagerWorkflow.run",
            "MoonMindProviderProfileManagerWorkflow._drain_queue",
            "MoonMindProviderProfileManagerWorkflow._verify_active_workflows",
            "MoonMindProviderProfileManagerWorkflow._verify_lease_holders",
        ),
        command_boundary="activity: durable lease-table writes on every grant/release/reclaim path (_persist_lease_grant, _remove_lease_from_db, _sync_leases_to_db across 10 call sites)",
        old_behavior="kept lease accounting in memory only; grants, releases, and reclaims issued no lease-table activity",
        new_behavior="persists every grant, removes every release, and syncs reclaims through the durable lease table so leases survive Continue-As-New and restarts",
        introduction_rev="300207786d566ebe977905ea3438fd88c416c672",
        fixture="none yet: deprecation requires a replay test pairing a pre-change history (marker recorded, no lease-table activity) with a post-change history, replayed against the changed call sites",
        classification=CLASSIFICATION_RETAINED_HISTORY,
        deprecate_condition="audit reports safe_to_deprecate: healthy evidence, admission cutoff newer than every admitted execution that could predate 2026-03-28, no pre-patch workers, and a replay test pairing pre/post-change histories passes against the deprecate_patch call sites",
        remove_condition="audit reports safe_to_remove for this id: no retained history carries the marker within retention and retained closed executions are no longer reset/replay eligible",
    ),
    PatchCatalogEntry(
        patch_id="provider-profile-manager-capacity-scope-v1",
        workflow_types=(
            "MoonMindProviderProfileManagerWorkflow.report_cooldown",
            "MoonMindProviderProfileManagerWorkflow._maintenance_consumes_scope",
            "MoonMindProviderProfileManagerWorkflow._apply_profile_sync",
            "MoonMindProviderProfileManagerWorkflow._clear_expired_cooldowns",
            "MoonMindProviderProfileManagerWorkflow._profile_admitted_by_capacity",
            "MoonMindProviderProfileManagerWorkflow._load_profiles_from_db",
        ),
        command_boundary="capacity accounting scope: which lease purposes consume execution slots and whether a 429 report withdraws the profile or lowers effective admission (no new activity commands; admission decisions change)",
        old_behavior="every maintenance purpose consumed provider capacity scope and a rate-limit report withdrew the profile via cooldown_until",
        new_behavior="purpose-aware capacity ledger exempts validation and maintenance leases from execution slots and a rate-limit report lowers effective admission instead of withdrawing the profile",
        introduction_rev="a962b88c0fc444c7e6a6a1c36b8ecf848ca30e69",
        fixture="none yet: deprecation requires a replay test pairing a pre-change history (marker recorded, legacy scope accounting) with a post-change history, replayed against the changed call sites",
        classification=CLASSIFICATION_RETAINED_HISTORY,
        deprecate_condition="audit reports safe_to_deprecate: healthy evidence, admission cutoff newer than every admitted execution that could predate 2026-09-04, no pre-patch workers, and a replay test pairing pre/post-change histories passes against the deprecate_patch call sites",
        remove_condition="audit reports safe_to_remove for this id: no retained history carries the marker within retention and retained closed executions are no longer reset/replay eligible",
    ),
    PatchCatalogEntry(
        patch_id="run-durable-finalization-outcome-v1",
        workflow_types=(
            "MoonMindRunWorkflow._record_prepublication_checkpoint",
            "MoonMindRunWorkflow.run",
            "MoonMindRunWorkflow._run_execution_stage",
        ),
        command_boundary="step-ledger finalization outcome recording and RunWorkflowOutput enrichment (executionOutcome/finalizationOutcome propagation; checkpoint failure recorded instead of raised)",
        old_behavior="checkpoint failures raised and the run output carried no durable execution/finalization outcomes",
        new_behavior="records a finalizationOutcome row on checkpoint failure and propagates the latest executionOutcome/finalizationOutcome pair into the run output",
        introduction_rev="1c05b2bee29636cae541b052afdf7a51cca681cb",
        fixture="none yet: deprecation requires a replay test pairing a pre-change history (marker recorded, legacy checkpoint/output shape) with a post-change history, replayed against the changed call sites",
        classification=CLASSIFICATION_RETAINED_HISTORY,
        deprecate_condition="audit reports safe_to_deprecate: healthy evidence, admission cutoff newer than every admitted execution that could predate 2026-07-10, no pre-patch workers, and a replay test pairing pre/post-change histories passes against the deprecate_patch call sites",
        remove_condition="audit reports safe_to_remove for this id: no retained history carries the marker within retention and retained closed executions are no longer reset/replay eligible",
    ),
    PatchCatalogEntry(
        patch_id="run-omnigent-remediation-checkpoint-restore-v1",
        workflow_types=(
            "MoonMindRunWorkflow._initialize_remediation_loop_controller",
            "MoonMindRunWorkflow._canonical_remediation_checkpoint_evidence",
            "MoonMindRunWorkflow._remediation_workspace_materialization_required",
            "MoonMindRunWorkflow._advance_remediation_workspace_head",
            "MoonMindRunWorkflow._run_execution_stage",
        ),
        command_boundary="dynamic remediation child request and workspace checkpoint restore sequence (workflow-owned archive ref passed into the host restore boundary; workspace-head adoption across attempts)",
        old_behavior="archived the remediation destination before the child started and seeded the loop from the verifier read-only checkout with the prior pre-execution checkpoint command sequence",
        new_behavior="passes the workflow-owned archive ref into the existing host restore boundary, validates the captured result after execution, and adopts the latest preceding candidate archive as the workspace head",
        introduction_rev="43261c55453c5317fee6d6e2e081c47a010fb536",
        fixture="none yet: deprecation requires a replay test pairing a pre-change history (marker recorded, prior checkpoint command sequence) with a post-change history, replayed against the changed call sites",
        classification=CLASSIFICATION_RETAINED_HISTORY,
        deprecate_condition="audit reports safe_to_deprecate: healthy evidence, admission cutoff newer than every admitted execution that could predate 2026-09-02, no pre-patch workers, and a replay test pairing pre/post-change histories passes against the deprecate_patch call sites",
        remove_condition="audit reports safe_to_remove for this id: no retained history carries the marker within retention and retained closed executions are no longer reset/replay eligible",
    ),
)

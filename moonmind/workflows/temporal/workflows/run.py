...
class MoonMindRunWorkflow:
    ...
    def _initialize_from_payload(
        ...
        if not isinstance(input_payload, dict):
            raise exceptions.ApplicationError("input_payload must be a dictionary", type="ValueError", non_retryable=True)
        ...
        if workflow_type != WORKFLOW_NAME:
            raise exceptions.ApplicationError(f"workflowType must be {WORKFLOW_NAME}", type="ValueError", non_retryable=True)
    ...
    async def _run_integration_stage(
        ...
        if not isinstance(metadata, dict):
            raise exceptions.ApplicationError("integration metadata must be an object when provided", type="ValueError", non_retryable=True)
    ...
    def _principal(self) -> str:
        if not self._owner_id:
            raise exceptions.ApplicationError("Trusted owner metadata is required", type="ValueError", non_retryable=True)
        return self._owner_id

    def _integration_activity_type(self, operation: str = "start") -> str:
        if not self._integration:
            raise exceptions.ApplicationError("integration is required for integration activities", type="ValueError", non_retryable=True)
    ...
    def _required_string(
        ...
        *keys: str,
        ...
        if value is None:
            raise exceptions.ApplicationError(error_message, type="ValueError", non_retryable=True)
        return value
    ...
    def _optional_string(self, payload: Mapping[str, Any], *keys: str) -> Optional[str]:
        for key in keys:
            ...
            if not isinstance(value, str):
                raise exceptions.ApplicationError(f"{key} must be a string when provided", type="ValueError", non_retryable=True)
    ...
    def _mapping_value(self, payload: Mapping[str, Any], *keys: str) -> dict[str, Any]:
        for key in keys:
            ...
            if not isinstance(value, Mapping):
                raise exceptions.ApplicationError(f"{key} must be an object when provided", type="ValueError", non_retryable=True)
    ...
    def _json_mapping(self, value: Mapping[str, Any], *, path: str) -> dict[str, Any]:
        ...
        for key, item in value.items():
            if not isinstance(key, str):
                raise exceptions.ApplicationError(f"{path} keys must be strings", type="ValueError", non_retryable=True)
    ...
    def _json_value(self, value: Any, *, path: str) -> Any:
        ...
        raise exceptions.ApplicationError(f"{path} must contain only JSON-compatible values", type="ValueError", non_retryable=True)

"""Inline run-time orchestration for the one-shot MM-658 → "In Progress" story.

This module implements the spec at `specs/353-transition-mm658-in-progress/`:

- Pre-fetches `MM-658` to record `priorStatus` (and short-circuits when already
  in `In Progress`).
...
- Verifies the post-transition status by re-reading `MM-658`.
- Emits a redacted, ≤500-char run report conforming to
  `specs/353-transition-mm658-in-progress/data-model.md` and
  `specs/353-transition-mm658-in-progress/contracts/transition-mm658.md`.
"""
...
def build_outcome(
    *,
    ...
    return TransitionMM658Outcome(
        issueKey=_redact_report_string(MM658_ISSUE_KEY) or MM658_ISSUE_KEY,
...
async def transition_mm658_to_in_progress(
    *,
    trusted_jira: TrustedJiraToolSurface | None,
) -> TransitionMM658Outcome:
    """Drive `MM-658` to `In Progress` per `contracts/transition-mm658.md`.

    The driver enforces the single-issue invariant (every Jira tool call uses
    `issue_key="MM-658"`), never invokes `jira.edit_issue` /
    ...
    try:
        issue_payload = await trusted_jira.get_issue(issue_key=MM658_ISSUE_KEY)
    ...
    if selection.kind == "ambiguous":
        ...
        return build_outcome(
            ...
            error_reason=(
                f"More than one transition for {MM658_ISSUE_KEY} targets "
    ...
    if missing:
        return build_outcome(
            ...
            error_reason=(
                f"Transition {matched.get('id')!r} for {MM658_ISSUE_KEY} requires "
    ...
    try:
        verification_payload = await trusted_jira.get_issue(issue_key=MM658_ISSUE_KEY)
    except (JiraToolError, TrustedJiraToolUnavailable) as exc:
        return _build_stopped_from_exception(
            prior_status=prior_status,
            exc=exc,

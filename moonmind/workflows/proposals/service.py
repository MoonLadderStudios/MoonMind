"""Business logic for workflow proposal queue operations."""
...
from moonmind.workflows.proposals.repositories import (
...
class WorkflowProposalService:
    ...
    def __init__(
        self,
        repository: WorkflowProposalRepository,
        *,
        ...
        proposal_settings = getattr(settings, "workflow_proposals", None)
    ...
    def _normalize_category(self, value: object) -> str | None:
        ...
        if not text:
            return None
        if len(text) > 64:
            raise WorkflowProposalValidationError("category exceeds max length")
        return text
    ...
    def _normalize_tags(
        ...
        for raw in values:
            ...
            if not candidate or candidate in seen:
                continue
            if len(candidate) > 64:
                raise WorkflowProposalValidationError("tag exceeds max length")
    ...
    @staticmethod
    def _priority_rank(value: WorkflowProposalReviewPriority) -> int:
        ordering = {
            WorkflowProposalReviewPriority.LOW: 0,
            WorkflowProposalReviewPriority.NORMAL: 1,
            WorkflowProposalReviewPriority.HIGH: 2,
            WorkflowProposalReviewPriority.URGENT: 3,
        }
        return ordering.get(value, 1)

    def _derive_moonmind_priority(
        self, tags: Sequence[str], metadata: Mapping[str, Any]
    ) -> tuple[WorkflowProposalReviewPriority | None, str | None]:
        signal = metadata.get("signal")
        signal_dict = signal if isinstance(signal, Mapping) else {}
        severity = self._clean_str(signal_dict.get("severity")).lower()
        if severity in {"high", "critical"}:
            return WorkflowProposalReviewPriority.HIGH, "signal:severity"
        ...
        if "missing_ref" in tag_set:
            ...
            if isinstance(missing_refs, (list, tuple)) and missing_refs:
                return WorkflowProposalReviewPriority.HIGH, "signal:missing_ref"
        if "retry" in tag_set:
            ...
            try:
                ...
                retry_count = 0
            if retry_count >= 2:
                return WorkflowProposalReviewPriority.HIGH, "signal:retry_exhausted"
            return WorkflowProposalReviewPriority.NORMAL, "signal:retry"
        if "duplicate_output" in tag_set or "artifact_gap" in tag_set:
            return WorkflowProposalReviewPriority.NORMAL, "signal:quality_gap"
    ...
    def _normalize_origin_source(
        ...
        if not text:
            raise WorkflowProposalValidationError("origin.source is required")
        try:
            return WorkflowProposalOriginSource(text)
        except ValueError as exc:  # pragma: no cover - validation guard
            allowed = ", ".join(member.value for member in WorkflowProposalOriginSource)
            raise WorkflowProposalValidationError(
    ...
    @staticmethod
    def _encode_cursor(proposal: WorkflowProposal) -> str:
    ...
    def _decode_cursor(cursor: str) -> tuple[datetime, UUID]:
        try:
            ...
            raise WorkflowProposalValidationError("cursor is invalid") from exc
    ...
    async def sync_proposal_delivery(self, *, proposal_id: UUID) -> WorkflowProposal:
    ...
    async def create_proposal(
        ...
        *,
        ...
        requested_priority = (
            ...
            else WorkflowProposalReviewPriority.NORMAL
        )
        ...
        if callable(duplicate_finder):
            ...
            if duplicate is not None and not isinstance(duplicate, Mock):
                logger.info(
                    "Reusing open workflow proposal %s (provider=%s repository=%s)",
        ...
        logger.info(
            "Created workflow proposal %s (repository=%s category=%s)",
            proposal.id,
            repository,
    ...
    async def list_proposals(
        self,
        *,
        status: WorkflowProposalStatus | None,
        category: str | None,
        repository: str | None,
        origin_source: WorkflowProposalOriginSource | None,
        origin_id: UUID | None,
        cursor: str | None,
        limit: int,
    ) -> tuple[list[WorkflowProposal], str | None]:
        if limit < 1 or limit > 200:
            raise WorkflowProposalValidationError("limit must be between 1 and 200")
        normalized_category = self._normalize_category(category)
    ...
    async def get_proposal(self, proposal_id: UUID) -> WorkflowProposal:
        proposal = await self._repository.get_proposal(proposal_id)
        if proposal is None:
            raise WorkflowProposalNotFoundError(str(proposal_id))
        return proposal
    ...
    async def get_similar_proposals(
        self, proposal: WorkflowProposal, limit: int | None = None
    ...
    async def record_provider_decision_event(
        ...
        *,
        ...
        if result.accepted and result.decision == "dismiss":
            ...
            resulting_state = WorkflowProposalStatus.DISMISSED.value
        elif result.accepted and result.decision == "promote":
            proposal.status = WorkflowProposalStatus.ACCEPTED
            proposal.decision_note = (
                self._scrub_text(self._clean_str(result.note)) or None
            )
            resulting_state = WorkflowProposalStatus.ACCEPTED.value
    ...
    async def dismiss_proposal(
        ...
        *,
        ...
        proposal = await self._repository.get_proposal_for_update(proposal_id)
        if proposal.status is not WorkflowProposalStatus.OPEN:
            raise WorkflowProposalStatusError(

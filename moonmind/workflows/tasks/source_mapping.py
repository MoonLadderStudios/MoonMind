...
class TaskSourceMappingService:
    ...
    async def resolve_task(
        ...
        *,
        ...
        canonical_task_id = db_models.TemporalExecutionRecord.canonicalize_identifier(
            task_id
        )
        mapping = await self._session.get(
            db_models.TaskSourceMapping, canonical_task_id
        )
        ...
        matches = await self._probe_backing_records(
            task_id=canonical_task_id, user=user
        )

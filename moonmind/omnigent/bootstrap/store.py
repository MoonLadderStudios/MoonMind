...
def load_resolved_state() -> ResolvedOmnigentDeploymentState | None:
    ...
    # Prefer neutral, fallback to alias
    effective_host_ref = runtime_ref or host_ref
    build_digest = os.getenv("OMNIGENT_BUILD_DIGEST", "").strip()
    if server_ref or effective_host_ref or build_digest:
        try:
            return ResolvedOmnigentDeploymentState(
                serverImageRef=server_ref or None,
                opencodeHostImageRef=host_ref or effective_host_ref or None,
                runtimeHostImageRef=runtime_ref or effective_host_ref or None,

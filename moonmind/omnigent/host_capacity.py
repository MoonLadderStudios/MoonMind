...
admits. It says nothing about how many on-demand Docker hosts may exist at
once, and nothing about how many of those hosts may begin a cold launch at
once. A deployment-selected provider ceiling must therefore be admitted
against two further limits before a container is created:
...
There is deliberately no automatic machine-resource accounting here. Fixed
per-container limits are enforced directly by Docker; this module only bounds
how many hosts exist and how fast cold launches start.
...
#: Serializes the count-and-admit pair for generic host admission. Two
...
#: constant: it names the generic-host domain and never carries a
...
class GenericHostCapacityDecision:
    ...
    #: Retained for payload compatibility; always False now that there is no
    #: resource-admission layer that could report an unsatisfiable request.
...
def evaluate_generic_host_capacity(
    *,
    ...
    The aggregate ceiling is checked first: when the host count is already
    full, reporting the cold-launch layer would send the operator after the
    wrong limit.
    """
...
class GenericHostCapacityAdmission:
    ...
    async def evaluate(
        ...
        *,
        ...
        This read reserves nothing and is therefore advisory: passing it
        does not hold a host slot, and the durable allocation may still refuse
        a race the caller must return to waiting for.
        """
    ...
    async def evaluate_within(
        ...
        *,
        ...
        The caller must insert its durable lease row in this same
    ...
    def _verdict(
        ...
        *,
        ...
        if already_allocated:
            # An execution counted in the ledger already holds its slot,
            # so it is never re-admitted against it.
            return GenericHostCapacityDecision(
                admitted=True,
                limiting_layer=None,
                active_hosts=active_hosts,

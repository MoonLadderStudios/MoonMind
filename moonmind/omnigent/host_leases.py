...
class DbOmnigentHostLeaseRepository:
    def __init__(
        ...
        *,
        ...
        # limits are only real if the count and the durable lease are one
    ...
    async def acquire(self, **kwargs: Any) -> HostLeaseAuthority:
        ...
        if resource_limits is not None:
            # Resource limits are explicit ceilings enforced directly by
            # Docker. A zero CPU value is never a pool selection here: fail
            # closed rather than launching unlimited work.
            try:
                cpu_millis = int(resource_limits.get("cpuMillis", 0))
            except (AttributeError, TypeError, ValueError):
                cpu_millis = 0
            if cpu_millis < 1:
                raise HarnessPlatformError(
                    "generic host allocation requires a positive explicit "
                    "CPU limit",

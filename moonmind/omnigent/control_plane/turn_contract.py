...
def idempotency_scope_for(request: TurnSubmissionRequest) -> str:
    ...
    The caller-supplied key is part of the immutable identity. Re-delivery of
    the same key collapses to one turn, while two intentional messages with the
    same content but different keys remain distinct provider turns.
    """
    ...
    if request.remediation is not None:
        ...
        # Exact controller inputs participate in the durable decision digest.
        # A reused idempotency key cannot silently point at different gate,
        # remaining-work, candidate, authority, or budget evidence.
        parts["remediation"] = {
            "loop_id": remediation.loop_id,
            "ordinal": remediation.remediation_attempt_ordinal,
            "of_turn_attempt_id": remediation.of_turn_attempt_id,
            "gate_result_ref": remediation.gate_result_ref,
            "remaining_work_ref": remediation.remaining_work_ref,
            "candidate_workspace_ref": remediation.candidate_workspace_ref,
            "candidate_checkpoint_ref": remediation.candidate_checkpoint_ref,
            "remediator_skill": remediation.remediator_skill,
            "runtime_authority_ref": remediation.runtime_authority_ref,
            "production_boundary_evidence_ref": (
                remediation.production_boundary_evidence_ref
            ),
            "attempt_budget": remediation.attempt_budget,
            "branch_budget": remediation.branch_budget,
            "verification_requirements": remediation.verification_requirements,
            "allow_same_session_reuse": remediation.allow_same_session_reuse,
            "grants_publication_authority": (
                remediation.grants_publication_authority
            ),
            "granted_dimensions": {
                dimension.name: getattr(
                    remediation.granted_dimensions, dimension.name
                )
                for dimension in fields(remediation.granted_dimensions)
            },
        }
...
def plan_turn_submission(
    ...
    if not request.caller_id.strip():
        raise CallerAuthorityError("turn submission requires caller authority")
    if not request.idempotency_key.strip():
        raise CallerAuthorityError("turn submission requires an idempotency key")
    if not request.instruction_digest.strip():
        raise CallerAuthorityError("turn submission requires an instruction digest")

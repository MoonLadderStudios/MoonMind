...
def credential_runtime_ref(
    *,
    ...
    """Return the secret-free identity later used by materialization."""
    ...
    return f"credential-runtime:sha256:{identity_digest}"
...
class TrustedCredentialMaterializer:
    ...
    async def materialize(
        ...
        *,
        ...
        runtime_ref = credential_runtime_ref(
        ...
        identity_digest = runtime_ref.rsplit(":", 1)[-1]
        root = self._credential_root(runtime_ref)
        ...
        handle["credentialRuntimeRef"] = runtime_ref
        ...
        async with self._session_factory() as session:
            existing = await session.get(
                OmnigentCredentialRuntimeRecord, runtime_ref
            )
            values = {
                "provider_profile_ref": profile.profile_id,
                "provider_lease_ref": provider_lease_ref,
            ...
            if existing is None:
                session.add(
                    OmnigentCredentialRuntimeRecord(
                        credential_runtime_ref=runtime_ref, **values
                    )
                )
...
class DeploymentGenericHostServices:
    ...
    async def _materialize_skills(
        ...
        if resolved_ref:
            artifact_id = resolved_ref.removeprefix("artifact:")
            if (
                state.request is None
                or state.request.resolved_skillset_ref != artifact_id
            ):
                raise HarnessPlatformError(
                    "request Skill snapshot differs from the admitted plan",
                    code=HarnessPlatformFailure.OMNIGENT_SKILL_DELIVERY_MISMATCH,
                )
            read_artifact = getattr(self._artifact_gateway, "read", None)
            if read_artifact is None:
                raise HarnessPlatformError(
                    "resolved Skill artifact reader is unavailable to the host owner",
                    code=HarnessPlatformFailure.OMNIGENT_SKILL_SNAPSHOT_UNAVAILABLE,
                )
            try:
                _artifact, manifest_body = await read_artifact(
                ...
                raise HarnessPlatformError(
                    "resolved Skill manifest artifact is unavailable",
                    code=HarnessPlatformFailure.OMNIGENT_SKILL_SNAPSHOT_UNAVAILABLE,
                ) from exc
            ...
            if manifest_digest != digest:
                raise HarnessPlatformError(
                    "resolved Skill manifest digest differs from the admitted plan",
                    code=HarnessPlatformFailure.OMNIGENT_SKILL_DELIVERY_MISMATCH,
                )
            try:
                resolved = AgentResolvedSkillSet.model_validate(manifest_payload)
                materialized = await AgentSkillMaterializer(
                    workspace_root=str(self._runtime_root / "skill-workspaces"),
                    artifact_service=self._artifact_gateway,
                    backing_root=str(projection),
                    project_adapter_aliases=False,
                ).materialize(
                    resolved_skillset=resolved,
                    runtime_id=(
                        state.plan.payload.harnessId
                ...
                raise HarnessPlatformError(
                    "resolved Skill snapshot could not be materialized",
                    code=HarnessPlatformFailure.OMNIGENT_SKILL_SNAPSHOT_UNAVAILABLE,
                ) from exc
            if len(materialized.workspace_paths) != 1:
                raise HarnessPlatformError(
                    "resolved Skill materialization did not produce one active snapshot",
                    code=HarnessPlatformFailure.OMNIGENT_SKILL_DELIVERY_MISMATCH,
                )
            projection = Path(materialized.workspace_paths[0]).resolve()
            if not projection.is_relative_to(self._runtime_root):
                raise HarnessPlatformError(
                    "resolved Skill materialization escaped runtime authority",
                    code=HarnessPlatformFailure.OMNIGENT_SKILL_DELIVERY_MISMATCH,
                )
        else:

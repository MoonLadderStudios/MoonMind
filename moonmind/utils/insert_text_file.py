...
def insert_text_file(file_path, text, line_number, blank_lines_before=0, blank_lines_after=0):
    ...
    try:
        ...
        modified_content_lines = current_lines[:insert_index] + lines_to_insert_block + current_lines[insert_index:]

        # 5. Write the new content back to the file
        with open(file_path, "w", encoding="utf-8") as f:
            f.writelines(modified_content_lines)

        logger.info(f"Successfully inserted text in file: {file_path} at line {line_number}")
        return True

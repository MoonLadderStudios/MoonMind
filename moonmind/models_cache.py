...
class ModelCache:
    ...
    def __init__(
        ...
        with self._lock:
            ...
            self._refresh_operation_lock = (
                Lock()
            )  # New instance lock for refresh operations
    ...
    def _fetch_all_models(self) -> Tuple[List[Dict[str, Any]], Dict[str, str]]:
            ...
            if settings.is_provider_enabled("google"):
                ...
                for model in google_models_list:  # Iterate over the list
                    ...
                    capabilities = {
                        ...
                        "text_completion": "generateContent"
                        in model.supported_generation_methods,
                ...
                if not settings.google.google_enabled:
                    ...
                    self.logger.warning(
                        "Google API key not set. Skipping Google models."
                    )
            ...
            if settings.is_provider_enabled("openai"):
                ...
                for (
                    ...
                    if "gpt-4" in model_id:  # Covers gpt-4, gpt-4-32k etc.
                        ...
                        if "turbo-2024-04-09" in model_id or "128k" in model_id:
                    ...
                    capabilities = {  # Assume chat models are for chat/text completion
                        ...
                        "embedding": "embedding" in model_id,
                    }
                    model_entry = {
                        "id": model_id,
                        "object": "model",
                        "created": int(getattr(model, "created", time.time())),
                        "owned_by": "OpenAI",
                        "permission": [],
                        "root": model_id,
                        "parent": None,
                ...
                if not settings.openai.openai_enabled:
                    ...
                    self.logger.warning(
                        "OpenAI API key not set. Skipping OpenAI models."
                    )
        except Exception as e:
            ...
            if settings.is_provider_enabled("anthropic"):
                ...
                if anthropic_model_name:
                    ...
                    model_entry = {
                        ...
                        "created": int(
                            time.time()
                        ),  # Placeholder, Anthropic models don't have a creation timestamp via API
                        "owned_by": "Anthropic",
                        "permission": [],
                    ...
                    self.logger.info(
                        f"Added configured Anthropic model: {anthropic_model_name}"
                    )
            else:
                if not settings.anthropic.anthropic_enabled:
                    ...
                    self.logger.warning(
                        "Anthropic API key not set. Skipping Anthropic models."
                    )
        except Exception as e:
            self.logger.exception(f"Error adding Anthropic model: {e}")

        self.logger.info(
            f"Total models fetched: {len(all_models_data)}. Model to provider map size: {len(model_to_provider_map)}"
        )
        ...
        try:
            ...
            default_provider_name = (
                settings.default_chat_provider.capitalize()
            )  # Capitalize to match "owned_by"

            self.logger.info(
                f"Default chat provider: {default_provider_name}, Default chat model: {default_chat_model_id}"
            )
            ...
            for i, model_entry in enumerate(all_models_data):
                ...
                if (
                    ...
                    and current_model_owner == "Google"
                ):
                    ...
                    if default_chat_model_id.startswith("models/"):
                        ...
                        is_default_model = (
                            current_model_id == f"models/{default_chat_model_id}"
                        )
                elif (
                ...
                if is_default_model:
                    ...
                    self.logger.info(
                        f"Found default model '{default_chat_model_id}' from provider '{default_provider_name}' and moved it to the top."
                    )
                    break
            ...
            if default_model_entry:
                ...
                self.logger.warning(
                    f"Default model '{default_chat_model_id}' from provider '{default_provider_name}' not found in the fetched models list."
                )

        except Exception as e:
            self.logger.exception(
                f"Error while trying to prioritize default model: {e}"
            )
    ...
    def _periodic_refresh(self):
        ...
        while True:
            ...
            if time_since_last_refresh >= self.refresh_interval_seconds:
                self.logger.info(
                    f"Scheduled refresh interval reached ({self.refresh_interval_seconds}s). Refreshing models."
                )
    ...
    def get_all_models(self) -> List[Dict[str, Any]]:
        if not self.models_data or (
            time.time() - self.last_refresh_time > self.refresh_interval_seconds
        ):  # also check if cache is empty
            self.logger.info(
                "Models data is empty or stale, attempting synchronous refresh before returning."
            )
            self.refresh_models_sync()
    ...
    def get_model_provider(self, model_id: str) -> Optional[str]:
        if not self.model_to_provider or (
            ...
            self.logger.info(
                "Model to provider map is empty or stale, attempting synchronous refresh."
            )
            self.refresh_models_sync()
    ...
    def get_model_details(self, model_id: str) -> Optional[Dict[str, Any]]:
        ...
        if not self.models_data or (
            ...
            self.logger.info(
                "Models data is empty or stale, attempting synchronous refresh before returning details."
            )
            self.refresh_models_sync()

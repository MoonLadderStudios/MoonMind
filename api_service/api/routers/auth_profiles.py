...
async def create_profile(
    ...
    await _sync_auth_profile_manager(session=session, runtime_id=body.runtime_id)
...
async def update_profile(
    ...
    await session.commit()
    await session.refresh(profile)
    await _sync_auth_profile_manager(session=session, runtime_id=profile.runtime_id)
    return _row_to_dict(profile)
...
async def delete_profile(
    ...
    await session.commit()
    await _sync_auth_profile_manager(session=session, runtime_id=runtime_id)
...
async def _sync_auth_profile_manager(
    *,

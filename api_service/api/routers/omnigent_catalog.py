...
_BOOTSTRAP_EVIDENCE_SCHEMA_VERSION = "moonmind.omnigent.catalog-bootstrap-evidence/v1"
...
class OmnigentCodexCatalogReadiness(BaseModel):
    ...
    execution_profiles: list[ExecutionProfileReadiness] = Field(
        alias="executionProfiles"
    )
...
class GenericExecutionTargetReadiness(BaseModel):
    ...
    agent_profile_ref: dict[str, Any] = Field(alias="agentProfileRef")
    available: bool
    support_tier: Literal["experimental", "supported"] = Field(alias="supportTier")
    compatible_provider_profiles: list[dict[str, Any]] = Field(
        alias="compatibleProviderProfiles"
    )
    compatible_host_classes: list[str] = Field(alias="compatibleHostClasses")
    policies: list[str]
    models: list[str]
    gate_reasons: list[GateReason] = Field(alias="gateReasons")
...
class OmnigentExecutionReadiness(BaseModel):
    ...
    schema_version: Literal["moonmind.omnigent-execution-readiness.v3"] = Field(
        "moonmind.omnigent-execution-readiness.v3", alias="schemaVersion"
    )
    runtime_id: Literal["omnigent"] = Field("omnigent", alias="runtimeId")
    display_name: Literal["Omnigent"] = Field("Omnigent", alias="displayName")
    execution_targets: list[GenericExecutionTargetReadiness] = Field(
        alias="executionTargets"
    )
...
class AcceptanceBootstrapEvidence(BaseModel):
    ...
    schema_version: Literal["moonmind.omnigent.catalog-bootstrap-evidence/v1"] = Field(
        _BOOTSTRAP_EVIDENCE_SCHEMA_VERSION, alias="schemaVersion"
    )
    ...
    def _require_immutable_image(cls, value: str) -> str:
        if not _DIGEST_IMAGE.fullmatch(value) or value.endswith("@sha256:" + "0" * 64):
...
_REASONS: dict[str, tuple[str, str]] = {
    "bridge_disabled": (
        "Enable the Omnigent bridge in deployment settings.",
        "/settings#omnigent",
    ),
    "bridge_conformance_gated": (
        "Complete Omnigent bridge conformance checks.",
        "/settings#omnigent",
    ),
    "bridge_endpoint_unavailable": (
        "Configure the selected Omnigent endpoint.",
        "/settings#omnigent",
    ),
    ...
    "rollout_gate_disabled": (
        "Enable the Omnigent runtime rollout gate.",
        "/settings#omnigent",
    ),
    "host_auth_unavailable": (
        "Configure or rotate Omnigent bridge credentials.",
        "/settings#omnigent",
    ),
    ...
    "static_host_not_ready": (
        "Start and validate the selected static Omnigent host.",
        "/settings#omnigent",
    ),
    "immutable_image_unavailable": (
        "Retry stock Omnigent image acquisition or configure explicit server and host image digests.",
        "/settings#omnigent",
    ),
    "network_policy_unavailable": (
        "Configure the required enforced egress policy.",
        "/settings#omnigent",
    ),
    "acceptance_evidence_unavailable": (
        "Publish a current #3508 browser acceptance matrix for this source commit.",
        "/settings#omnigent",
    ),
    ...
    "live_verification_stale": (
        "Restore fresh protected-live verification evidence before rollout.",
        "/settings#omnigent",
    ),
    "workspace_resolver_unavailable": (
        "Restore the workflow workspace resolver.",
        "/settings#system",
    ),
    "profile_reconnect_required": (
        "Reconnect this OAuth Provider Profile.",
        "/settings#provider-profiles",
    ),
    "profile_validation_required": (
        "Validate this OAuth Provider Profile.",
        "/settings#provider-profiles",
    ),
    ...
    "generic_realizer_not_ready": (
        "Finish the generic Omnigent host deployment setup.",
        "/settings#omnigent",
    ),
    "opencode_support_not_qualified": (
        "Run the protected OpenCode qualification before enabling this target.",
        "/settings#omnigent",
    ),
    "harness_catalog_unavailable": (
        "Synchronize the authenticated Omnigent harness catalog.",
        "/omnigent/agents",
    ),
    "host_class_unavailable": (
        "Configure the digest-pinned host runtime pack for this harness.",
        "/settings#omnigent",
    ),
    "compatible_provider_profile_unavailable": (
    ...
    *,
    ...
    if bootstrap_evidence is not None:
        ...
        return SupportEvidence(
            ...
            server_digest=bootstrap_evidence.server_image_ref_observed.rsplit("@", 1)[
                1
            ],
    ...
    try:
        ...
        images = (
            manifest.get("images") if isinstance(manifest.get("images"), dict) else {}
        )
        age = evidence_age
    ...
    try:
        if not exact_path or not source_commit:
            raise ExactArtifactConformanceError(
                "exact-artifact evidence is not configured"
            )
        evidence = json.loads(Path(exact_path).read_text(encoding="utf-8"))
        if not isinstance(evidence, dict):
            raise ExactArtifactConformanceError(
                "exact-artifact evidence must be an object"
            )
...
def _websocket_runtime_available() -> bool:
    ...
    return any(
        getattr(route, "path", "") == "/{chat_binding_id}/omnigent/{omnigent_path:path}"
...
async def get_omnigent_codex_catalog_readiness(
    ...
    for row in rows:
        ...
        credential_source = getattr(
            row.credential_source, "value", row.credential_source
        )
    ...
    snapshot_observed_at, event_observed_at = (
        await session.execute(
            select(
                ...
                select(func.max(OmnigentObservation.observed_at))
                .where(
                    OmnigentObservation.observation_type.in_(_EVENT_OBSERVATION_TYPES)
                )
                .scalar_subquery(),
            )
        )
    ...
    for profile in PROFILES.values():
        # OpenCode and future harnesses are compiled by readiness v3 from the
        # persisted catalog/selector plane. They must never inherit Codex
        # policy matching from this legacy Codex readiness projection.
        ...
        for identity, version in persisted_policies:
            ...
            if not version.validation_json.get("valid"):
                policy_reasons.append(_reason("execution_profile_unavailable"))
            if not all(
                _DIGEST_IMAGE.fullmatch(str(host.get(field) or ""))
                for field in ("serverImageRef", "hostImageRef")
            ):
                policy_reasons.append(_reason("immutable_image_unavailable"))
            if (
                network.get("attachmentRef") not in live_readiness.enforced_network_refs
    ...
    deployed_reconciler_ready = (
        ...
        and "agent_runtime.reconcile_managed_sessions" in live_readiness.activity_types
    )
    janitor_healthy = (
        live_readiness.backend_ready
        and "integration.omnigent.oauth_host_janitor" in live_readiness.activity_types
    )
    ...
    event_transport_ready = bool(
        event_age is not None and timedelta(0) <= event_age <= timedelta(minutes=10)
    )
...
def _catalog_model_ids(value: Any) -> set[str]:
    models: set[str] = set()
    if isinstance(value, str) and "/" in value and len(value) <= 255:
        models.add(value)
    elif isinstance(value, dict):
        for item in value.values():
            models.update(_catalog_model_ids(item))
...
async def get_omnigent_execution_readiness(
    session: AsyncSession = Depends(get_async_session),
    current_user: User = Depends(get_current_user()),
) -> OmnigentExecutionReadiness:
    """Compile generic launchability with the planner's catalog/selector data."""
    ...
    visible_providers = [
        ...
        if _can_view_profile(profile, current_user)
    ]
    ...
    for profile_row in profiles:
        ...
        if (
            version is None
            or version.document.get("schemaVersion")
            != "moonmind.omnigent-agent-profile.v2"
        ):
            continue
        reasons: list[GateReason] = []
        if not generic_host_enabled():
            reasons.append(_reason("generic_realizer_not_ready"))
        if (
            not version.validation_result
            or version.validation_result.get("ready") is not True
        ):
            reasons.append(_reason("agent_profile_not_ready"))
        try:
            profile = validate_agent_profile(dict(version.document))
        except Exception:
            continue
        ...
        if catalog is None or fresh_catalog is None:
            ...
            harness = next(
                (
                    ...
                    == profile.harness.implementationRef
                ),
                None,
            )
            trust = next(
        ...
        for provider in visible_providers:
            ...
            try:
                ...
                if harness is None or catalog is None:
                    continue
                policy_refs = profile.allowedLaunchPolicyRefs
                selected_classes = [
                    selector.select(
                        harness=harness,
            ...
            compatible.append(
                {
                    ...
                    "label": provider.account_label or provider.profile_id,
        ...
        targets.append(
            GenericExecutionTargetReadiness(
                ...
                compatibleProviderProfiles=compatible,
                compatibleHostClasses=sorted(host_classes),
...
async def synchronize_omnigent_harness_catalog(
    session: AsyncSession = Depends(get_async_session),
    current_user: User = Depends(get_current_user()),
) -> dict[str, Any]:

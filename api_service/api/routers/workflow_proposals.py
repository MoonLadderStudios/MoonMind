"""REST router for workflow proposal queue operations."""
...
async def _get_service(
    ...
    return get_workflow_proposal_service(session)
...
def _build_workflow_preview(
    ...
    return WorkflowProposalPreview(
...
def _serialize_promotion_result(proposal: WorkflowProposal) -> dict[str, object] | None:
    provider_metadata = getattr(proposal, "provider_metadata", None)
    decision_rows = (
...
def _serialize_proposal(
    ...
    origin = WorkflowProposalOriginModel(
...
def _promotion_title(
    ...
    return title or "Promoted Workflow"
...
async def list_proposals(
    *,
    ...
    if status_filter:
        ...
        try:
            status_value = WorkflowProposalStatus(normalized_status)
    ...
    if origin_source:
        try:
            origin_value = WorkflowProposalOriginSource(origin_source.lower())
        except ValueError as exc:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def get_proposal(
    *,
    ...
    similar_rows: list[WorkflowProposal] | None = None
    if include_similars:
...
async def promote_proposal(
    *,
    proposal_id: UUID,
    payload: WorkflowProposalPromoteRequest = Body(
        default_factory=WorkflowProposalPromoteRequest
    ),
    service: WorkflowProposalService = Depends(_get_service),
...
async def provider_decision(
    *,
    proposal_id: UUID,
    payload: WorkflowProposalProviderDecisionRequest = Body(...),
    service: WorkflowProposalService = Depends(_get_service),
    execution_service: TemporalExecutionService = Depends(
        _get_temporal_execution_service
    ),
...
@router.get("/{proposal_id}/delivery", response_model=WorkflowProposalModel)
...
@router.post("/{proposal_id}/redeliver", response_model=WorkflowProposalModel)
...
@router.post("/{proposal_id}/sync", response_model=WorkflowProposalModel)
...
async def dismiss_proposal(
    *,
    proposal_id: UUID,
    payload: WorkflowProposalDismissRequest = Body(
        default_factory=WorkflowProposalDismissRequest
    ),
    service: WorkflowProposalService = Depends(_get_service),
    user: User = Depends(get_current_user()),
) -> WorkflowProposalModel:
...
async def update_priority(
    *,
    proposal_id: UUID,
    payload: WorkflowProposalPriorityRequest = Body(...),
    service: WorkflowProposalService = Depends(_get_service),
    user: User = Depends(get_current_user()),
) -> WorkflowProposalModel:
    try:

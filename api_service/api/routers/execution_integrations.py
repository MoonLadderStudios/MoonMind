...
def _expected_callback_token(integration_name: str) -> str | None:
    normalized = str(integration_name or "").strip().lower()
    if normalized == "jules":
        token = str(settings.jules.jules_callback_token or "").strip()
        return token or None
    return None


def _callback_payload_limit(integration_name: str) -> int:
    normalized = str(integration_name or "").strip().lower()
    if normalized == "jules":
        return int(settings.jules.jules_callback_max_payload_bytes)
    return _DEFAULT_CALLBACK_MAX_PAYLOAD_BYTES


def _callback_rate_limit(integration_name: str) -> tuple[int, int]:
    normalized = str(integration_name or "").strip().lower()
    if normalized == "jules":
        return (
            int(settings.jules.jules_callback_rate_limit_per_window),
            int(settings.jules.jules_callback_rate_limit_window_seconds),
        )
    return (30, 60)


def _capture_callback_artifacts(integration_name: str) -> bool:
    normalized = str(integration_name or "").strip().lower()
    if normalized == "jules":
        return bool(settings.jules.jules_callback_artifact_capture_enabled)
    return False
...
async def _validate_callback_request(
    *,
    ...
    expected_token = _expected_callback_token(integration_name)
    ...
    max_payload_bytes = _callback_payload_limit(integration_name)
    ...
    limit, window_seconds = _callback_rate_limit(integration_name)
    bucket_key = f"{str(integration_name).strip().lower()}:{request.url.path}"
...
async def ingest_integration_callback(
    ...
    try:
        ...
        if payload_artifact_ref is None and _capture_callback_artifacts(integration_name):

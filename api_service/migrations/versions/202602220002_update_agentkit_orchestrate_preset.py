...
revision: str = "202602220002"
down_revision: str | None = "202602220001"
...
def upgrade() -> None:
    ...
    bind.execute(
        ...
        .where(templates_table.c.id == str(template_id))
        .values(
    ...
    bind.execute(
        ...
        .where(versions_table.c.id == str(version_id))
        .values(
            required_capabilities=required_capabilities,
            steps=steps,
...
def downgrade() -> None:
    """Best-effort backward-compatibility migration; does not reintroduce codex defaults."""

...
def main():
    ...
    for i in range(max_retries):
        try:
            ...
            print(f"[{i+1}/{max_retries}] Workflow {workflow_id} state: {state} (raw: {raw_state}), temporalStatus: {temporal_status}, dashboardStatus: {dashboard_status}")
            
            # Wait for completion
            if temporal_status in ("completed", "failed", "canceled"):
                print(f"Worker finished task. Final temporalStatus is: {temporal_status}")
    ...
    try:
        ...
        if current_state.get("dashboardStatus") != "completed":
            print(f"Warning: Dashboard status is {current_state.get('dashboardStatus')}, expected 'completed'")
        else:
        ...
        print(f"Execution UI Actions available: {list(k for k, v in actions.items() if v is True)}")
    except Exception as e:

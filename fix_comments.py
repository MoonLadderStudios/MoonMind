...
if "from api_service.api.routers.executions import _get_service, router" in content:
    content = content.replace("from api_service.api.routers.executions import _get_service, router\\n", "")
    content = content.replace("from api_service.api.routers.executions import _get_service, router", "")
    content = content.replace("app.include_router(router)", "app.include_router(executions_module.router)")
    content = content.replace("app.dependency_overrides[_get_service]", "app.dependency_overrides[executions_module._get_service]")
